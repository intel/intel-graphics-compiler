...
#define OPTS_INVALID_SPEC(M)                                                   \
  fatalExitWithMessage("INTERNAL ERROR: INVALID SPEC given to "                \
                       "opts.hpp\n" #M)
...
static bool parseInt(const char *inp, int &val) {
  try {
    ...
    if (inp[0] && inp[0] == '0' && inp[1] && (inp[1] == 'x' || inp[1] == 'X')) {
...
struct ErrorHandler {
  ...
  ErrorHandler(const char *exe, const char *opt) : exeName(exe), optName(opt) {}
  ...
  void operator()(const std::string &msg) const { fail(msg); }
...
template <typename O> struct Opt {
  ...
  bool tryMatch(int argc, const char **argv, int &argIx,
                const ErrorHandler &errHandler, O &opts) {
    ...
    if (isOpt()) {
      ...
      } else if (longName && token[0] == '-' && token[1] == '-' &&
      ...
      if (*value == 0) {
        ...
        } else if (hasAttribute(OPT_FLAG_VAL) && argv[argIx + 1][0] == '-') {
          // -v -p=...
          //  ^
  ...
  void appendHelpMessage(std::ostream &os, int sCw = 0, int lCw = 0,
                         int tCw = 0, bool appendExtDesc = true) const {
    ...
    if (isArg() && hasAttribute(ALLOW_MULTI))
      ...
      typeNameStr += ' ';
    os << "  " << std::left << std::setw(tCw + 1)
       << typeNameStr; //+1 for + or *
    if (description)
    ...
    if (appendExtDesc && extendedDescription) {
      ...
      auto canBreakHere = [&](size_t k) {
        return extendedDescription[k] == ' ' ||
               ...
               extendedDescription[k] == '-' || extendedDescription[k] == ',' ||
               extendedDescription[k] == '.' || extendedDescription[k] == ';';
      };
  ...
  std::string makeHelpMessage(int sCw = 0, int lCw = 0, int tCw = 0,
...
template <typename O> struct Group {
  ...
  void defineFlag(const char *sNm, const char *lNm, const char *desc,
    ...
    defineFlag(sNm, lNm, desc, extDesc, attrs,
               [&](const char *, const ErrorHandler &, O &) { value = true; });
  }
  ...
  void defineOpt(const char *sNm, const char *lNm, const char *type,
    ...
    define(
        sNm, lNm, type, desc, extDesc, attrs,
        [&](const char *inp, const ErrorHandler &, O &) { stringValue = inp; });
  }
...
template <typename O> class CmdlineSpec {
  const char *exeTitle;
  const char *exeName;
...
public:
  CmdlineSpec(const char *title, const char *exe, const char *examps = "",
              bool appendHelpOpt = true)
      : exeTitle(title), exeName(exe), opts(nullptr, nullptr),
        examples(examps) {
    if (appendHelpOpt) {
      defineFlag("h", "help", "shows help on an option",
                 "Without any argument -h will print general help on all "
                 "options.  However, "
                 "if given an argument, -h will attempt to lookup that "
                 "argument.  Options are "
                 "given without preceding - or --.  We check long option names "
                 "first, short "
                 "names second, and finally argument indices last.  Argument "
                 "indices are "
                 ...
                 [&](const char *inp, const ErrorHandler &err, O &) {
                   CmdlineSpec::handleHelpArgument(inp, err, exeTitle, exeName,
                                                   opts, optGroups, args,
                                                   examples);
                 }); // end  defineFlag(...)
  ...
  void handleHelpArgument(const char *inp, const ErrorHandler &err,
    ...
    if (!inp || !*inp) {
      ...
      if (exeTitle)
        std::cerr << exeTitle << "\n";
      CmdlineSpec::appendUsage(std::cerr, opts, groups, args, exeName,
                               examples);
      exit(EXIT_SUCCESS);
    } else {
      ...
      if (opt) {
        ...
        if (opts::parseInt(inp + 1, argIx)) {
          if (argIx - 1 < 0 || argIx - 1 >= (int)args.size()) {
            ...
            args[argIx - 1].appendHelpMessage(std::cerr, 0, 0, 0, true);
            exit(EXIT_SUCCESS);
          }
        } // else e.g. "#1abc"
      } else {
        std::string str = "-h=...: passed unknown option, argument, or group ";
        err(str + inp);
      }
    } // end if *inp != 0
  }
  ...
  static void appendUsage(std::ostream &os, const Group<O> &opts,
    ...
    os << "\n";
    for (const auto &g : groups) {
      os << std::setw(3 + gCw) << std::left << concat("  -", g->prefix, "...");
