...
iga_status_t KernelEncoder::encode(std::ostream &errStr) {
  ...
  enc_opt.autoDepPreserveInsts = m_autoDepPreserveInsts;
  enc_opt.swsbEncodeMode = m_swsbEncodeMode;

  Encoder enc(m_kernel->getModel(), errHandler, enc_opt);
  enc.encodeKernel(*m_kernel, m_kernel->getMemManager(), m_buf, m_binarySize);
#ifdef _DEBUG
...
bool KernelEncoder::patchImmValue(const Model &model, unsigned char *binary,
                                  Type type, const ImmVal &val) {
  // check if the first instruction is compacted and get the instruction length
  // FIXME: compact bit extract code copy from
  // Decoder::getBitField(COMPACTION_CONTROL, 1)
  const uint32_t *inst_start = (const uint32_t *)binary;
  uint32_t mask = (1 << (uint32_t)1) - 1;
  int32_t inst_len =
      ((inst_start[COMPACTION_CONTROL / 32] >> (COMPACTION_CONTROL % 32)) &
       mask) != 0
          ? COMPACTED_SIZE
          : UNCOMPACTED_SIZE;
  ...
  GED_RETURN_VALUE status =
      GED_DecodeIns(ged_model, binary, (uint32_t)inst_len, &ged_inst);
