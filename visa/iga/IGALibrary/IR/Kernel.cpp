...
Kernel::Kernel(const Model &model) : m_model(model), m_mem(4096) {}
...
Block *Kernel::createBlock() { return new (&m_mem) Block(); }

void Kernel::appendBlock(Block *blk) { m_blocks.push_back(blk); }

Instruction *
Kernel::createBasicInstruction(const OpSpec &os, const Predication &predOpnd,
                               const RegRef &freg, ExecSize execSize,
                               ChannelOffset chOff, MaskCtrl mc,
                               FlagModifier condMod, Subfunction sf) {
...
Instruction *Kernel::createBranchInstruction(
    const OpSpec &os, const Predication &predOpnd, const RegRef &flagReg,
    ExecSize execSize, ChannelOffset chOff, MaskCtrl ectr, Subfunction sf) {
  Instruction *inst = new (&m_mem) Instruction(os, execSize, chOff, ectr);
...
Instruction *Kernel::createSendInstruction(const OpSpec &op, SFID sfid,
  ...
  if (immDescsHaveLens) {
    ...
    PayloadLengths lens(p, sfid, inst->getExecSize(), desc.imm);
    dstLen = lens.dstLen;
  }
...
Instruction *Kernel::createNopInstruction() {
  Instruction *inst =
      new (&m_mem) Instruction(m_model.lookupOpSpec(Op::NOP), ExecSize::SIMD1,
...
Instruction *Kernel::createIllegalInstruction() {
  Instruction *inst = new (&m_mem)
      Instruction(m_model.lookupOpSpec(Op::ILLEGAL), ExecSize::SIMD1,
                  ChannelOffset::M0, MaskCtrl::NORMAL);
...
static Instruction *createSyncInstruction(SWSB &sw, const OpSpec &ops,
                                          SyncFC sf, MemManager &mem) {
  Instruction *inst = new (&mem)
      Instruction(ops, ExecSize::SIMD1, ChannelOffset::M0, MaskCtrl::NOMASK);
  inst->setSubfunction(sf);
...
Instruction *Kernel::createSyncNopInstruction(SWSB sw) {
  return createSyncInstruction(sw, m_model.lookupOpSpec(Op::SYNC), SyncFC::NOP,
                               m_mem);
}
Instruction *Kernel::createSyncAllRdInstruction(SWSB sw) {
  return createSyncInstruction(sw, m_model.lookupOpSpec(Op::SYNC),
                               SyncFC::ALLRD, m_mem);
}
