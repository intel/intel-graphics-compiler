...
bool InstCompactor::compactIndex(const CompactionMapping &cm, int immLo,
                                 int immHi) {
  ...
  for (int i = (int)cm.numMappings - 1; i >= 0; i--) {
    ...
    if (rangeContains(immLo, immHi, mappedFragment.offset) ||
      ...
      relevantBits &=
          ~getFieldMask<uint64_t>(indexOffset, mappedFragment.length);
    } else {
      ...
      // bits
      uint64_t uncompactedValue = uncompactedBits.getFragment(mappedFragment);
      iga::setBits(mappedValue, indexOffset, mappedFragment.length,
                   uncompactedValue);
    }
