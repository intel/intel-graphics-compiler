...
namespace iga {
#define GED_ENCODE(FUNC, ARG) GED_ENCODE_TO(FUNC, ARG, &m_gedInst)
...
public:
  ...
  void encodeKernel(Kernel &k, MemManager &m, void *&bits, uint32_t &bitsLen);
...
protected:
  ...
  void encodeTernaryInstruction(const Instruction &inst,
...
protected:
  ...
  void encodeSrcChanSel(GED_SWIZZLE chSelX, GED_SWIZZLE chSelY,
                        GED_SWIZZLE chSelZ, GED_SWIZZLE chSelW);
...
private:
  ...
  // BASIC INSTRUCTIONS
  ///////////////////////////////////////////////////////////////////////
  void encodeBasicInstruction(const Instruction &inst,
                              GED_ACCESS_MODE accessMode);
  void
  encodeBasicDestination(const Instruction &inst, const Operand &dst,
                         GED_ACCESS_MODE accessMode = GED_ACCESS_MODE_Align1);
  template <SourceIndex S>
  void encodeBasicSource(const Instruction &inst, const Operand &src,
                         GED_ACCESS_MODE accessMode = GED_ACCESS_MODE_Align1);
  ///////////////////////////////////////////////////////////////////////
  // BRANCH INSTRUCTIONS
  ///////////////////////////////////////////////////////////////////////
...
template <SourceIndex S>
void Encoder::encodeSrcSubRegNum(uint32_t subRegInByte) {
  if (S == SourceIndex::SRC0) {
...
template <SourceIndex S> void Encoder::encodeSrcMathMacroReg(MathMacroExt a) {
  if (S == SourceIndex::SRC0) {
...
template <SourceIndex S>
void Encoder::encodeSrcReg(RegName regName, uint16_t regNum) {
  uint32_t regBits = 0;
...
void Encoder::encodeSrcRegion(const Region &rgn, bool hasRgnWi) {
  ...
  if (rgn.getVt() == Region::Vert::VT_VxH) {
    ...
    errorT(S == SourceIndex::SRC0 ? "invalid region vertical stride on src0"
                                  : "invalid region vertical stride on src1");
  }

  uint32_t w = static_cast<uint32_t>(rgn.getWi());
  if (rgn.getWi() == Region::Width::WI_INVALID) {
    errorT(S == SourceIndex::SRC0 ? "invalid region width on src0"
  ...
  if (rgn.getHz() == Region::Horz::HZ_INVALID) {
    h = 1;
    errorT(S == SourceIndex::SRC0 ? "invalid region horizontal stride on src0"
...
template <SourceIndex S> void Encoder::encodeSrcRegionHorz(Region::Horz s) {
  if (S == SourceIndex::SRC0) {
