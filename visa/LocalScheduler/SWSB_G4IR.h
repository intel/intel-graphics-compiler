...
struct SBFootprint {
  ...
  bool hasOverlap(const SBFootprint *liveFootprint,
    ...
    for (const SBFootprint *curFootprintPtr = this; curFootprintPtr;
      ...
      for (const SBFootprint *curFootprint2Ptr = liveFootprint;
           curFootprint2Ptr; curFootprint2Ptr = curFootprint2Ptr->next) {
...
public:
  ...
  void finalizeDistanceType1(IR_Builder &builder,
    ...
    if (distDep.size()) {
      ...
      if (distDep.size() > 1) {
        if (diffPipes) // Dependence instructions belong to different ALU
                       // pipelines.
        {
          if (sameOperandType) // But the operand type is from same instruction
                               // pipeline.
          {
            assert(
            ...
            if (mulitpleSamePipe) {
              instVec.front()->setDistanceTypeXe(
...
public:
  ...
  void add(SBBucketNode *bucketNode, int bucket) {
    ...
    if (std::find(nodeVec.begin(), nodeVec.end(), bucketNode) ==
        nodeVec.end()) {
    ...
    for (int curBucket = 0; curBucket < numOfBuckets; curBucket++) {
      if (nodeBucketsArray[curBucket]->size()) {
        ...
        for (SBBucketNode *liveBN : *nodeBucketsArray[curBucket]) {
          std::cerr << " " << liveBN->node->getNodeID() << "("
                    << liveBN->opndNum << ")";
        }
        std::cerr << "\t";
        if ((curBucket + 1) % 8 == 0) {
