...
//
// Set the global ID bit vector of each bucket toucheced by corresponding
// operands
//
void SWSB::SWSBInitializeGlobalNodesInBuckets(
    std::vector<llvm_SBitVector>
  ...
  for (unsigned curBucket = 0; curBucket < kernel.getNumRegTotal();
       curBucket++) {
    llvm_SBitVector dstBitSet;
    llvm_SBitVector srcBitSet;
...
void SWSB::SWSBGlobalTokenGenerator(PointsToAnalysis &p, LiveGRFBuckets &LB,
  ...
  std::vector<llvm_SBitVector> dstGlobalIDs;
  std::vector<llvm_SBitVector> srcGlobalIDs;
  if (!fg.builder->hasRSOrSLMWAs() && //No WA needs check specific instruction
  ...
  for (G4_BB_SB *bb : BBVector) {
    ...
    if (!fg.builder->hasRSOrSLMWAs() && !indexes.DPASIndex) {
...
void G4_BB_SB::setSendGlobalIDMayKilledByCurrentBB(
    std::vector<llvm_SBitVector> &dstTokenBit,
    std::vector<llvm_SBitVector> &srcTokenBit, SBNODE_VECT &SBNodes,
    PointsToAnalysis &p) {
