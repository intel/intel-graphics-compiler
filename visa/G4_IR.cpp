...
bool G4_INST::canPropagateTo(G4_INST *useInst, Gen4_Operand_Number opndNum,
  ...
  if (opndNum == Opnd_src1 && useInst->isSplitSend()) {
    ...
    // Send payloads are raw bytes with no type semantics.  Only a pure-copy
    // MOV (MT == Copy, same type and no conversion) may be propagated; any
    // widening, narrowing, or type-converting MOV must be rejected.
    if (MT != Copy) {
      return false;
    }
