    ...
    */
    ...
    void Optimizer::loadThreadPayload()
    {
        ...
        if (CTIS < 0)
        {
            ...
            // N = inlinedata size
            // Cross thread data size is aligned to dw,
            ...
            localIDsOffset = AlignUp(loadedCrossThreadInputSize + correction, TypeSize(Type_UD));
            localIDsOffset -= useInlineData ? inlineDataSize : 0;

            // cross-thread payload vars
            numCrossThreadDW = AlignUp(loadedCrossThreadInputSize, TypeSize(Type_UD)) / TypeSize(Type_UD);
