    ...
    for (auto I = bb->begin(), E = bb->end(); I != E;) {
      ...
      if (inst->isSend() && inst->getMsgDesc()->getSFID() == SFID::SAMPLER &&
        ...
        if (!isEval && cloneSample && messageType == 0 && numParams == 3) {
          ...
          assert(inst->getExecSize() == g4::SIMD8 ||
...
void replaceAllSpilledRegions(G4_Kernel &kernel, G4_Declare *oldDcl,
                              G4_Declare *newDcl) {
...
void getPhyRegs(G4_Operand *opnd, unsigned int &start, unsigned int &end,
  ...
  if (b && b->isRegVar()) {
    auto r = b->asRegVar();
    if (r->getPhyReg() && r->getPhyReg()->isGreg()) {
      if (!opnd->isAddrExp()) {
...
void computeGlobalFreeGRFs(G4_Kernel &kernel) {
  ...
  std::vector<bool> freeGRFs(
      kernel.getNumRegTotal() * kernel.numEltPerGRF<Type_UB>(), true);
...
void storeGRFAssignments(DECLARE_LIST &declares,
                         std::vector<Assignment> &assignments) {
  for (auto dcl : declares) {
    auto regVar = dcl->getRegVar();
    if (regVar && regVar->isPhyRegAssigned() && regVar->getPhyReg()->isGreg()) {
      Assignment a;
...
void Optimizer::reRAPostSchedule() {
  ...
  if (RAFail || gtpin->getNumFreeGlobalRegs() < freeGRFsBeforeReRA) {
...
void Optimizer::FoldAddrImmediate() {
  ...
  for (G4_BB *bb : fg) {
    ...
    for (ii = bb->begin(); ii != iend; ii++) {
      ...
      if (dst != NULL && dst->getRegAccess() == Direct && src0 != NULL &&
          src0->isSrcRegRegion() &&
          src0->asSrcRegRegion()->getRegAccess() == Direct && dst->isAreg() &&
          dst->isA0() && src0->isAreg() && src0->asSrcRegRegion()->isA0() &&
          src1 == NULL) {
        continue;
      }
      ...
      if (inst->opcode() == G4_add && inst->getExecSize() == g4::SIMD1 &&
          ...
          (src1->isImm() && !src1->isRelocImm()) && dst &&
          dst->getRegAccess() == Direct && src0 && src0->isSrcRegRegion() &&
          src0->asSrcRegRegion()->getRegAccess() == Direct && dst->isAreg() &&
          dst->isA0() && src0->isAreg() && src0->asSrcRegRegion()->isA0() &&
        ...
        for (unsigned i = 0; i < num_srcs; i++) {
          ...
          if (src && src->isSrcRegRegion() &&
              ...
              src->asSrcRegRegion()->isAreg() &&
              src->asSrcRegRegion()->isA0()) {
  ...
  if (rel == Rel_gt || srcElSize != defDstElSize ||
    ...
    if (rel == Rel_gt) {
      ...
      if (src->asSrcRegRegion()->isScalar() || contRegion) {
        ...
        MUST_BE_TRUE(srcRegionDesc->horzStride == 0 ||
  ...
  for (auto BB : fg) {
    for (auto iter = BB->begin(), iterEnd = BB->end(); iter != iterEnd;
      ...
      auto isGoodSrc0Def = [isSrc1Const](G4_INST *def, G4_INST *use,
                                         const IR_Builder &builder) {
        assert(use->getSrc(0)->isSrcRegRegion() &&
  ...
  for (G4_BB *bb : fg) {
    ...
    while (ii != iend) {
      ...
      for (iter = inst->use_begin(); iter != iend1; iter++) {
        ...
        if (inst->getDst()->isA0() && useInst->isSplitSend() &&
...

{
  ...
  if (canFoldOnSIMD1()) {
    ...
    if (inst->getMaskOffset() == 16) {
      MUST_BE_TRUE(inst->getExecSize() == g4::SIMD16,
      ...
      if (inst->getSrc(1) != nullptr) {
        MUST_BE_TRUE(inst->getSrc(1)->isSrcRegRegion() &&
    ...
    if (logicSrc1 == nullptr) {
      MUST_BE_TRUE(inst->opcode() == G4_pseudo_not,
  ...
  for (auto bb : fg) {
    ...
    for (auto iter = bb->begin(), iterEnd = bb->end(); iter != iterEnd;) {
      ...
      if (isDstExtDesc(inst)) {
        ...
        G4_DstRegRegion *dst = inst->getDst();
        if (dst && dst->isAddress()) // If the address register is used for none
                                     // extdesc
        {
          values.clear();
        } else {
...
bool Optimizer::chkNewDefBetweenSends(G4_INST *inst, MSGTableList &msgList,
  ...
  if (inst->getDst() && inst->getDst()->isAddress()) {
    ...
    for (int i = 0; i < inst->getNumSrc(); i++) {
      if (inst->getSrc(i) && inst->getSrc(i)->isAddress()) {
        isDef = myA0.isA0Redef = true;
        break;
      }
    }
...
// not modify the instrppuctions otherwise
...
void Optimizer::resetA0() {
  ...
  for (auto bb : kernel.fg) {
    for (auto inst : *bb) {
      // VxH must be in src0
      if (inst->getSrc(0) != nullptr && inst->getSrc(0)->isSrcRegRegion() &&
...
G4_Declare *Optimizer::createInstsForCallTargetOffset(InstListType &insts,
  ...
  // target
  assert((fcall->getDst()->getLinearizedStart() /
...
private:
  ...
  G4_Type getCommonACCType() {
    ...
    assert((IS_FTYPE(T) || IS_HFTYPE(T) || IS_INT(T)) &&
...
//
// Tried to improve the accuracy of send's footprint to make the footprint be
// just a single DW, but the change has some unknown regression (VMIT-9224). For
// now, revert that change. We need to revisit this in the future.
//
void Optimizer::dce() {
