...
// [NoMask WA]
// EU Fusion introduced a new hardware : fused Mask (2 bits, one for each fused EUs to
// indicate whether EU is on or off) to control NoMask instructions from running on
// off EU. However, there is a hw bug that will not let the fused mask change from 01
// to 00, causing off EU to run NoMask inst that should not run.
//
...
//
// The HW still have the correct CE mask so that the above mov&cmp sequence still works,
// that is, f0.0 will be all zero if no active lanes and will not be zero if there is
...
//  In doing so, f0.1 will be the original value, and the above issue is avoided.
//
...
//   (Note that mul at (2) will not run because the channel enable is off. Thus it
//   modifies the content at offset[4x32], which is wrong.
//
//   Before RA:
//     BB1:
