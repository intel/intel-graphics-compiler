...
public:
  ...
  PhyRegUsageParms(GlobalRA &g, LiveRange *l[], G4_RegFileKind r,
                   unsigned int m, unsigned int &startARF,
                   unsigned int &startFlag, unsigned int &startGRF,
                   unsigned int &bank1_s, unsigned int &bank1_e,
                   unsigned int &bank2_s, unsigned int &bank2_e, bool doBC,
                   bool *avaGReg, uint32_t *avaSubReg, bool *avaAddrs,
                   bool *avaFlags,
                   uint8_t *weakEdges);
};
...
class PhyRegUsage {
  ...
  uint32_t *availableSubRegs; // each entry is a 16-bit value of the words that
                              // are free in a GRF
...
public:
  ...
  uint8_t getWeakEdgeUse(unsigned int reg) const { return weakEdgeUsage[reg]; }
