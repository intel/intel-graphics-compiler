...
class G4_Declare {
  ...
  uint16_t
      PreDefinedVar : 1; // indicate if this dcl is created from preDefinedVars.
...
public:
  ...
  G4_DstRegRegion *getDstRepRegion() const {
    return repRegion->asDstRegRegion();
  }
  G4_SrcRegRegion *getSrcRepRegion() const {
    return repRegion->asSrcRegRegion();
  }
};
