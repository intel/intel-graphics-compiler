...
namespace gtpin
{
    namespace igc
    {
         ...
         *
         * -    The igc_init_t structure should be compatible extension of the igc_info_vN_t structure
         *      for each older version 'N' of the interface. Compatible extension means that
         *      static_cast of a newer structure to an older one is a valid operation.
         *
         * -    Normally, new interface versions do not modify definitions of existing patch tokens.
         *      The recommended paractice is deprecation of an existing token and defining a new one.
         *      In case an existing token has to be modified, make sure the new version of the patch item is
         *      a compatible extension the older one.
         */
 ...
 */
        ...
        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_init_v2_s : igc_init_v1_t {} igc_init_v2_t;

        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_init_v3_s : igc_init_v0_t
        {
            uint32_t     scratch_area_size;      // Number of bytes in the Scratch Space to be reserved for GTPin
            uint32_t     igc_init_size;          // Size of the igc_init_t structure in bytes: sizeof(igc_init_t)
        } igc_init_v3_t;

        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_init_s : igc_init_v3_t {} igc_init_t;
        ...
        {
            igc_init_t          init_status;        // IGC-supported version and status of features requested by GTPin
            uint32_t            num_tokens;         // Number of patch items in the information block
            // igc_token_header_t token_list[];     // Contiguous block of memory containing patch items
        } igc_info_t;
         ...
         */
        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_token_free_reg_info_t : igc_token_header_t
        {
         ...
         */
        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_token_scratch_area_info_t : igc_token_header_t
        {
            ...
            union
            {                                   // Actual number of bytes in the Scratch Space, reserved for GTPin
         ...
         */
        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_token_kernel_start_info_t : igc_token_header_t
        {
        ...
        /*!
         * Patch item detailing the number of GRF registers allocated for each thread of the kernel.
         * Token ID = GTPIN_IGC_TOKEN_NUM_GRF_REGS
         */
        typedef struct GTPIN_IGC_PACK_ATTRIBUTE igc_token_num_grf_regs_t : igc_token_header_t
        {
