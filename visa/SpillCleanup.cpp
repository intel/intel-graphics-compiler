...
void CoalesceSpillFills::copyToOldFills(
    ...
    std::list<std::pair<G4_DstRegRegion*, std::pair<unsigned int, unsigned int>>> indFills,
    ...
    for (auto bb : kernel.fg)
    {
            ...
            instIt++)
        {
            ...
            if (inst->isFillIntrinsic())
            {
                ...
                G4_ExecSize execSize = kernel.getSimdSize() > g4::SIMD16 ? g4::SIMD16 : kernel.getSimdSize();
...
void CoalesceSpillFills::removeRedundantWrites()
{
    ...
    for (auto removeSp : spillToRemove)
    {
        ...
        if ((inst->isSpillIntrinsic() && !isGRFAssigned(inst->asSpillIntrinsic()->getPayload())) ||
            (inst->isFillIntrinsic() && !isGRFAssigned(inst->asFillIntrinsic()->getDst())))
        {
#if 0
