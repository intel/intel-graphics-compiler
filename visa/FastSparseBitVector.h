...
public:
  ...
  void unionWith(const SparseBitVectorElement &RHS) {
    if constexpr (ElementSize % 2 == 0) {
      uint64_t *LHS64Bits = (uint64_t *)Bits;
      uint64_t *RHS64Bits = (uint64_t *)RHS.Bits;
      for (unsigned i = 0; i < BITWORDS_PER_ELEMENT/2; ++i) {
        LHS64Bits[i] |= RHS64Bits[i];
      }
    } else {
      for (unsigned i = 0; i < BITWORDS_PER_ELEMENT; ++i) {
        Bits[i] |= RHS.Bits[i];
