...
public:
  ...
  /// Records a malformed vISA operand and returns VISA_FAILURE. In vISA
  /// generated by IGC such an operand is an IGC bug and still asserts, but vISA
  /// parsed from text can carry user inline assembly, so there the message is
  /// only recorded. Callers must propagate the returned status, otherwise the
  /// error is silently dropped.
  int recordInputError(const char *what);
  ...
  void addFcallInfo(const G4_INST *FcallInst, uint16_t ArgSize,
    ...
    m_fcallInfo.emplace(
        FcallInst, G4_FCALL(ArgSize, RetSize, isUniform, isNoReturn));
  }
