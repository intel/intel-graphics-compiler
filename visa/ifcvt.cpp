...
class IfConverter {
  ...
  getInnermostIfBlock(G4_BB *BB) const {
    ...
    // Only PartialIfConvert is valid here: s1 must be kept.
    ...
    // Restricted to Xe3+ and requires the -ifcvtpartial option.
    if (fg.builder->getPlatform() >= Xe3 &&
        fg.builder->getOption(vISA_ifCvtPartialConvert) &&
        op == G4_goto && s0->getPhysicalSucc() == s1 &&
...
void IfConverter::partialConvert(IfConvertible &IC) {
  vISA_ASSERT(fg.builder->getPlatform() >= Xe3 &&
                  fg.builder->getOption(vISA_ifCvtPartialConvert),
              "partialConvert requires Xe3+ and -ifcvtpartial");
  ...
  // On Xe3+ goto jumps on true (gotoJumpOnTrue() == true unconditionally),
  // so every hoisted instruction receives the negated predicate (-P).
  ...
  // Add empty label to the BB, so that the BB can be handled correctly in
  // following removeEmptyBlocks pass
  s0->markEmpty(fg.builder);
...
void runIfCvt(FlowGraph &fg) {
  ...
  for (auto II = ifList.rbegin(), IE = ifList.rend(); II != IE; ++II) {
    if (II->kind != PartialIfConvert)
      converter.convert(*II);
  }

  // PartialIfConvert removes a BB from the logical CFG (markEmpty), which
  // invalidates physical-adjacency assumptions for other PartialIfConvert
  // entries still in ifList.  Re-analyze after each conversion so that every
  // entry is validated against the current CFG state.
  // Detection in getInnermostIfBlock is already gated on platform and option,
  // so this loop is a no-op when either guard is not satisfied.
  for (;;) {
    ifList.clear();
    converter.analyze(ifList);
    auto it = std::find_if(ifList.rbegin(), ifList.rend(),
                           [](const IfConvertible &IC) {
                             return IC.kind == PartialIfConvert;
                           });
    if (it == ifList.rend())
      break;
