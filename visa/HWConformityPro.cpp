...
void HWConformityPro::fixMadw(INST_LIST_ITER it, G4_BB *bb) {
  ...
  // madw will convert to gen mullh which does not support saturation and
  // must be GRF-aligned for dst.
  ...
  // Dst can't overlap with src2. Otherwise, src2 will be clobbered
  // after generating the expanded code sequence. For example:
  // madw(M1, 16) v0(0,0)<1> v1(0,0)<0;1,0> 0x33778259:ud v0(1,0)<1;1,0>
  // =>
  // mullh (16)  v0(0,0)<1>:ud  v1(0,0)<0;1,0>:ud  0x33778259:ud
  // addc (16)   v0(0,0)<1>:ud  v0(0,0)<1;1,0>:ud  v0(1,0)<1;1,0>:ud {AccWrEn}
  // add (16)    v0(1,0)<1>:ud  acc0.0<1;1,0>:ud  v0(1,0)<1;1,0>:ud
  // After exapnding, the original src2 of madw is now the src1 of addc which
  // is clobbered by dst of mullh.
  bool dstSrc2Overlap = (src2->isSrcRegRegion() &&
                         (src2->compareOperand(dst, builder) != Rel_disjoint &&
                          src2->compareOperand(dst, builder) != Rel_undef));
  if (dstSrc2Overlap) {
    replaceSrcWithRawMov(it, bb, 2, /*stride*/ 0, /*tmpAlign*/ Any);
    src2 = inst->getSrc(2);
  }
  ...
  if (!(src2->isImm() && src2->asImm()->getImm() == 0)) {
    ...
    auto accSrcOpnd = builder.createSrc(
        ...
        tmpType);
    auto addInst =
        builder.createBinOp(G4_add, execSize, builder.duplicateOperand(dstHi32),
                            accSrcOpnd, src1Add, origOptions, false);
...
// must make acc operand is algined to dst. For example:
//   addc (16) r1.1<2> ...
//   mov (16) r2.1<2> acc0.0<1;1,0>
// into
//   addc (16) r1.1 ...
//   mov (16) r3.0<1> acc0.0<1;1,0>
//   mov (16) r1.1<2> r3.0<1>
void HWConformityPro::fixAccRestrictions(INST_LIST_ITER it, G4_BB *bb) {
  ...
  // Make dst as GRF-aligned
  if (!builder.tryToAlignOperand(inst->getDst(), builder.numEltPerGRF<Type_UB>()))
    replaceDstWithRawMov(it, bb, /*tmpStride*/ 0, builder.getGRFAlign());

  if (inst->getExecSize() == g4::SIMD1)
    return;

  // If src operand is acc, must make it aligned with dst
  for (int i = 0; i < inst->getNumSrc(); i++) {
    auto src = inst->getSrc(i);
    if (!src || !src->isAccReg())
      break;
    ...
    uint16_t dstStrideInBytes = inst->getDst()->getExecTypeSize();
    uint16_t srcStrideInBytes = getSrcStrideInBytes(src->asSrcRegRegion());
    if (dstStrideInBytes != srcStrideInBytes)
      replaceSrcWithRawMov(it, bb, i, /*tmpStride*/ 0, builder.getGRFAlign());
  }
}
