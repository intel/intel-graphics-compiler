  ...
  for (auto &[callee, sgInvokeList] : callee2Callers) {
    ...
    for (auto &it : sgInvokeList) {
      ...
      if (removeStackArg) {
        ...
        for (auto calleeIt = calleeInsts.begin();
             calleeIt != calleeInsts.end();) {
          ...
          for (int i = 0, numSrc = inst->getNumSrc(); i < numSrc; ++i) {
            ...
            if (rootDcl == calleeBuilder->getFE_SP()) {
              ...
              if (inst->opcode() == G4_add &&
                ...
                removeDeadCode(loadInst->getSrc(0), calleeInsts,
                               calleeBuilder->getFE_SP());
...
void CISA_IR_Builder::summarizeFunctionInfo(
  ...
  for (VISAKernelImpl *mfunc : m_kernelsAndFunctions) {
    ...
    // - TGL and prior platfroms if hasStackCall
    //    Workaround unknown TGL issue that smaller scratch size caused
    //    regressions.
    ...
    if (funcInfoMap[mfunc] || hasRecursion ||
        (getPlatform() <= GENX_TGLLP && subFunction.size()))
