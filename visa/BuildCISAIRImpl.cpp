...
int CISA_IR_Builder::Compile(const char *isaasmFileName, bool emit_visa_only) {
  ...
  if (IS_GEN_BOTH_PATH) {
    ...
    for (int i = 0; iter != iend; iter++, i++) {
      ...
      // needed after Phase 1 compilation, to reduce peak memory.
...
bool CISA_IR_Builder::CISA_create_info_3d_instruction(
    VISA_opnd* pred, VISASampler3DSubOpCode subOpcode, VISA_EMask_Ctrl emask,
    unsigned exec_size, ChannelMask channel, const char* surfaceName,
    unsigned int surfaceIndex, VISA_opnd* lod, VISA_opnd* dst, int lineNum) {
  ...
  VISA_Exec_Size executionSize = Get_VISA_Exec_Size_From_Raw_Size(exec_size);
  VISA_CALL_TO_BOOL(AppendVISA3dInfo, subOpcode, (VISA_PredOpnd*)pred, emask, executionSize,
...
bool CISA_IR_Builder::createSample4Instruction(
  ...
  int status = m_kernel->AppendVISA3dGather4(
      ...
      (VISA_PredOpnd *)pred, emask, executionSize, channel.getSingleChannel(),
      (VISA_VectorOpnd *)aoffimmi, sampler, samplerIndex, surface, surfaceIndex,
...
bool CISA_IR_Builder::create3DSampleInstruction(
    ...
    ChannelMask channels, VISA_EMask_Ctrl emask, unsigned exec_size,
    VISA_opnd *aoffimmi, const char *samplerName, unsigned int samplerIdx,
    const char *surfaceName, unsigned int surfaceIdx, VISA_opnd *pairedSurface,
