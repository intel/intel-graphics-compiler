...
// DO NOT USE ASSERT MACRO DIRECTLY IN vISA CODEBASE
#define ASSERT(x, errormsg, causemsg) \
  assert_and_exit(x, errormsg, causemsg)
...
#define vISA_ASSERT(x, causemsg) \
  ASSERT(x, "error, assertion failed", causemsg)
#define vISA_ASSERT_UNREACHABLE(causemsg) \
  ASSERT(false, "unreachable state, assertion failed", causemsg)
#define vISA_ASSERT_INPUT(x, causemsg) \
  ASSERT(x, "input error, assertion failed", causemsg)
