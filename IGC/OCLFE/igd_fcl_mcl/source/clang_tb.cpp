...
#include "../headers/fcl_pch_cache_config.h"
...
// Build-time SHA-256 digest of CTHeader.h, generated by header_id_gen.py.
extern "C" {
extern const char CTHeaderId[];
}
...
// Per-module attempt to enable opencl-clang's persistent auto-PCH disk cache.
// Guarded by a flag on the module instance rather than a process-lifetime
// static, so a freshly loaded/reloaded module can be configured again.
// Never fails Create(): a false/no-op result here simply leaves the disk
// cache unconfigured and FCL falls back to its existing in-memory-only
// behavior.
static bool EnsureAutoPCHDiskCacheConfigured(CCModuleStruct &CCModule) {
  if (CCModule.autoPCHDiskCacheConfigureAttempted)
    return false;
  CCModule.autoPCHDiskCacheConfigureAttempted = true;

  if (!CCModule.pConfigureAutoPCHDiskCache)
    return false; // older opencl-clang build: symbol not present

#if defined(IGC_DEBUG_VARIABLES)
  DWORD mode = 0;
  ReadIGCRegistry("EnableFclPchCache", &mode, sizeof(mode), IGCFlagType_int);
  if (static_cast<IGC::FclPchCacheMode>(mode) != IGC::FclPchCacheMode::OnDisk)
    return false;

  std::string dir = ResolveFclPchCacheDir();
  if (dir.empty())
    return false;

  return CCModule.pConfigureAutoPCHDiskCache(dir.c_str(), ResolveFclPchCacheMaxSizeBytes(), CTHeaderId);
#else
  return false;
#endif
}
    ...
    if (overrideName[0] != '\0') {
      ...
      if (CCModule.pModule) {
        ...
        if (!CCModule.pCompile) {
          ...
          CCModule.pConfigureAutoPCHDiskCache =
              (CCModuleStruct::PFcnCCConfigureAutoPCHDiskCache)dlsym(CCModule.pModule, "ConfigureAutoPCHDiskCache");
        }
      } else {
    ...
    if (success && CCModule.pCompile == nullptr) {
      ...
      if (CCModule.pModule != nullptr) {
        ...
        if (!success) {
          ...
          CCModule.pConfigureAutoPCHDiskCache = (CCModuleStruct::PFcnCCConfigureAutoPCHDiskCache)GetProcAddress(
              (HMODULE)CCModule.pModule, "ConfigureAutoPCHDiskCache");
        }
      } else {
...
#else
    if (success && !CCModule.isDynamicallyLoaded) {
      CCModule.pConfigureAutoPCHDiskCache =
          (CCModuleStruct::PFcnCCConfigureAutoPCHDiskCache)dlsym(RTLD_DEFAULT, "ConfigureAutoPCHDiskCache");
    }
#endif

    if (success) {
      EnsureAutoPCHDiskCacheConfigured(CCModule);
    }
  }
...
void CClangTranslationBlock::EnsureProperPCH(TranslateClangArgs *pArgs, const char *pInternalOptions,
  ...
  if (m_cthBuffer) {
    ...
    if (IGC_GET_FLAG_VALUE(EnableFclPchCache) != 0) {
      pArgs->optionsEx.append(" --auto-pch");
    }
  }
}
