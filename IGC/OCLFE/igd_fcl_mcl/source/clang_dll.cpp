...
TRANSLATION_BLOCK_API void Register(STB_RegisterArgs *pRegisterArgs) {
  ...
  uint32_t maxNumTranslationCodes = sizeof(g_cClangTranslationCodes) / sizeof(STB_TranslationCode);

  if (pRegisterArgs->pTranslationCodes != NULL) {
    memcpy_s(pRegisterArgs->pTranslationCodes, pRegisterArgs->NumTranslationCodes * sizeof(STB_TranslationCode),
             g_cClangTranslationCodes, sizeof(g_cClangTranslationCodes));
  } else {
...
TRANSLATION_BLOCK_API void Delete(CTranslationBlock *pTranslationBlock) {
  CClangTranslationBlock *pClangTranslationBlock = (CClangTranslationBlock *)pTranslationBlock;

  CClangTranslationBlock::Delete(pClangTranslationBlock);
}
