...
} __ocl_svml_internal_dasinh_ha_data_t;
static __ocl_svml_internal_dasinh_ha_data_t __ocl_svml_internal_dasinh_ha_data =
    {
        /* Log_HA_table */
...
static __constant _iml_v2_dp_union_t __dasinh_ha_CoutTab[4] = {
...
// For infinity, we return the same infinity
// For NaN, just return a NaN.
__attribute__((always_inline)) inline int
__ocl_svml_internal_dasinh_ha(double *a, double *r) {
  int nRet = 0;
  double yM;
  double purex, y, yN;
  ...
  purex = (*a);
  {
    ...
    // Otherwise it's infinity, so just return the same thing
    else {
      (*r) = purex;
      return nRet;
    }
...
double __ocl_svml_asinh_ha(double x) {
  double r;
  unsigned int vm;
  double va1;
  double vr1;
  va1 = x;
  {
    ...
    One = as_double(__ocl_svml_internal_dasinh_ha_data.One);
    SgnMask = as_double(__ocl_svml_internal_dasinh_ha_data.SgnMask);
    XThreshold = as_double(__ocl_svml_internal_dasinh_ha_data.XThreshold);
    XhMask = as_double(__ocl_svml_internal_dasinh_ha_data.XhMask);
    ...
    dLargestFinite =
        as_double(__ocl_svml_internal_dasinh_ha_data.dLargestFinite);
    dSpecialMask = as_double((unsigned long)((
        (!(dInput <= dLargestFinite)) ? 0xffffffffffffffff : 0x0)));
    ...
    dBigThreshold = as_double(__ocl_svml_internal_dasinh_ha_data.dBigThreshold);
    ...
    dTmp3 = (dTmp2 + dX2Hi);
    dTmp3 = (dTmp3 + dX2Lo);
    dTopMask29 = as_double(__ocl_svml_internal_dasinh_ha_data.dTopMask29);
    ...
    dLittleThreshold =
        as_double(__ocl_svml_internal_dasinh_ha_data.dLittleThreshold);
    dLittleMask = as_double((unsigned long)((dInput <= dLittleThreshold)
                                                ? 0xffffffffffffffff
                                                : 0x0));
    dTmp5 = as_double((((~as_ulong(dLittleMask)) & as_ulong(dTmp5)) |
                       (as_ulong(dLittleMask) & as_ulong(dTmpf1))));
    dTmp7 = as_double((((~as_ulong(dLittleMask)) & as_ulong(dTmp7)) |
                       (as_ulong(dLittleMask) & as_ulong(dTmpf3))));
    ...
    // case.
    XScale = as_double(__ocl_svml_internal_dasinh_ha_data.XScale);
    ...
    ExpMask = as_double(__ocl_svml_internal_dasinh_ha_data.ExpMask);
    Two10 = as_double(__ocl_svml_internal_dasinh_ha_data.Two10);
    /* preserve mantissa, set input exponent to 2^(-10) */
    Mantissa = as_double((as_ulong(X) & as_ulong(ExpMask)));
    Mantissa = as_double((as_ulong(Mantissa) | as_ulong(Two10)));
    MinLog1p = as_double(__ocl_svml_internal_dasinh_ha_data.MinLog1p);
    MaxLog1p = as_double(__ocl_svml_internal_dasinh_ha_data.MaxLog1p);
    ExpMask0 = (__ocl_svml_internal_dasinh_ha_data.ExpMask0);
    ExpMask2 = (__ocl_svml_internal_dasinh_ha_data.ExpMask2);
    ...
    dThirtyOne = as_double(__ocl_svml_internal_dasinh_ha_data.dThirtyOne);
    ...
    THL[0] = as_double(
        ((unsigned long *)((char *)(&__ocl_svml_internal_dasinh_ha_data
                                         .Log_HA_table[0]) -
                           0x810000))[Index >> 3]);
    THL[1] = as_double(
        ((unsigned long *)((char *)(&__ocl_svml_internal_dasinh_ha_data
    ...
    poly_coeff[2] = as_double(__ocl_svml_internal_dasinh_ha_data.poly_coeff[2]);
    ...
    dSign = as_double(__ocl_svml_internal_dasinh_ha_data.dSign);
    // Sign = x & Sign
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_dasinh_ha(&__cout_a1, &__cout_r1);
