...
__ocl_svml_internal_dcos_ha(double *a, double *pres) {
  ...
  dS.f = __spirv_ocl_fma(x.f, (__dcos_ha_INV_PI64).f,
                                                (__dcos_ha_SHIFTER).f);
  // N ~ x*(1/pi)
  ...
  lindex = dS.w & 1;
  // Rh = x - N*PI1
  Rh = __spirv_ocl_fma(dN, (__dcos_ha_PI1_BITS).f, x.f);
  // Rm = Rh - N*PI2
  Rm = __spirv_ocl_fma(dN, (__dcos_ha_PI2_BITS).f,
                                              Rh); // FENCE(Rh - dNP2);
  // R = Rm - N*PI3
  R = __spirv_ocl_fma(dN, (__dcos_ha_PI3_BITS).f,
                                             Rm); // FENCE(Rm - dNP3);
  // (N*PI2)_high
  dNP2_h = Rh - Rm;
  // R^2
...
COS_MAIN_PATH:
  ...
  // 2.0 - R^2
  Ch = __spirv_ocl_fma((-R), R, (__dcos_ha_two).f);
  // (-R^2)_high
  R2h = Ch - (__dcos_ha_two).f;
  // (R^2)_low
  R2l = __spirv_ocl_fma(R, R, R2h);
  // 0.5*R2l+ R*Rl
  R2l = __spirv_ocl_fma(R2l, (__dcos_ha_half).f, R_Rl);
  c_poly2.f = __spirv_ocl_fma(R2, c_poly, (-R2l));
  c_poly2.f =
      __spirv_ocl_fma(Ch, (__dcos_ha_half).f, c_poly2.f);
...
COS_SPECIAL:
  ...
  // R*Rl
  R_Rl = R * Rl;
  c_poly = __spirv_ocl_fma(R2, (__dcos_ha_c_coeff[7]).f,
                                                  (__dcos_ha_c_coeff[6]).f);
  s_poly = __spirv_ocl_fma(R2, (__dcos_ha_s_coeff[7]).f,
  ...
  res[0].f =
      __spirv_ocl_fma(Ch, (__dcos_ha_half).f, c_poly);
