...
} __ocl_svml_internal_dlog1p_ha_data_gpu_t;
static __ocl_svml_internal_dlog1p_ha_data_gpu_t
    __ocl_svml_internal_dlog1p_ha_data_gpu = {
        // OneEighth = 0.125
        ...
        0x3fd2d69f78622232uL, 0x3fc2896ff3f66bbduL, 0x3fc8c588ce05a097uL,
        0x3fd013265a806008uL, 0x3fd55613246dd7cfuL, 0x3fdfffdc599639e8uL,
        // Coefficients for polynomial p
        0xbfbabc3280883f85uL, 0x3fb7569b705ccf22uL, 0xbfb98321c986c73fuL,
        0x3fbc725277fd96beuL, 0xbfc00011b12ec07fuL, 0x3fc2492404508eb2uL,
        0xbfc5555548c81da2uL, 0x3fc9999999f242c7uL, 0xbfd000000001dd61uL,
        0x3fd5555555554d65uL, 0xbfdfffffffffffd9uL,
...
static __constant _iml_v2_dp_union_t __dlog1p_ha_CoutTab[210] = {
...
__ocl_svml_internal_dlog1p_ha(double *a0, double *r) {
  ...
  if ((((((_iml_v2_dp_union_t *)&a[0])->dwords.hi_dword >> 20) & 0x7FF) !=
    ...
    */
    ...
    if (((((_iml_v2_dp_union_t *)&x)->dwords.hi_dword >> 20) & 0x7FF) == 0) {
      ...
      x *= ((__constant double *)__dlog1p_ha_CoutTab)[200];
      iN -= 60;
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((__constant double *)__dlog1p_ha_CoutTab)[201]) {
      ...
      if (dbAbsU > ((__constant double *)__dlog1p_ha_CoutTab)[199]) {
        /* 6) "Main" path */
        /* a) Range reduction */
        ...
        */
        dbNLn2Hi = (dbN * ((__constant double *)__dlog1p_ha_CoutTab)[195]);
        dbNLn2Lo = (dbN * ((__constant double *)__dlog1p_ha_CoutTab)[196]);
        /* Get y */
        y = x;
        ...
        /* Obtain j */
        dbTmp = (y + ((__constant double *)__dlog1p_ha_CoutTab)[197]);
        j = (((_iml_v2_dp_union_t *)&dbTmp)->dwords.lo_dword) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of RcprY, LnRcprYHi, LnRcprYLo */
        dbRcprY = ((__constant double *)__dlog1p_ha_CoutTab)[3 * (j)];
        dbLnRcprYHi = ((__constant double *)__dlog1p_ha_CoutTab)[3 * (j) + 1];
        dbLnRcprYLo = ((__constant double *)__dlog1p_ha_CoutTab)[3 * (j) + 2];
        /* Calculate WHi and WLo */
        dbWHi = (dbNLn2Hi + dbLnRcprYHi);
        dbWLo = (dbNLn2Lo + dbLnRcprYLo);
        /* Get YHi and YLo */
        dbTmp = (y + ((__constant double *)__dlog1p_ha_CoutTab)[198]);
        dbYHi = (dbTmp - ((__constant double *)__dlog1p_ha_CoutTab)[198]);
        dbYLo = (y - dbYHi);
        /* Get UHi, uLo and U */
        ...
        u = (dbUHi + dbuLo);
        /* b) Approximation */
        dbP = (((((((__constant double *)__dlog1p_ha_CoutTab)[209] * u +
                   ((__constant double *)__dlog1p_ha_CoutTab)[208]) *
                      u +
                  ((__constant double *)__dlog1p_ha_CoutTab)[207]) *
                     u +
                 ((__constant double *)__dlog1p_ha_CoutTab)[206]) *
                    u +
                ((__constant double *)__dlog1p_ha_CoutTab)[205]) *
                   u +
               ((__constant double *)__dlog1p_ha_CoutTab)[204]) *
                  u +
              ((__constant double *)__dlog1p_ha_CoutTab)[203];
        dbP = (dbP * u * u);
        /* c) Reconstruction */
        dbResHi = (dbWHi + dbUHi);
      ...
      if (x == ((__constant double *)__dlog1p_ha_CoutTab)[201]) {
        ...
        r[0] = -((__constant double *)__dlog1p_ha_CoutTab)[202] /
               ((__constant double *)__dlog1p_ha_CoutTab)[201];
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        r[0] = ((__constant double *)__dlog1p_ha_CoutTab)[201] /
               ((__constant double *)__dlog1p_ha_CoutTab)[201];
        nRet = 1;
      }
    }
...
double __ocl_svml_log1p_ha(double x) {
  double r;
  unsigned int vm;
  double va1;
  double vr1;
  va1 = x;
  {
    ...
    OneEighth = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.OneEighth);
    t = (OneEighth * va1);
    t = (t + OneEighth);
    ...
    // 2^-1023 <= s <= 2^53
    //
    ExpMask = (__ocl_svml_internal_dlog1p_ha_data_gpu.ExpMask);
    ExpInv = (__ocl_svml_internal_dlog1p_ha_data_gpu.ExpInv);
    lt = as_ulong(t);
    te = (lt & ExpMask);
    ls = (ExpInv - te);
    s = as_double(ls);
    ...
    //
    OneHalf = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.OneHalf);
    tt = (s * t);
    tt = (tt + OneHalf);
    ...
    //
    Eight = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.Eight);
    ConvExp = (__ocl_svml_internal_dlog1p_ha_data_gpu.ConvExp);
    ConvConst = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.ConvConst);
    sss = (ss * Eight);
    ls = as_ulong(sss);
    ...
    //
    One = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.One);
    q = (One - ss);
    ...
    //
    Shifter = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.Shifter);
    ThreeQuarters =
        as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.ThreeQuarters);
    sm = (Shifter + r);
    mm = (sm - Shifter);
    m = (mm * ThreeQuarters);
    // Compute
    ...
    //
    // For LA and EP, we do not need the low part.
    //
    // The subtraction 1 - m is exact (only a couple of
    // cases to test).
    //
    a = (One - m);
    mpzhi = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(r, a, 0.0);
    ...
    //
    du = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD7);
    dg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD6);
    cdu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD5);
    cdg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD4);
    du = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(v, du, cdu);
    dg = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(v, dg, cdg);
    cdu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD3);
    cdg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffD2);
    du = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(v, du, cdu);
    dg = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(v, dg, cdg);
    dd = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(m, du, dg);
    // Polynomial evaluation of p(z) = z + w * pp(z)
    //
    pgg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP12);
    puu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP11);
    pug = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP10);
    pgu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP9);
    cpgg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP8);
    cpuu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP7);
    cpug = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP6);
    cpgu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP5);
    ...
    cpgg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP4);
    cpuu = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP3);
    cpug = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.CoeffP2);
    pgg = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(ww, pgg, cpgg);
    puu = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(ww, puu, cpuu);
    pug = SPIRV_OCL_BUILTIN(fma, _f64_f64_f64, )(ww, pug, cpug);
    ...
    //
    // For LA and EP, we need log(2) only on 1 FP constant
    //
    LogTwoHi = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.LogTwoHi);
    LogTwoLo = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.LogTwoLo);
    lkhi = (k * LogTwoHi);
    lklo = (k * LogTwoLo);
    // Here we have to do the final summation
    //
    // log(1 + x) ~= lkhi + lklo + m + v * dd + z + w * pp
    //             = (lkhi + mpzhi) + (mpzlo + lklo + v * dd + w * pp)
    //
    // For LA and EP, we sum differently.
    //
    yhi = (lkhi + mpzhi);
    tts = (yhi - lkhi);
    tlo = (mpzhi - tts);
    ...
    MinArg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.MinArg);
    MaxArg = as_double(__ocl_svml_internal_dlog1p_ha_data_gpu.MaxArg);
    MaskMin =
        as_double((unsigned long)((va1 < MinArg) ? 0xffffffffffffffff : 0x0));
    MaskMax = as_double(
        (unsigned long)(((!(va1 <= MaxArg)) ? 0xffffffffffffffff : 0x0)));
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_dlog1p_ha(&__cout_a1, &__cout_r1);
