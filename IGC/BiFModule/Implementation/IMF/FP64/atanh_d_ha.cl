...
} __ocl_svml_internal_datanh_ha_data_t;
static __ocl_svml_internal_datanh_ha_data_t __ocl_svml_internal_datanh_ha_data =
    {
        /* Log_HA_table */
...
static __constant _iml_v2_dp_union_t __datanh_ha_CoutTab[4] = {
...
__ocl_svml_internal_datanh_ha(double *a, double *r) {
  int nRet = 0;
  double yM;
  double absx, y, yN;
       ...
       ((_iml_uint32_t)(0) << 31));
  {
    ...
    if ((((_iml_v2_dp_union_t *)&(absx))->hex[0] ==
         ((__constant _iml_v2_dp_union_t *)&(
              ((__constant double *)__datanh_ha_CoutTab)[0]))
             ->hex[0]) &&
        (((_iml_v2_dp_union_t *)&(absx))->hex[1] ==
         ((__constant _iml_v2_dp_union_t *)&(
              ((__constant double *)__datanh_ha_CoutTab)[0]))
             ->hex[1])) {
      (*r) = (*a) / (((__constant double *)__datanh_ha_CoutTab)[1]);
      nRet = 2;
      return nRet;
    }
    // Otherwise it's |x| > 1 (including infinity), so return NaN + invalid
    {
      (*r) = ((__constant double *)__datanh_ha_CoutTab)[2] *
             (((__constant double *)__datanh_ha_CoutTab)[1]);
      nRet = 1;
      return nRet;
    }
  }
}
double __ocl_svml_atanh_ha(double x) {
  double r;
  unsigned int vm;
  double va1;
  double vr1;
  va1 = x;
  {
    ...
    One = as_double(__ocl_svml_internal_datanh_ha_data.One);
    SgnMask = as_double(__ocl_svml_internal_datanh_ha_data.SgnMask);
    XThreshold = as_double(__ocl_svml_internal_datanh_ha_data.XThreshold);
    XhMask = as_double(__ocl_svml_internal_datanh_ha_data.XhMask);
    ...
    dSpecialMask = as_double(
        (unsigned long)(((!(dInput < One)) ? 0xffffffffffffffff : 0x0)));
    ...
    dUTmp = (One - dU);
    dUTmp = (dUTmp - dInput);
    dTopMask41 = as_double(__ocl_svml_internal_datanh_ha_data.dTopMask41);
    ...
    Xl = (A - X);
    Xl = (Xl + B);
    ExpMask = as_double(__ocl_svml_internal_datanh_ha_data.ExpMask);
    Two10 = as_double(__ocl_svml_internal_datanh_ha_data.Two10);
    /* preserve mantissa, set input exponent to 2^(-10) */
    Mantissa = as_double((as_ulong(X) & as_ulong(ExpMask)));
    Mantissa = as_double((as_ulong(Mantissa) | as_ulong(Two10)));
    MinLog1p = as_double(__ocl_svml_internal_datanh_ha_data.MinLog1p);
    MaxLog1p = as_double(__ocl_svml_internal_datanh_ha_data.MaxLog1p);
    ExpMask0 = (__ocl_svml_internal_datanh_ha_data.ExpMask0);
    ExpMask2 = (__ocl_svml_internal_datanh_ha_data.ExpMask2);
    ...
    THL[0] = as_double(
        ((unsigned long *)((char *)(&__ocl_svml_internal_datanh_ha_data
                                         .Log_HA_table[0]) -
                           0x810000))[Index >> 3]);
    THL[1] = as_double(
        ((unsigned long *)((char *)(&__ocl_svml_internal_datanh_ha_data
    ...
    poly_coeff[2] = as_double(__ocl_svml_internal_datanh_ha_data.poly_coeff[2]);
    ...
    dHalf = as_double(__ocl_svml_internal_datanh_ha_data.dHalf);
    // Half = Half^Sign
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_datanh_ha(&__cout_a1, &__cout_r1);
