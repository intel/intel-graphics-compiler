...
//
//           (3) Calculate P0(x2), where x2 is approximation of x^2:
//
//                 x2 := dbXXHi+dbXXLo
//
...
//               to integer we use "right shifter" technique:
//
//                   TmpJ1 := <M1Hi + RSJ>,
//
...
static __constant union {
  ...
  float f;
  int i;
} __sasin_ha_c5 = {0x3d2edc07u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_c4 = {0x3cc32a6bu};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_c3 = {0x3d3a9ab4u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_c2 = {0x3d997c12u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_c1 = {0x3e2aaaffu};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_c0 = {0xb19b5f15u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_small_float = {0x01800000u};
// 2.0
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_two = {0x40000000u};
// pi/2
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_pi2h = {0x3FC90FDBu};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __sasin_ha_pi2l = {0xB33BBD2Eu};
__attribute__((always_inline)) inline int
__ocl_svml_internal_sasin_ha(float *pxin, float *pres) {
  int nRet = 0;
  float xin = *pxin;
  union {
    unsigned int w;
  ...
  // float asinf(float xin)
  {
    ...
    // rsqrt((1-|x|)/2)
    RS.f = 1.0f /
           SPIRV_OCL_BUILTIN(sqrt, _f32, )((y.f + __sasin_ha_small_float.f));
    // Sh ~ sqrt((1-|x|)/2)
    Sh = y.f * RS.f;
    // pi/2_high - 2*Sh
    High.f = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )((-Sh), __sasin_ha_two.f,
                                                    __sasin_ha_pi2h.f);
    //  Shh2 = (-2*Sh)_high
    Shh2.f = High.f - __sasin_ha_pi2h.f;
    Shh = 0.5f * Shh2.f;
    // y.f - Sh*Sh
    E = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )((-Shh), Shh, y.f);
    // Low = (pi/2)_low - low part of 2*sqrt((1-|x|)/2)
    Low.f =
        SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )((-E), RS.f, __sasin_ha_pi2l.f);
    // polynomial
    poly = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(__sasin_ha_c5.f, R,
                                                  __sasin_ha_c4.f);
    poly = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(poly, R, __sasin_ha_c3.f);
    poly = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(poly, R, __sasin_ha_c2.f);
    poly = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(poly, R, __sasin_ha_c1.f);
    poly = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(poly, R, __sasin_ha_c0.f);
    // R0 = sel_mask? Shh2 : xa.f
    R0.f = (xa.f <= 0.5f) ? xa.f : Shh2.f;
    // High = sel_mask ? High : xa.w
    High.f = (xa.f <= 0.5f) ? xa.f : High.f;
    // Low = 0 for sel-mask=0
    Low.f = (xa.f <= 0.5f) ? 0 : Low.f;
    // poly*R + Low
...
float __ocl_svml_asinf_ha(float x) {
  float r;
  __ocl_svml_internal_sasin_ha(&x, &r);
  return r;
}
