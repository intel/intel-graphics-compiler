...
} __ocl_svml_internal_sln_ha_data_t;
static __ocl_svml_internal_sln_ha_data_t __ocl_svml_internal_sln_ha_data = {
    {
...
static __constant _iml_v2_sp_union_t __sln_ha_CoutTab[210] = {
...
__attribute__((always_inline)) inline int __ocl_svml_internal_sln_ha(float *a,
                                                                     float *r) {
  float fap1, *ap1 = &fap1;
  float x, y, u;
  float fP;
  ...
  if ((((((_iml_v2_sp_union_t *)&a[0])->hex[0] >> 23) & 0xFF) != 0xFF)) {
    ...
    */
    ...
    if (((((_iml_v2_sp_union_t *)&x)->hex[0] >> 23) & 0xFF) == 0) {
      ...
      x = (x * ((__constant float *)__sln_ha_CoutTab)[200]);
      iN -= (27);
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((__constant float *)__sln_ha_CoutTab)[201]) {
      ...
      if (fAbsU > ((__constant float *)__sln_ha_CoutTab)[199]) {
        /* 6) "Main" path */
        /* a) Range reduction */
        ...
        */
        if (isDenorm == 1) {
          fNLn2Hi = (fN * (((__constant float *)__sln_ha_CoutTab)[195] +
                           ((__constant float *)__sln_ha_CoutTab)[196]));
          fNLn2Lo = 0.0f;
        } else {
          fNLn2Hi = (fN * ((__constant float *)__sln_ha_CoutTab)[195]);
          fNLn2Lo = (fN * ((__constant float *)__sln_ha_CoutTab)[196]);
        }
        /* Get y */
        y = x;
        ...
        /* Obtain j */
        fTmp = (y + ((__constant float *)__sln_ha_CoutTab)[197]);
        j = (((_iml_v2_sp_union_t *)&fTmp)->hex[0] & 0x007FFFFF) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of RcprY, LnRcprYHi, LnRcprYLo */
        fRcprY = ((__constant float *)__sln_ha_CoutTab)[3 * (j)];
        fLnRcprYHi = ((__constant float *)__sln_ha_CoutTab)[3 * (j) + 1];
        fLnRcprYLo = ((__constant float *)__sln_ha_CoutTab)[3 * (j) + 2];
        /* Calculate WHi and WLo */
        fWHi = (fNLn2Hi + fLnRcprYHi);
        fWLo = (fNLn2Lo + fLnRcprYLo);
        /* Get YHi and YLo */
        fTmp = (y + ((__constant float *)__sln_ha_CoutTab)[198]);
        fYHi = (fTmp - ((__constant float *)__sln_ha_CoutTab)[198]);
        fYLo = (y - fYHi);
        /* Get UHi, uLo and U */
        ...
        // printf("u = %g\n", u);
        /* b) Approximation */
        fP = ((((((((__constant float *)__sln_ha_CoutTab)[209] * u +
                   ((__constant float *)__sln_ha_CoutTab)[208]) *
                      u +
                  ((__constant float *)__sln_ha_CoutTab)[207]) *
                     u +
                 ((__constant float *)__sln_ha_CoutTab)[206]) *
                    u +
                ((__constant float *)__sln_ha_CoutTab)[205]) *
                   u +
               ((__constant float *)__sln_ha_CoutTab)[204]) *
                  u +
              ((__constant float *)__sln_ha_CoutTab)[203]);
        fP = (fP * u * u);
        /* c) Reconstruction */
        fResHi = (fWHi + fUHi);
        fResLo = ((fWLo + fuLo) + fP);
      ...
      if (x == ((__constant float *)__sln_ha_CoutTab)[201]) {
        ...
        r[0] = -((__constant float *)__sln_ha_CoutTab)[202] /
               ((__constant float *)__sln_ha_CoutTab)[201];
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        r[0] = ((__constant float *)__sln_ha_CoutTab)[201] /
               ((__constant float *)__sln_ha_CoutTab)[201];
        nRet = 1;
      }
    }
...
float __ocl_svml_logf_ha(float x) {
  float r;
  unsigned int vm;
  float va1;
  float vr1;
  va1 = x;
  {
    ...
    unsigned int iR;
    float sP;
    iHiDelta = (__ocl_svml_internal_sln_ha_data.iHiDelta);
    iLoRange = (__ocl_svml_internal_sln_ha_data.iLoRange);
    iX = as_uint(va1);
    ...
    iRangeMask = ((unsigned int)(-(signed int)((signed int)iXTest <
                                               (signed int)iLoRange)));
    iBrkValue = (__ocl_svml_internal_sln_ha_data.iBrkValue);
    iOffExpoMask = (__ocl_svml_internal_sln_ha_data.iOffExpoMask);
    ...
    vm = 0;
    vm = iRangeMask;
    sOne = as_float(__ocl_svml_internal_sln_ha_data.sOne);
    /* reduced argument R */
    sR = (sR - sOne);
    sPoly[8] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[8]);
    sPoly[7] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[7]);
    /* polynomial evaluation starts here */
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sPoly[8], sR, sPoly[7]);
    sPoly[6] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[6]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[6]);
    sPoly[5] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[5]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[5]);
    sPoly[4] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[4]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[4]);
    sPoly[3] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[3]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[3]);
    sPoly[2] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[2]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[2]);
    sPoly[1] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[1]);
    sP = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sP, sR, sPoly[1]);
    sPoly[0] = as_float(__ocl_svml_internal_sln_ha_data.sPoly[0]);
    ...
    sLn2Hi = as_float(__ocl_svml_internal_sln_ha_data.sLn2Hi);
    sLn2Lo = as_float(__ocl_svml_internal_sln_ha_data.sLn2Lo);
    /* final reconstruction: */
    /* add exponent_value*(log2_hi + log2_lo) to polynomial result */
    vr1 = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sN, sLn2Lo, sP);
    vr1 = SPIRV_OCL_BUILTIN(fma, _f32_f32_f32, )(sN, sLn2Hi, vr1);
  }
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_sln_ha(&__cout_a1, &__cout_r1);
