...
} __scos_ep_ReductionTab_t;
static __scos_ep_ReductionTab_t __ocl_svml_internal_scos_ep_reduction_data = {{
    /*     P_hi                  P_med               P_lo                */
...
typedef struct {
  ...
  unsigned int _sOne;
} __ocl_svml_internal_scos_ep_data_t;
static __ocl_svml_internal_scos_ep_data_t __ocl_svml_internal_scos_ep_data = {
    {
...
static __constant int __scos_ep___ip_h = 0x0517CC1B;
static __constant int __scos_ep___ip_m = 0x727220A9;
static __constant int __scos_ep___ip_l = 0x28;
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c5 = {0xbbe61a2du};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c5l = {0xaeba0fbau};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c4 = {0x3da807fcu};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c4l = {0xaf614e97u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c3 = {0xbf196889u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c3l = {0xaf584810u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c2 = {0x402335e0u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c2l = {0xb2f6c1feu};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c1 = {0xc0a55de6u};
static __constant union {
  unsigned int w;
  float f;
  int i;
} __scos_ep___c0 = {0x40490fdau};
static __constant unsigned int __scos_ep_invpi_tbl[] = {
    0,          0x28BE60DB, 0x9391054A, 0x7F09D5F4,
    0x7D4D3770, 0x36D8A566, 0x4F10E410, 0x7F9458EA};
__attribute__((always_inline)) inline int
__ocl_svml_internal_scos_ep(float *a, float *pres) {
  int nRet = 0;
  float xin = *a;
  ...
  if (__builtin_expect(((unsigned)(ex - 0x7f + 12)) > (20 + 12), (0 == 1))) {
    ...
    ip_low = (((unsigned int)__scos_ep_invpi_tbl[index]) * ((unsigned int)mx));
    IP = (((unsigned long)((unsigned int)(__scos_ep_invpi_tbl[index + 1]))) *
          ((unsigned int)(mx))) +
         (((unsigned long)ip_low) << 32);
    // scaled by 2^(95-j)
    IP2 = (((unsigned long)((unsigned int)(__scos_ep_invpi_tbl[index + 2]))) *
           ((unsigned int)(mx))) +
          ((((unsigned long)((unsigned int)(__scos_ep_invpi_tbl[index + 3]))) *
            ((unsigned int)(mx))) >>
           32);
    IP = IP + (IP2 >> 32);
    // scale 2^63
  ...
  } else // main path
  {
    ...
    IP_s = (((long)((int)(mx))) * ((int)(__scos_ep___ip_h)));
    IP = (unsigned long)IP_s;
    IP2_s = (((long)((int)(mx))) * ((int)(__scos_ep___ip_m)));
    IP2 = (unsigned long)IP2_s;
    // scale (23-ex)*2^(28+32+7)
    ip_low_s = (((int)mx) * ((int)__scos_ep___ip_l));
  ...
  poly_h = __scos_ep___c5.f;
  poly_h =
      __spirv_ocl_fma(poly_h, R2h, __scos_ep___c4.f);
  poly_h =
      __spirv_ocl_fma(poly_h, R2h, __scos_ep___c3.f);
  poly_h =
      __spirv_ocl_fma(poly_h, R2h, __scos_ep___c2.f);
  poly_h =
      __spirv_ocl_fma(poly_h, R2h, __scos_ep___c1.f);
  poly_h =
      __spirv_ocl_fma(poly_h, R2h, __scos_ep___c0.f);
  res.f = __spirv_ocl_fma(poly_h, Rh.f, 0.0f);
  res.w ^= sgn_x;
  *pres = res.f;
  return nRet;
}
float __ocl_svml_cosf_ep(float x) {
  float r;
  unsigned int vm;
  float va1;
  float vr1;
  va1 = x;
  {
    ...
    float sAbsMask;
    vm = 0;
    sHalfPI = as_float(__ocl_svml_internal_scos_ep_data._sHalfPI);
    sAbsMask = as_float(__ocl_svml_internal_scos_ep_data._sAbsMask);
    ...
    sRangeReductionVal =
        as_float(__ocl_svml_internal_scos_ep_data._sRangeReductionVal);
    sRangeReductionMask =
        as_float(((unsigned int)(-(signed int)(!(sAX <= sRangeReductionVal)))));
    {
      float sN;
      float sSign;
      float sR;
      ...
      sInvPI = as_float(__ocl_svml_internal_scos_ep_data._sInvPI);
      sRShifter = as_float(__ocl_svml_internal_scos_ep_data._sRShifter);
      // 1) Range reduction to [-Pi/2; +Pi/2] interval
      //    c) Getting octant Y by 1/Pi multiplication
      //    d) Add "Right Shifter" (0x4B000000) value
      sN = __spirv_ocl_fma(sX, sInvPI, sRShifter);
      //    e) Treat obtained value as integer for destination sign setting.
      ...
      sN = (sN - sRShifter);
      sOneHalf = as_float(__ocl_svml_internal_scos_ep_data._sOneHalf);
      //    g) Subtract 0.5 from result for octant correction
      sN = (sN - sOneHalf);
      sPI1 = as_float(__ocl_svml_internal_scos_ep_data._sPI1_FMA);
      //    h) Subtract Y*PI from X argument, where PI divided to 4 parts:
      //       X = X - Y*PI1 - Y*PI2 - Y*PI3
      sR = __spirv_ocl_fma(-(sN), sPI1, sAX);
      sPI2 = as_float(__ocl_svml_internal_scos_ep_data._sPI2_FMA);
      ...
      sA1 = as_float(__ocl_svml_internal_scos_ep_data._sA1);
      //    b) Calculate polynomial:
      //       R = X + X * X^2 * (A3 + x^2 * (A5));
      sSRA1 = (sR * sA1);
      sA3 = as_float(__ocl_svml_internal_scos_ep_data._sA3);
      sA5 = as_float(__ocl_svml_internal_scos_ep_data._sA5);
      sP = __spirv_ocl_fma(sA5, sR2, sA3);
      sR3 = (sR * sR2);
      vr1 = __spirv_ocl_fma(sP, sR3, sSRA1);
      // result=1.0 if arg. near 0
      sOne = as_float(__ocl_svml_internal_scos_ep_data._sOne);
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_scos_ep(&__cout_a1, &__cout_r1);
