...
} __ocl_svml_internal_slog1p_ha_data_t;
static __ocl_svml_internal_slog1p_ha_data_t __ocl_svml_internal_slog1p_ha_data =
    {
        /* Log_HA_table */
...
static __constant _iml_v2_sp_union_t __slog1p_ha_CoutTab[210] = {
...
__ocl_svml_internal_slog1p_ha(float *a, float *r) {
  float fap1, *ap1 = (&fap1);
  float x, y, u;
  float fP;
  float fAbsU;
  ...
  if ((((((_iml_v2_sp_union_t *)&ap1[0])->hex[0] >> 23) & 0xFF) != 0xFF)) {
    ...
    */
    ...
    if (((((_iml_v2_sp_union_t *)&x)->hex[0] >> 23) & 0xFF) == 0) {
      ...
      x *= ((__constant float *)__slog1p_ha_CoutTab)[200];
      iN -= 40;
    }
    /* Starting from this point x is finite normalized number */
    if (x > ((__constant float *)__slog1p_ha_CoutTab)[201]) {
      ...
      if (fAbsU > ((__constant float *)__slog1p_ha_CoutTab)[199]) {
        /* 6) "Main" path */
        /* ap1) Range reduction */
        ...
        */
        if (isDenorm == 1) {
          fNLn2Hi = (fN * (((__constant float *)__slog1p_ha_CoutTab)[195] +
                           ((__constant float *)__slog1p_ha_CoutTab)[196]));
          fNLn2Lo = 0.0f;
        } else {
          fNLn2Hi = (fN * ((__constant float *)__slog1p_ha_CoutTab)[195]);
          fNLn2Lo = (fN * ((__constant float *)__slog1p_ha_CoutTab)[196]);
        }
        /* Get y */
        y = x;
        ...
        /* Obtain j */
        fTmp = (y + ((__constant float *)__slog1p_ha_CoutTab)[197]);
        j = (((_iml_v2_sp_union_t *)&fTmp)->hex[0] & 0x007FFFFF) &
            ((1 << (6 + 1)) - 1);
        /* Get table values of RcprY, LnRcprYHi, LnRcprYLo */
        fRcprY = ((__constant float *)__slog1p_ha_CoutTab)[3 * (j)];
        fLnRcprYHi = ((__constant float *)__slog1p_ha_CoutTab)[3 * (j) + 1];
        fLnRcprYLo = ((__constant float *)__slog1p_ha_CoutTab)[3 * (j) + 2];
        ...
        ;
        /* Get YHi and YLo */
        fTmp = (y + ((__constant float *)__slog1p_ha_CoutTab)[198]);
        fYHi = (fTmp - ((__constant float *)__slog1p_ha_CoutTab)[198]);
        fYLo = (y - fYHi);
        /* Get UHi, uLo and U */
        ...
        u = (fUHi + fuLo);
        /* b) Approximation */
        fP = ((((((((__constant float *)__slog1p_ha_CoutTab)[209] * u +
                   ((__constant float *)__slog1p_ha_CoutTab)[208]) *
                      u +
                  ((__constant float *)__slog1p_ha_CoutTab)[207]) *
                     u +
                 ((__constant float *)__slog1p_ha_CoutTab)[206]) *
                    u +
                ((__constant float *)__slog1p_ha_CoutTab)[205]) *
                   u +
               ((__constant float *)__slog1p_ha_CoutTab)[204]) *
                  u +
              ((__constant float *)__slog1p_ha_CoutTab)[203]);
        fP = (fP * u * u);
        /* c) Reconstruction */
        fResHi = (fWHi + fUHi);
      ...
      if (x == ((__constant float *)__slog1p_ha_CoutTab)[201]) {
        ...
        r[0] = -((__constant float *)__slog1p_ha_CoutTab)[202] /
               ((__constant float *)__slog1p_ha_CoutTab)[201];
        nRet = 2;
      } else {
        /* Path 4). Here if argument is negative number */
        r[0] = ((__constant float *)__slog1p_ha_CoutTab)[201] /
               ((__constant float *)__slog1p_ha_CoutTab)[201];
        nRet = 1;
      }
    }
...
float __ocl_svml_log1pf_ha(float x) {
  float r;
  unsigned int vm;
  float va1;
  float vr1;
  va1 = x;
  {
    ...
    float B;
    float dR2;
    One = as_float(__ocl_svml_internal_slog1p_ha_data.sOne);
    SgnMask = as_float(__ocl_svml_internal_slog1p_ha_data.SgnMask);
    XThreshold = as_float(__ocl_svml_internal_slog1p_ha_data.XThreshold);
    XhMask = as_float(__ocl_svml_internal_slog1p_ha_data.XhMask);
    ...
    Xl = (A - X);
    Xl = (Xl + B);
    ExpMask = as_float(__ocl_svml_internal_slog1p_ha_data.ExpMask);
    Two10 = as_float(__ocl_svml_internal_slog1p_ha_data.Two10);
    ...
    MinLog1p = as_float(__ocl_svml_internal_slog1p_ha_data.MinLog1p);
    MaxLog1p = as_float(__ocl_svml_internal_slog1p_ha_data.MaxLog1p);
    ExpMask0 = (__ocl_svml_internal_slog1p_ha_data.ExpMask0);
    ExpMask2 = (__ocl_svml_internal_slog1p_ha_data.ExpMask2);
    ...
    THL[0] =
        as_float(((unsigned int *)((char *)(&__ocl_svml_internal_slog1p_ha_data
                                                 .Log_HA_table[0]) -
                                   0x21800))[Index >> 2]);
    THL[1] =
        as_float(((unsigned int *)((char *)(&__ocl_svml_internal_slog1p_ha_data
    ...
    poly_coeff[1] =
        as_float(__ocl_svml_internal_slog1p_ha_data.ha_poly_coeff[0]);
    poly_coeff[0] =
  ...
  if (__builtin_expect((vm) != 0, 0)) {
    ...
    __ocl_svml_internal_slog1p_ha(&__cout_a1, &__cout_r1);
