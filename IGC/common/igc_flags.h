...
DECLARE_IGC_REGKEY(bool, VectorizerAllowWAVEBROADCAST, true, "Allow WAVEBROADCAST instructions inside vectorizer", true)
...
DECLARE_IGC_REGKEY(bool, EnableBdpasScaleCoalescing, true,
...
DECLARE_IGC_REGKEY(bool, RegenerateTileID, true, "Recalculate TileID from sr0.1 on each use", false)
DECLARE_IGC_REGKEY(bool, RegenerateEngineID, false, "Recalculate EngineID from sr0.1 on each use", false)
...
DECLARE_IGC_REGKEY(bool, PreferSIMD32ForCompute, true, "Prefer SIMD32 for compute kernels", true)
DECLARE_IGC_REGKEY(
    DWORD, PreferSIMD32ForComputeSubset, 0x28,
    "Prefer SIMD32 for a subset of compute kernels using bit mask, impacting also PS shaders with Ray Query", true)
DECLARE_IGC_REGKEY(DWORD, ForceGeomFFSIMDWidth, 32,
                   "SIMD mode for VS, GS, DS, HS; valid values: 0 = default, 8 = SIMD8, 16 = SIMD16, 32 = SIMD32", true)
...
DECLARE_IGC_REGKEY(DWORD, ForceGRFModeUp, 0,
                   "Set the GRF mode # higher than the one selected by VRT default. Pass it to vISA", true)
...
DECLARE_IGC_REGKEY(DWORD, OCLVRTSimd16DropSimd16Low, 120, "...and SIMD16 RPE is below this value", false)
