...
DECLARE_IGC_REGKEY(bool, LoopSinkCoarserLoadsRescheduling, true, "Try to reschedule multi-instruction load candidates in larger chunks. Disabling is experimental and can produce functionally incorrect code.", false)
...
DECLARE_IGC_GROUP("Raytracing Options")
    ...
    DECLARE_IGC_REGKEY(bool, SyncDispatchRaysDisableShaderInlining, false, "Will skip direct calls in synchronous raytracing and immediately call raytracing shaders via KSP shader ptr", false)
    DECLARE_IGC_REGKEY(bool, SyncDispatchRaysDisableDeferredInlining, false, "Will inline calls to __TraceRay, __Invoke and __TraceRaySyncToAsyncAdapter even when indirect calls are not necessary", false)
    DECLARE_IGC_REGKEY_ENUM(SyncDispatchRaysDispatchMethod, 0, "Selects how sync dispatch rays handles indirect calls", SYNC_DISPATCH_RAYS_SHADER_INVOKE_METHODS, false)
