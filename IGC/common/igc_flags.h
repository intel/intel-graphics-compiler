...
DECLARE_IGC_REGKEY(bool, EnableLoadsLoopSink,           false, "Allow sinking of loads in the loop", false)
DECLARE_IGC_REGKEY(DWORD, CodeSinkingMinSize,           32,  "Don't sink if the number of instructions in the kernel is less", false)
DECLARE_IGC_REGKEY(DWORD, CodeSinkingLoadSchedulingInstr, 5,  "Instructions number to step to schedule loads in advance before the load use to cover latency. 1 to insert it immediately before use", false)
...
DECLARE_IGC_REGKEY(DWORD, EnableNewTileYCheck,          1,     "Enable new TileY check. 0 - off, 1 - on, 2 - platform default", false)
