...
class CompilerStats {
  // Internal class for storing data. Every statistic knows its type and keeps
  // values for each simd size (potenatially wastes memory but improves
  // execution time).
  struct Statistic {
    // Supported data types:
    enum Type { type_bool, type_int64, type_double };
    ...
    // Helper function to convert from SimdType enum to integer represenation
    // (8/16/32). Returns 0 for generic simd.
    ...
    // Get stat value for simd size expressed as integer. Pass 0 to access
    // generic.
  ...
  inline Statistic &PrivateInit(const std::string &name, Statistic::Type type,
                                int simd);
...
// collectOnlyInitialized - if true, statistic initialization is required,
// otherwise statistic update will be ignored
...
// Create statistic with value set to zero/false.
void CompilerStats::Init(const std::string &name, Statistic::Type type,
                         int simd) {
  if (IsEnabled()) {
    auto f = m_pState->m_Stats.find(name);
    if (f == m_pState->m_Stats.end()) {
...
// Return true if the statistic name exists.
bool CompilerStats::Find(const std::string &name) { return PrivateFind(name); }
...
// Increase value of integer statistic.
// If statistic does not exist, create new one.
void CompilerStats::IncreaseI64(const std::string &name, int64_t value,
                                int simd) {
  if (IsEnabled()) {
    auto f = m_pState->m_Stats.find(name);
    if (f == m_pState->m_Stats.end()) {
...
// Increase value of floating point statistic.
// If statistic does not exist, create new one.
void CompilerStats::IncreaseF64(const std::string &name, double value,
                                int simd) {
  if (IsEnabled()) {
    auto f = m_pState->m_Stats.find(name);
    if (f == m_pState->m_Stats.end()) {
...
std::string CompilerStats::ToCsv() const {
  ...
  std::map<std::string, Statistic> ordered(m_pState->m_Stats.begin(),
                                           m_pState->m_Stats.end());
  for (auto elem : ordered) {
    ss << elem.first << ",";
    for (int simd_type = 0; simd_type < Statistic::SimdType::NUM_SIMD_TYPES;
         simd_type++) {
...
// Create empty statistic if does not exist, return existing if it does.
CompilerStats::Statistic &CompilerStats::PrivateInit(const std::string &name,
                                                     Statistic::Type type,
                                                     int simd) {
  IGC_UNUSED(simd);
  Statistic s;
...
// Try to find statistic. Return empty one if it does not exist.
const CompilerStats::Statistic &
CompilerStats::PrivateGet(const std::string &name, int simd) const {
  IGC_UNUSED(simd);
  auto f = m_pState->m_Stats.find(name);
  if (f != m_pState->m_Stats.end()) {
    return f->second;
...
// Helper function to convert from SimdType enum to integer represenation
// (8/16/32). Returns 0 for generic simd.
int CompilerStats::Statistic::type_to_simd(
    CompilerStats::Statistic::SimdType simd_type) {
  switch (simd_type) {
  case SIMD_8:
    return 8;
  case SIMD_16:
...
// Helper function to convert from integer represenation (8/16/32) to SimdType
// enum. Returns SIMD_GENERIC for unexpected values.
CompilerStats::Statistic::SimdType
CompilerStats::Statistic::simd_to_type(int simd) {
  switch (simd) {
  case 8:
    return SIMD_8;
  case 16:
...
CompilerStats::Statistic::Value &
CompilerStats::Statistic::operator[](int simd) {
  const SimdType simd_type = simd_to_type(simd);
  return values[simd_type];
}

// Get stat value for simd size expressed as integer. Pass 0 to access generic.
const CompilerStats::Statistic::Value &
CompilerStats::Statistic::operator[](int simd) const {
  const SimdType simd_type = simd_to_type(simd);
  return values[simd_type];
}
