...
// otherwise lift it to 512 GRF on retry. With -cl-intel-256-GRF-per-thread set,
// the explicit pin blocks the gate, so both the first attempt and the retry
// must stay at 256 GRF.

// RUN: ocloc compile -file %s -device cri \
// RUN: -options "-cl-intel-256-GRF-per-thread -igc_opts 'EnableOCL512GRFForSIMD16=1,DumpASMToConsole=1'" \
// RUN: 2>&1 | FileCheck %s --check-prefix=COMPILE --implicit-check-not "numGRF=512"

// The kernel spills at 256, so a recompilation fires. On this pinned path
// DumpASMToConsole emits a single asm dump; the explicit pin keeps it at 256 GRF,
// and the --implicit-check-not "numGRF=512" above ensures no asm dump in this
// output escalates to 512.
// COMPILE: numGRF=256
