...
// when run on functions in same group.

// RUN: rm -rf %t && mkdir %t
// RUN: ocloc compile -file %s -options "-igc_opts 'DumpRegPressureEstimate=1, DumpRegPressureEstimateFilter=no_inline_func, DumpToCustomDir=%t, EnableStackCallFuncCall=1'" -device pvc
// RUN: cat %t/*_no_inline_func_final_RegEst.ll | FileCheck %s --check-prefix=CHECK

// CHECK: SIMD: 32, external pressure: 0
