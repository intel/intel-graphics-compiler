...
// REQUIRES: typed-pointers, dg2-supported
// UNSUPPORTED: sys32
...
__kernel void foo() {
  // CHECK-LLVM: [[PRV_PTR:%.*]] = load {{i8|i32}}*, {{i8|i32}}** %prv_ptr
  // CHECK-LLVM: [[CAST:%.*]] = addrspacecast {{i8|i32}}* [[PRV_PTR]] to {{i8|i32}} addrspace(4)*
  // CHECK-LLVM: [[PRED:%.*]] = icmp ne {{i8|i32}}* [[PRV_PTR]], null
  // CHECK-LLVM: [[SEL:%.*]] = select i1 [[PRED]], {{i8|i32}} addrspace(4)* [[CAST]], {{i8|i32}} addrspace(4)* null
  int* a = NULL;
