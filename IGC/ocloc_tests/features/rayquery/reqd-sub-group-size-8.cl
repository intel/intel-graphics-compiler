...
// RUN: not ocloc compile -file %s -options "-cl-std=CL3.0" -internal_options "-cl-ext=-all,+cl_intel_rt_production" -device nvl-p-b0 2>&1 | FileCheck %s --check-prefix CHECK-NVLP
// CHECK-NVLP: error: Kernel compiled with required subgroup size 8, which is unsupported on this platform

// RUN: not ocloc compile -file %s -options "-cl-std=CL3.0" -internal_options "-cl-ext=-all,+cl_intel_rt_production" -device ptl-h 2>&1 | FileCheck %s --check-prefix CHECK-PTLH
