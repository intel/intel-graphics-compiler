...
; A packed FP4 intrinsic has an explicit A byte offset. An out-of-range value
; must assert before it is narrowed to uint16_t.
...
define spir_kernel void @invalid_packed_a(ptr addrspace(1) %a_ptr, ptr addrspace(1) %b_ptr, ptr addrspace(1) %sa_ptr, ptr addrspace(1) %sb_ptr, ptr addrspace(1) %out) #0 !intel_reqd_sub_group_size !0 {
