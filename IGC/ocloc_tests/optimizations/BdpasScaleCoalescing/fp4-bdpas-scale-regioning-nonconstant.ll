...
; Packed scale offsets are part of the compile-time intrinsic contract. A
; kernel argument in an offset position must terminate compilation.
...
define spir_kernel void @nonconstant_packed_offsets(ptr addrspace(1) %a_ptr, ptr addrspace(1) %b_ptr,
                                                    ptr addrspace(1) %sa_ptr, ptr addrspace(1) %sb_ptr,
                                                    i32 %a_offset, ptr addrspace(1) %out) #0
                                                    !intel_reqd_sub_group_size !0 {
  ...
  %r = call <8 x float> @llvm.genx.GenISA.sub.group.bdpas.packed.v8f32.v8f32.v8i16.v8i32.v4i8.v4i8(
      <8 x float> zeroinitializer, <8 x i16> %a, <8 x i32> %b, <4 x i8> %sa, <4 x i8> %sb,
      i32 13, i32 13,
      i32 %a_offset, i32 0)
