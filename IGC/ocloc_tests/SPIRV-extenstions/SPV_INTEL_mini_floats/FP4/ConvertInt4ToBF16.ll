...
; RUN: llvm-spirv %t.bc -o %t.spv --spirv-ext=+SPV_INTEL_int4,+SPV_KHR_bfloat16
...
define spir_kernel void @Int4_to_BF16_scalar(<2 x i4> addrspace(1)* %input, bfloat addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_scalar"
; CHECK: mov ({{.*}}, 1) [[LUT:[A-z0-9.]*]](0,0)<1> 0x3f803f8000000000:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,1)<1> 0x4040404040004000:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,2)<1> 0x40a040a040804080:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,3)<1> 0x40e040e040c040c0:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,4)<1> 0xc0e0c0e0c100c100:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,5)<1> 0xc0a0c0a0c0c0c0c0:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,6)<1> 0xc040c040c080c080:q
; CHECK: mov ({{.*}}, 1) [[LUT]](0,7)<1> 0xbf80bf80c000c000:q
...
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %3 = call bfloat @_Z38__builtin_spirv_ConvertInt4ToBF16INTELi(i4 %2)
  store bfloat %3, bfloat addrspace(1)* %outputAddr, align 2
  ret void
}
declare dso_local spir_func bfloat @_Z38__builtin_spirv_ConvertInt4ToBF16INTELi(i4)

define spir_kernel void @Int4_to_BF16_vector2(<2 x i4> addrspace(1)* %input, <2 x bfloat> addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_vector2"
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST:[A-z0-9]*]].0 [[LUT_SRC:[A-z0-9]*]](0,0)<0;16,1> [[INPUT:[A-z0-9]*]](0,0)<4;1,0>
; CHECK-NOT: shfl_idx4
; CHECK-DAG: // .decl [[DST]] v_type=G type=ud num_elts=32
; CHECK-DAG: // .decl [[LUT_SRC]] v_type=G type=ud num_elts=16
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %1 = load <2 x i4>, <2 x i4> addrspace(1)* %inputAddr, align 1
  %2 = call <2 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv2_i(<2 x i4> %1)
  store <2 x bfloat> %2, <2 x bfloat> addrspace(1)* %outputAddr, align 4
  ret void
}
declare dso_local spir_func <2 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv2_i(<2 x i4>)

define spir_kernel void @Int4_to_BF16_vector3(<4 x i4> addrspace(1)* %input, <3 x bfloat> addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_vector3"
...
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %3 = call <3 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv3_i(<3 x i4> %2)
  store <3 x bfloat> %3, <3 x bfloat> addrspace(1)* %outputAddr, align 8
  ret void
}
declare dso_local spir_func <3 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv3_i(<3 x i4>)

define spir_kernel void @Int4_to_BF16_vector4(<4 x i4> addrspace(1)* %input, <4 x bfloat> addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_vector4"
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST0:[A-z0-9]*]].0 [[LUT_SRC:[A-z0-9]*]](0,0)<0;16,1> [[INPUT:[A-z0-9]*]](0,0)<4;1,0>
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST1:[A-z0-9]*]].0 [[LUT_SRC]](0,0)<0;16,1> [[INPUT]](0,0)<4;1,0>
; CHECK-NOT: shfl_idx4
; CHECK-DAG: // .decl [[DST0]] v_type=G type=ud num_elts=32
...
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %1 = load <4 x i4>, <4 x i4> addrspace(1)* %inputAddr, align 2
  %2 = call <4 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv4_i(<4 x i4> %1)
  store <4 x bfloat> %2, <4 x bfloat> addrspace(1)* %outputAddr, align 8
  ret void
}
declare dso_local spir_func <4 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv4_i(<4 x i4>)

define spir_kernel void @Int4_to_BF16_vector8(<8 x i4> addrspace(1)* %input, <8 x bfloat> addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_vector8"
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST0:[A-z0-9]*]].0 [[LUT_SRC:[A-z0-9]*]](0,0)<0;16,1> [[INPUT:[A-z0-9]*]](0,0)<4;1,0>
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST1:[A-z0-9]*]].0 [[LUT_SRC]](0,0)<0;16,1> [[INPUT]](0,0)<4;1,0>
...
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %2 = call <8 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv8_i(<8 x i4> %1)
  store <8 x bfloat> %2, <8 x bfloat> addrspace(1)* %outputAddr, align 16
  ret void
}
declare dso_local spir_func <8 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv8_i(<8 x i4>)

define spir_kernel void @Int4_to_BF16_vector16(<16 x i4> addrspace(1)* %input, <16 x bfloat> addrspace(1)* %output) {
; CHECK-LABEL: .kernel "Int4_to_BF16_vector16"
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST0:[A-z0-9]*]].0 [[LUT_SRC:[A-z0-9]*]](0,0)<0;16,1> [[INPUT:[A-z0-9]*]](0,0)<4;1,0>
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST1:[A-z0-9]*]].0 [[LUT_SRC]](0,0)<0;16,1> [[INPUT]](0,0)<4;1,0>
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST2:[A-z0-9]*]].0 [[LUT_SRC]](0,0)<0;16,1> [[INPUT]](0,0)<4;1,0>
; CHECK: shfl_idx4 ({{.*}}, 32) [[DST3:[A-z0-9]*]].0 [[LUT_SRC]](0,0)<0;16,1> [[INPUT]](0,0)<4;1,0>
...
; CHECK-DAG: // .decl [[INPUT]] v_type=G type=ub num_elts=128
  ...
  %2 = call <16 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv16_i(<16 x i4> %1)
  store <16 x bfloat> %2, <16 x bfloat> addrspace(1)* %outputAddr, align 32
  ret void
}
declare dso_local spir_func <16 x bfloat> @_Z38__builtin_spirv_ConvertInt4ToBF16INTELDv16_i(<16 x i4>)
