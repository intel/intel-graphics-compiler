...
#define DECLARE_IGC_REGKEY(dataType, regkeyName, defaultValue, description, flagAvailability) OPTION_##regkeyName,
...
template <typename TEnum> TEnum enumFrom(EnumStr valueStr) {
  for (int i = static_cast<int>(TEnum::BEGIN); i < static_cast<int>(TEnum::END); ++i) {
...
template <typename TEnum> TEnum enumFrom(const char *str) { return TEnum::END; }
...
inline bool GetDebugFlag(DebugFlag flag) {
  (void)flag;
  return false;
}
...
inline void IGC_DEBUG_API_CALL SetShaderCorpusName(CorpusName name) { (void)name; }
