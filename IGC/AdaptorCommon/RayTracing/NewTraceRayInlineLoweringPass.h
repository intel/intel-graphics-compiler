...
private:
  ...
  UnpackedData getPackedData(llvm::RTBuilder &IRB, llvm::Value *rqObject) {
    ...
    auto iterator = {
        ...
        std::make_tuple(&data.CommittedStatus, RQPackedDataBitOffsets::TraceRayCtrl,
                        RQPackedDataBitOffsets::CommittedStatus),
        std::make_tuple(&data.CandidateType, RQPackedDataBitOffsets::CommittedStatus,
                        RQPackedDataBitOffsets::CandidateType),
        std::make_tuple(&data.HasAcceptHitAndEndSearchFlag, RQPackedDataBitOffsets::CandidateType,
    ...
    for (auto [field, bitoffset, nextbitoffset] : iterator) {
      *field =
          IRB.CreateAnd({IRB.CreateLShr(packedData, bitoffset), IRB.getInt32((1 << (nextbitoffset - bitoffset)) - 1)});
    }
  ...
  void setPackedData(llvm::RTBuilder &IRB, llvm::Value *rqObject, const UnpackedData &data) {
    ...
    for (auto [field, bitoffset, nextbitoffset] : iterator) {
      packedData = IRB.CreateOr(
          packedData, IRB.CreateShl(IRB.CreateAnd(*field, (1 << (nextbitoffset - bitoffset)) - 1), bitoffset));
    }
  ...
  void StopAndStartRayquery(llvm::RTBuilder &IRB, llvm::Instruction *I, llvm::Value *rqObject, bool doSpillFill,
