...
namespace IGC
{
    ...
    private:
        ...
        void AssignGlobalBuffers(llvm::Function& F, const LivenessDataMap& livenessDataMap);
        void HandleOptimizationsAndSpills(llvm::Function& F, LivenessDataMap& livenessDataMap, llvm::DominatorTree& DT, llvm::LoopInfo& LI);
        void LowerGlobalBufferPtrs(llvm::Function& F);
        void LowerStackPtrs(llvm::Function& F);

        enum RQTraceRayCtrl : uint8_t
        {
            TRACE_RAY_INITIAL = static_cast<uint8_t>(RTStackFormat::TraceRayCtrl::TRACE_RAY_INITIAL),
            TRACE_RAY_INSTANCE = static_cast<uint8_t>(RTStackFormat::TraceRayCtrl::TRACE_RAY_INSTANCE),
            TRACE_RAY_COMMIT = static_cast<uint8_t>(RTStackFormat::TraceRayCtrl::TRACE_RAY_COMMIT),
            TRACE_RAY_CONTINUE = static_cast<uint8_t>(RTStackFormat::TraceRayCtrl::TRACE_RAY_CONTINUE),
        ...
        llvm::Value* getAtIndexFromRayQueryObject(llvm::RTBuilder& IRB, llvm::Value* rqObject, uint32_t i)
        {
            return IRB.CreateInBoundsGEP(m_RQObjectType, rqObject, { IRB.getInt32(0), IRB.getInt32(i) });
        }

        llvm::Value* getGlobalBufferPtr(llvm::RTBuilder& IRB, llvm::Value* rqObject)
        {
            return IRB.CreateLoad(m_Functions[GET_GLOBAL_BUFFER_PTR]->getReturnType(), getAtIndexFromRayQueryObject(IRB, rqObject, 0));
        }
        ...
        UnpackedData getPackedData(llvm::RTBuilder& IRB, llvm::Value* rqObject)
        {
            ...
            auto* packedData = IRB.CreateLoad(IRB.getInt32Ty(), getAtIndexFromRayQueryObject(IRB, rqObject, 1));
        ...
        void setPackedData(llvm::RTBuilder& IRB, llvm::Value* rqObject, const UnpackedData& data)
        {
            auto iterator = {
                std::make_tuple(&data.TraceRayCtrl, RQPackedDataBitOffsets::Start, RQPackedDataBitOffsets::TraceRayCtrl),
                std::make_tuple(&data.CommittedStatus, RQPackedDataBitOffsets::TraceRayCtrl, RQPackedDataBitOffsets::CommittedStatus),
        ...
        {
            return IRB.CreateLoad(IRB.getInt32Ty(), getAtIndexFromRayQueryObject(IRB, rqObject, 2));
        }
        ...
        llvm::RTBuilder::SyncStackPointerVal* getStackPtr(llvm::RTBuilder& IRB, llvm::Value* rqObject)
        {
            return static_cast<llvm::RTBuilder::SyncStackPointerVal*>(
                llvm::cast<llvm::Value>(IRB.CreateCall(m_Functions[GET_STACK_POINTER_FROM_GLOBAL_BUFFER_POINTER], getGlobalBufferPtr(IRB, rqObject)))
            );
        }

        void EmitPreTraceRayFence(llvm::RTBuilder& IRB, llvm::Value* rqObject);
        void InsertCacheControl(llvm::RTBuilder& IRB, llvm::RTBuilder::SyncStackPointerVal* stackPtr);
        void StopAndStartRayquery(llvm::RTBuilder& IRB, llvm::Instruction* I, llvm::Instruction* globalBufferPtr, bool doSpillFill, bool doRQCheckRelease);
