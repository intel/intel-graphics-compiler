...
bool InlineRaytracing::LowerAllocations(Function &F) {
  ...
  for (auto *I : AllocateRQInstructions) {
    ...
    setPackedData(IRB, rqObject,
                  {IRB.getInt32(TRACE_RAY_DONE), IRB.getInt32(0), IRB.getInt32(0), IRB.getInt32(0),
                   IRB.getInt32(CommittedHit), IRB.getInt32(0)});
...
void InlineRaytracing::LowerIntrinsics(Function &F) {
  ...
  for (auto RQI : RQInstructions) {
    ...
    case GenISAIntrinsic::GenISA_TraceRayInlineHL: {
      ...
      data.PendingStackWrite = IRB.getInt32(1);
    ...
    case GenISAIntrinsic::GenISA_TraceRaySyncProceedHL: {
      ...
      if (rqReturnOpt) {
        ...
        // TraceRay: PendingStackWrite is set at every site that writes the
        // HW/sync stack (createTraceRayInlinePrologue, resetNeedsStocRQ
        // above, CommitProceduralPrimitiveHit's setHitT) and cleared once the
        // fence below has run. Every other edge into this loop is a SW-STOC
        // back-edge (continueBlock/commitAndContinueBlock), which only
        // touches the private RQObject state, never the sync stack.
        Value *storesPending = IRB.CreateICmpNE(data.PendingStackWrite, IRB.getInt32(0));
      ...
      data.CommittedDataLocation = IRB.getInt32(CommittedHit);
      data.PendingStackWrite = IRB.getInt32(0);
