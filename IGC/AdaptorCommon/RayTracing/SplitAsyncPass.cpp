...
private:
    ...
    bool materializable(Instruction& I) const;

    void remat(Function& F, SuspendCrossingInfo& Checker) const;

    Optional<std::vector<Instruction*>> canFullyRemat(Instruction* I) const;
    bool canFullyRemat(
        Instruction* I,
    ...
    bool isReadOnly(const Value* Ptr) const;
...
bool SplitAsyncPass::isReadOnly(const Value* Ptr) const
{
...
bool SplitAsyncPass::canFullyRemat(
    Instruction* I,
    std::vector<Instruction*>& Insts,
    std::unordered_set<Instruction*>& Visited,
    unsigned Depth) const
{
    ...
    for (auto& Op : I->operands())
    {
        ...
        if (auto* A = dyn_cast<Argument>(Op))
        {
            ...
            if (ArgQuery{ F, *m_CGCtx }.getPayloadArg(&F) == A)
                continue;
        }
        ...
        if (!canFullyRemat(OpI, Insts, Visited, Depth - 1))
            return false;
    }
...
SplitAsyncPass::canFullyRemat(Instruction* I) const
{
    std::vector<Instruction*> Insts;
    std::unordered_set<Instruction*> Visited;
    const int Threshold = m_CGCtx->opts().RematThreshold;
    if (!canFullyRemat(I, Insts, Visited, Threshold))
        return None;
...
bool SplitAsyncPass::materializable(Instruction &I) const
{
