...
inline bool isPromotableStructType(const Module& M, const Argument* arg, bool isStackCall)
{
    if (arg->getType()->isPointerTy())
    {
        if (arg->hasStructRetAttr())
        {
            return isPromotableStructType(M, arg->getParamStructRetType(), isStackCall);
        }
        else if (arg->hasByValAttr())
        {
            return isPromotableStructType(M, arg->getParamByValType(), isStackCall);
        }
    }
    return false;
}
...
inline bool FunctionHasPromotableSRetArg(const Module& M, const Function* F)
{
    if (F->getReturnType()->isVoidTy() &&
        !F->arg_empty() &&
        isPromotableStructType(M, F->arg_begin(), F->hasFnAttribute("visaStackCall")))
    {
        return true;
    }
    return false;
...
void LegalizeFunctionSignatures::FixCallInstruction(Module& M, CallInst* callInst)
{
    ...
    if (callInst->getType()->isVoidTy() &&
        ...
        isPromotableStructType(M, callInst->getCalledFunction()->getArg(0), isStackCall))
    {
