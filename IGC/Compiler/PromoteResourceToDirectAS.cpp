...
void PromoteResourceToDirectAS::PromoteSamplerTextureToDirectAS(GenIntrinsicInst *&pIntr, Value *resourcePtr) {
  ...
  if (canPromote) {
    ...
    PointerType *newptrType = IGCLLVM::PointerType::get(dyn_cast<PointerType>(resourcePtr->getType()), addrSpace);
...
bool PatchGetElementPtr(const std::vector<Value *> &instList, Type *dstTy, unsigned directAS, Value *patchedSourcePtr,
  ...
  if (!patchedInst) {
    ...
    PointerType *newptrType = IGCLLVM::PointerType::get(patchTy, directAS);
...
void PromoteResourceToDirectAS::PromoteBufferToDirectAS(Instruction *inst, Value *resourcePtr) {
  ...
  if (LoadInst *load = dyn_cast<LoadInst>(inst)) {
    ...
    case GenISAIntrinsic::GenISA_ldrawvector_indexed: {
      ...
      PointerType *ptrType = IGCLLVM::PointerType::get(pBufferType, directAS);
    ...
    default: {
      ...
      PointerType *newptrType = IGCLLVM::PointerType::get(pBufferType, directAS);
