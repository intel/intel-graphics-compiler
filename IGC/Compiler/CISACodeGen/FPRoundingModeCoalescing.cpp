...
class FPRoundingModeGroup {
  public:
    ...
    bool contains(Instruction *I) const { return Instructions.count(I); }

    void setHead(Instruction *I);
    void insertAfterGroup(Instruction *I);
    bool comesAfterGroup(Instruction *I);
...
void FPRoundingModeGroup::insertAfterGroup(Instruction *I) {
    LLVM_DEBUG(dbgs() << "Moving ["; I->print(dbgs(), true); dbgs() << "] after ["; Head->print(dbgs(), true); dbgs() << "]\n");
    I->moveAfter(Tail);
}

// Returns true if given instruction comes after group.
bool FPRoundingModeGroup::comesAfterGroup(Instruction *I) { return IGCLLVM::comesBefore(I, Tail); }
...
bool FPRoundingModeCoalescingImpl::tryMove(Instruction &ToMove, FPRoundingModeGroup &Group) {
    ...
    // In case instruction will have to be moved, uses will have to be moved to.
    ...
    if (UsersSwitchingRM > 0) {
        ...
        // if it is already a part of the group.
        if (std::find_if(Users.begin(), Users.end(), [&](Instruction *I) { return Group.contains(I); }) != Users.end())
            return false;
        ...
        for (auto It = Users.rbegin(); It != Users.rend(); ++It) {
            if (IGCLLVM::comesBefore(*It, InsertPoint))
                (*It)->moveAfter(InsertPoint);
        }
    }

    if (InsertPoint)
        ToMove.moveAfter(InsertPoint);
