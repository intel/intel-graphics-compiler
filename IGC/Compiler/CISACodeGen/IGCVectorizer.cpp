...
bool IGCVectorizer::handleSelectInstruction(VecArr &Slice) {
  ...
  bool IsScalar = !Operands[0]->getType()->isVectorTy();
  CmpInst *VecCmp = llvm::dyn_cast<CmpInst>(Operands[0]);
  // we support select vectorization only if we use same predicate (allowed under flag)
  // or predicate was previously vectorized and is now vector cmp instruction
  bool Vectorize = VecCmp || (IsScalar && IGC_IS_FLAG_ENABLED(VectorizerAllowSamePredSelect));
  if (!Vectorize) {
    PRINT_LOG_NL("predicate is not generated by vector cmp or scalar instruction");
    return true;
  }
