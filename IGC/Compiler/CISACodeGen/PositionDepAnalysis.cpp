...
#define DEBUG_TYPE "position-dep-analysis"
...
bool PositionDepAnalysis::runOnFunction(Function &F) {
  m_ctx = getAnalysis<CodeGenContextWrapper>().getCodeGenContext();
  m_newURBEncoding = m_ctx->platform.hasLSCUrbMessage();
    ...
    if (intr->getIntrinsicID() == GenISAIntrinsic::GenISA_URBWrite) {
      if (m_ctx->type == ShaderType::VERTEX_SHADER) {
        if (ConstantInt *index = dyn_cast<ConstantInt>(intr->getOperand(0))) {
          ...
          if (index->isZero() || (m_newURBEncoding && index->getZExtValue() == 16) ||
              (!m_newURBEncoding && index->isOne())) {
...
void PositionDepAnalysis::UpdateDependency(Instruction *inst) {
  ...
  if (it.second) {
    LLVM_DEBUG(dbgs() << "[PosDep] dep:" << *inst << "\n");
