...
PushAnalysis::PushAnalysis() : ModulePass(ID), m_PDT(nullptr), m_cbToLoad((uint)-1), m_maxStatelessOffset(0) {
...
Value *PushAnalysis::addArgumentAndMetadata(llvm::Type *pType, std::string argName,
                                            IGC::WIAnalysis::WIDependancy dependency) {
...
bool PushAnalysis::IsPushableBindlessLoad(Instruction *inst, int &grfOffset, unsigned int &cbIdx,
                                          unsigned int &offset) {
  LdRawIntrinsic *ldRaw = dyn_cast<LdRawIntrinsic>(inst);
  if (!ldRaw ||
      BINDLESS_CONSTANT_BUFFER != DecodeBufferType(ldRaw->getResourceValue()->getType()->getPointerAddressSpace())) {
    return false;
  }
...
bool PushAnalysis::IsStatelessCBLoad(llvm::Instruction *inst, int &pushableAddressGrfOffset,
                                     int &pushableOffsetGrfOffset, unsigned int &pushableOffset) {
    ...
    if (genIntr->getIntrinsicID() == GenISAIntrinsic::GenISA_pair_to_ptr) {
      ...
      if (Lo && Hi && Lo->getOperand(0) == Hi->getOperand(0)) {
        if (GenIntrinsicInst *genIntr2 = dyn_cast<GenIntrinsicInst>(Lo->getOperand(0))) {
          ...
          while (genIntr2 && genIntr2->getIntrinsicID() == GenISAIntrinsic::GenISA_add_pair) {
            pAddress = nullptr;
  ...
  if (offset >= 0 && offset <= std::numeric_limits<uint>::max() && // pushableOffset is a positive 32bit value
      (potentialPushableAddresses.size() == 1 || potentialPushableAddresses.size() == 2)) {
    for (Value *potentialAddress : potentialPushableAddresses) {
      bool isPushable = IsPushableAddress(inst, potentialAddress, pushableAddressGrfOffset, pushableOffsetGrfOffset);
      if (!isPushable) {
        return false;
      }
    }
...
bool PushAnalysis::IsPushableAddress(llvm::Instruction *inst, llvm::Value *pAddress, int &pushableAddressGrfOffset,
  ...
  IGC_ASSERT(32 == runtimevalSize || 64 == runtimevalSize);
  ...
  for (const auto &it : pushInfo.pushableAddresses) {
    ...
    if (IGC_IS_FLAG_ENABLED(DisableStaticCheck) || it.isStatic || IsSafeToPushNonStaticBufferLoad(inst)) {
    ...
    uint &relativeOffsetInBytes) // offset in bytes starting from dynamic offset in buffer
{
  if (ConstantInt *constOffset = dyn_cast<ConstantInt>(offsetValue)) {
    ...
    if (src1 && src0 && src0->getOpcode() == Instruction::And && isa<ConstantInt>(src0->getOperand(1))) {
      uint orOffset = int_cast<uint>(src1->getZExtValue());
      uint andMask = int_cast<uint>(cast<ConstantInt>(src0->getOperand(1))->getZExtValue());
      uint offset = 0;
      if ((orOffset & andMask) == 0 && GetConstantOffsetForDynamicUniformBuffer(src0->getOperand(0), offset) &&
          offset == 0) {
...
bool PushAnalysis::IsPushableShaderConstant(Instruction *inst, SimplePushInfo &info) {
  ...
  if (!elemType->isFloatTy() && !elemType->isIntegerTy(32) &&
      !(elemType->isPointerTy() && GetSizeInBits(elemType) == 32)) {
    return false;
  }

  if (IsPushableBindlessLoad(inst, info.pushableOffsetGrfOffset, info.cbIdx, info.offset)) {
    if ((info.offset % 4) == 0) {
      info.isBindless = true;
      return true;
    }
  } else if (IsLoadFromDirectCB(inst, info.cbIdx, eltPtrVal)) {
    ...
    return false;
  } else if (IsStatelessCBLoad(inst, info.pushableAddressGrfOffset, info.pushableOffsetGrfOffset, info.offset)) {
...
bool PushAnalysis::CanPushConstants() {
  ...
  if (m_context->m_DriverInfo.WaDisablePushConstantsWithNoPushedAttributes() &&
      m_context->platform.getWATable().Wa_16011983264 && m_context->type == ShaderType::HULL_SHADER) {
    Function *URBReadFunction =
        GenISAIntrinsic::getDeclaration(m_pFunction->getParent(), GenISAIntrinsic::GenISA_URBRead);
...
unsigned int PushAnalysis::GetSizeInBits(Type *type) const {
  ...
  } else if (type->isVectorTy() && type->getScalarType()->isPointerTy()) {
    size = m_DL->getPointerSizeInBits(type->getScalarType()->getPointerAddressSpace());
    IGC_ASSERT(size == 32 || size == 64);
...
void PushAnalysis::AllocatePushedConstant(Instruction *load, const SimplePushInfo &newChunk,
                                          const unsigned int maxSizeAllowed) {
  if (!newChunk.isBindless && newChunk.cbIdx > m_context->m_DriverInfo.MaximumSimplePushBufferID()) {
    return;
  }
  ...
  for (piIndex = 0; piIndex < numSimplePush; piIndex++) {
    ...
    if (info.isStateless && newChunk.isStateless &&
    ...
    // need to match.
    if (info.isBindless && newChunk.isBindless && info.pushableOffsetGrfOffset == newChunk.pushableOffsetGrfOffset &&
        info.cbIdx == newChunk.cbIdx) {
      regionFound = true;
      break;
    }
    // Bound load - binding table indices need to match.
    if (!info.isStateless && !newChunk.isStateless && !info.isBindless && !newChunk.isBindless &&
        info.cbIdx == newChunk.cbIdx) {
      regionFound = true;
      break;
    }
  ...
  if (regionFound) {
    ...
    unsigned int newStartOffset =
        iSTD::RoundDown(std::min(newChunk.offset, info.offset), getMinPushConstantBufferAlignmentInBytes());
    unsigned int newEndOffset = iSTD::Round(std::max(newChunk.offset + size, info.offset + info.size),
...
void PushAnalysis::PromoteLoadToSimplePush(Instruction *load, SimplePushInfo &info, unsigned int offset) {
  ...
  if (pTypeToPush->isPointerTy()) {
    pTypeToPush =
        IntegerType::get(load->getContext(), m_DL->getPointerSizeInBits(pTypeToPush->getPointerAddressSpace()));
  }
...
void PushAnalysis::BlockPushConstants() {
  ...
  const unsigned int cthreshold = std::min(m_pullConstantHeuristics->getPushConstantThreshold(m_pFunction) *
                                               getMinPushConstantBufferAlignmentInBytes(),
  ...
  for (auto bb = m_pFunction->begin(), be = m_pFunction->end(); bb != be; ++bb) {
    for (auto i = bb->begin(), ie = bb->end(); i != ie; ++i) {
      ...
      if (isPushable) {
        AllocatePushedConstant(instr, info,
...
PushConstantMode PushAnalysis::GetPushConstantMode() {
    ...
    if (pushConstantMode == PushConstantMode::DEFAULT) {
      if (m_context->m_DriverInfo.SupportsSimplePushOnly()) {
        pushConstantMode = PushConstantMode::SIMPLE;
                 ...
                 m_context->m_DriverInfo.SupportsGatherConstantOnly()) {
  ...
  IGC_ASSERT_MESSAGE(pushConstantMode != PushConstantMode::DEFAULT,
                     "GetPushConstantMode should resolve a non-DEFAULT mode!");
...
void PushAnalysis::processGather(Instruction *inst, uint bufId, uint eltId) {
  ...
  for (unsigned int i = 0; i < num_elms; ++i) {
    ...
    if (it != pushInfo.constants.end() || (pushInfo.constantReg.size() + pushInfo.constants.size() <
                                           m_pullConstantHeuristics->getPushConstantThreshold(m_pFunction) * 8)) {
      llvm::Value *value = nullptr;
      ...
      if (it == pushInfo.constants.end()) {
        ...
        value = addArgumentAndMetadata(pTypeToPush,
                                       VALUE_NAME(std::string("cb_") + to_string(bufId) + std::string("_") +
...
void PushAnalysis::processInput(Instruction *inst, bool gsInstancingUsed) {
  ...
  if (
    ...
    if (llvm::ConstantInt *pIndex = llvm::dyn_cast<llvm::ConstantInt>(inst->getOperand(0))) {
      ...
      const bool uniformInput = (mode == EINTERPOLATION_CONSTANT) || gsInstancingUsed;
      ...
      if (it == pushInfo.inputs.end()) {
        ...
        IGC_ASSERT_MESSAGE((it->second.argIndex <= m_argIndex),
...
void PushAnalysis::processInputVec(Instruction *inst, bool gsInstancingUsed) {
  ...
  if (m_context->type == ShaderType::PIXEL_SHADER &&
      (m_context->platform.hasDualKSPPS() || m_context->platform.supportDualSimd8PS()))
    return;
      ...
      if (mode == EINTERPOLATION_CONSTANT || mode == EINTERPOLATION_VERTEX) {
        for (auto I = inst->user_begin(), E = inst->user_end(); I != E; ++I) {
          if (llvm::ExtractElementInst *extract = llvm::dyn_cast<llvm::ExtractElementInst>(*I)) {
            ...
            input.index =
                inputIndex * 4 + int_cast<uint>(llvm::cast<ConstantInt>(extract->getIndexOperand())->getZExtValue());
            input.interpolationMode = mode;
            // if we know input is uniform, update WI analysis results.
            const bool uniformInput = (mode == EINTERPOLATION_CONSTANT) || gsInstancingUsed;
            ...
            auto it = pushInfo.inputs.find(input.index);
            if (it == pushInfo.inputs.end()) {
              addArgumentAndMetadata(extract->getType(),
                                     VALUE_NAME(std::string("pushedinput_") + to_string(input.index)),
                                     uniformInput ? WIAnalysis::UNIFORM_GLOBAL : WIAnalysis::RANDOM);
              input.argIndex = m_argIndex;
              pushInfo.inputs[input.index] = input;
            } else {
...
void PushAnalysis::processURBRead(Instruction *inst, bool gsInstancingUsed,
  ...
  if (llvm::ConstantInt *pVertexIndex = llvm::dyn_cast<llvm::ConstantInt>(inst->getOperand(0))) {
    ...
    if (llvm::ConstantInt *pElementIndex = llvm::dyn_cast<llvm::ConstantInt>(inst->getOperand(1))) {
      ...
      bool pushCondition =
          ((urbOffset >= urbReadOffsetForPush) &&
           ((urbOffset - urbReadOffsetForPush) < (int)numberOfElementsPerVertexThatAreGoingToBePushed));
      ...
      if (pElementIndex && pushCondition) {
        ...
        uint currentElementIndex =
            (vertexIndex * numberOfElementsPerVertexThatAreGoingToBePushed * 4) + (elementIndex * 4);
        ...
        for (auto I = inst->user_begin(), E = inst->user_end(); I != E; ++I) {
          ...
          if (extract && llvm::isa<ConstantInt>(extract->getIndexOperand())) {
            ...
            input.index =
                currentElementIndex + int_cast<uint>(cast<ConstantInt>(extract->getIndexOperand())->getZExtValue());
            input.interpolationMode =
                AreUniformInputsBasedOnDispatchMode() ? EINTERPOLATION_CONSTANT : EINTERPOLATION_VERTEX;

            const bool uniformInput =
                AreUniformInputsBasedOnDispatchMode() ||
                (vsHasConstantBufferIndexedWithInstanceId && vsUrbReadIndexForInstanceIdSGV == input.index) ||
                gsInstancingUsed;
...
void PushAnalysis::processRuntimeValue(GenIntrinsicInst *intrinsic) {
  ...
  if (it == pushInfo.constantReg.end()) {
    while (runtimeValue->hasOneUse() &&
           (isa<BitCastInst>(runtimeValue->user_back()) || isa<IntToPtrInst>(runtimeValue->user_back()))) {
      Value *castInst = runtimeValue->user_back();
      if ((index % 2) != 0 && (64 == GetSizeInBits(castInst->getType())) && !castInst->getType()->isVectorTy()) {
    ...
    if (m_context->m_DriverInfo.SupportsDynamicUniformBuffers() &&
        IGC_IS_FLAG_DISABLED(DisableSimplePushWithDynamicUniformBuffers) && !pushInfo.dynamicBufferInfo.forceDisabled &&
        pushInfo.dynamicBufferInfo.numOffsets > 0 && index >= pushInfo.dynamicBufferInfo.firstIndex &&
    ...
    while (arg->getType() != runtimeValue->getType() && runtimeValue->hasOneUse() &&
           (isa<BitCastInst>(runtimeValue->user_back()) || isa<IntToPtrInst>(runtimeValue->user_back()))) {
...
void PushAnalysis::ProcessFunction() {
  ...
  for (auto BB = m_pFunction->begin(), E = m_pFunction->end(); BB != E; ++BB) {
    for (auto &instRef : *BB) {
      ...
      SimplePushInfo info{};
      if (pushConstantMode == PushConstantMode::GATHER && IsPushableShaderConstant(inst, info) && !info.isBindless &&
          !info.isStateless) {
        ...
        if (DispatchGRFHardwareWAForHSAndGSDisabled() &&
          ...
          processURBRead(inst, gsInstancingUsed, vsHasConstantBufferIndexedWithInstanceId,
  ...
  if (!m_context->getModuleMetaData()->pushInfo.inputs.empty() && m_context->type == ShaderType::VERTEX_SHADER &&
    ...
    for (int i = 0; i < maxIndex; ++i) {
      if (pushInfo.inputs.find(i) == pushInfo.inputs.end()) {
        ...
        addArgumentAndMetadata(Type::getFloatTy(m_pFunction->getContext()),
                               VALUE_NAME(std::string("urb_read_") + to_string(input.index)),
                               uniformInput ? WIAnalysis::UNIFORM_GLOBAL : WIAnalysis::RANDOM);
        input.argIndex = m_argIndex;
        pushInfo.inputs[input.index] = input;
      }
...
bool PushAnalysis::runOnModule(llvm::Module &M) {
  ...
  for (const auto &i : funcsMapping) {
    IGCMD::IGCMetaDataHelper::moveFunction(*m_pMdUtils, *m_context->getModuleMetaData(), i.first, i.second);
