...
namespace IGC
{
    ...
    {
        return m_Options.NoLocalToGeneric;
    }
    ...
    void OpenCLProgramContext::Options::parseOptions(const char* opts)
    {
        ...
        if (const opt::Arg* arg = apiOptions.getLastArg(OPT_large_grf_kernel_common))
        {
            llvm::StringRef valStr = arg->getValue();
            LargeGRFKernels.push_back(valStr.str());
        }

        if (const opt::Arg* arg = apiOptions.getLastArg(OPT_regular_grf_kernel_common))
        {
            llvm::StringRef valStr = arg->getValue();
