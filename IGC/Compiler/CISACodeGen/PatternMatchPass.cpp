  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (mul && mul->getOpcode() == Instruction::Mul) {
      ...
      struct {
        ...
        bool isCandidate() {
          return (src->getType()->isIntegerTy() && src->getType()->getIntegerBitWidth() <= 16) || needsRegioning ||
                 isConstant16BitEq;
        }
      ...
      // Get source modifiers for all operands
      ...
      if (I.getOpcode() == Instruction::Sub) {
        if (i == 0) {
          ...
          if (auto *CI = dyn_cast<ConstantInt>(oprdInfo[0].src)) {
            ...
            if (CI->isNegative()) {
              // can flip to positive, no need to check whether it is still candidate, just change isSigned
              ...
              if (val > INT16_MAX) {
                if (skipModifierSrc1) {
                  // must put modifier on src0
                  oprdInfo[0].src = ConstantInt::get(CI->getType(), -val, true);
                  oprdInfo[0].isSigned = true;
                  oprdInfo[0].isConstant16BitEq = false;
                } else {
                  // not restricted, can put modifier on src1 instead without making src0 non-16-bit candidate
                ...
                oprdInfo[0].src = ConstantInt::get(CI->getType(), -val, true);
                oprdInfo[0].isSigned = true;
                oprdInfo[0].isConstant16BitEq = true; // In case CI is INT16_MAX + 1
              }
            }
          } else {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (oprdInfo[i].isCandidate()) {
          ...
          // for now, Use W/UW only if needsRegioning or isConstant16BitEq is true, region_set is false or the src is scalar
          if ((oprdInfo[i].needsRegioning || oprdInfo[i].isConstant16BitEq) && !thisSrc.region_set ||
        ...
        if (isa<ConstantInt>(oprdInfo[i].src)) {
          if (!m_Platform.support16BitImmSrcForMad() || !isValueConstantInt16Bit(oprdInfo[i].src) || addToConstantPool) {
