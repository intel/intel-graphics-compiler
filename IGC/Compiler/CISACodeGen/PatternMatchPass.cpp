...
void CodeGenPatternMatch::visitBinaryOperator(llvm::BinaryOperator &I) {
  ...
  case Instruction::FAdd:
    ...
    break;
  case Instruction::And:
    match = MatchBitwiseI64Halves(I) || MatchBfn(I) || MatchBoolOp(I) || MatchLogicAlu(I);
    break;
  case Instruction::Or:
    match = MatchBitwiseI64Halves(I) || MatchPack4i8(I) || MatchBfn(I) || MatchBoolOp(I) || MatchLogicAlu(I);
    break;
  case Instruction::Xor:
    match = MatchBitwiseI64Halves(I) || MatchBfn(I) || MatchLogicAlu(I);
    break;
  default:
...
void CodeGenPatternMatch::visitBitCastInst(BitCastInst &I) {
  ...
  [[maybe_unused]] bool match = false;
  match = MatchPackBfnI64(I) || MatchPackI64(I) || MatchRepack4i8(I) || MatchPack4i8(I) || MatchSingleInstruction(I);
...
// Map LLVM bitwise opcode to IGC e_opcode. Returns false for non-bitwise ops.
static bool matchBitwiseOpcode(unsigned llvmOp, e_opcode &opCode) {
  switch (llvmOp) {
  case Instruction::And:
    opCode = EOPCODE_AND;
    return true;
...
// Match bitwise op (and/or/xor) on lo/hi halves extracted from i64 values:
//   %bc0 = bitcast i64 %a to <2 x i32>
//   %ext0 = extractelement <2 x i32> %bc0, i32 idx
//   %bc1 = bitcast i64 %b to <2 x i32>
//   %ext1 = extractelement <2 x i32> %bc1, i32 idx
//   %res = {and|or|xor} i32 %ext0, %ext1
// Also handles the case where only one operand is an extractelement.
// Emit the op with stride-2 regioning directly on i64 source aliases,
// eliminating intermediate extractelement mov instructions.
bool CodeGenPatternMatch::MatchBitwiseI64Halves(llvm::Instruction &I) {
  using namespace llvm::PatternMatch;

  // Skip stride-2 regioning optimization at O0 for better debuggability.
  if (m_ctx->getModuleMetaData()->compOpt.OptDisable)
    return false;

  // Stride-2 regioning and SIMD32 split logic assumes 64-byte GRFs.
  if (m_Platform.getGRFSize() != 64)
    return false;

  struct BitwiseI64HalvesPattern : public Pattern {
    SSource sources[2];
    uint32_t extractIdx;
    bool isI64Half[2]; // per-operand: true if from extract of bitcast i64
    e_opcode opCode;
    virtual void Emit(EmitPass *pass, const DstModifier &modifier) {
      pass->EmitBitwiseI64Halves(opCode, sources, extractIdx, isI64Half, modifier);
    }
  };

  // Try to match each operand as: extractelement(bitcast(i64 → <2 x i32>), const_idx)
  // Reject constant i64 sources — the emitter aliases via GetNewAlias which
  // does not accept immediates.
  bool isI64Half[2] = {false, false};
  uint32_t extractIdx[2] = {0, 0};
  Value *i64Srcs[2] = {nullptr, nullptr};

  for (unsigned i = 0; i < 2; ++i) {
    Value *src = nullptr;
    ConstantInt *idxCI = nullptr;
    if (!match(I.getOperand(i), m_ExtractElt(m_BitCast(m_Value(src)), m_ConstantInt(idxCI))))
      continue;
    if (!src->getType()->isIntegerTy(64) || isa<Constant>(src))
      continue;
    auto *extElt = cast<ExtractElementInst>(I.getOperand(i));
    auto *bc = cast<BitCastInst>(extElt->getVectorOperand());
    // All matched instructions must be in the same basic block.
    if (extElt->getParent() != I.getParent() || bc->getParent() != I.getParent())
      continue;
    auto *vecTy = dyn_cast<IGCLLVM::FixedVectorType>(bc->getType());
    if (!vecTy || vecTy->getNumElements() != 2 || !vecTy->getElementType()->isIntegerTy(32))
      continue;
    isI64Half[i] = true;
    extractIdx[i] = int_cast<uint32_t>(idxCI->getZExtValue());
    i64Srcs[i] = src;
  }

  // Need at least one operand from extractelement
  if (!isI64Half[0] && !isI64Half[1])
    return false;

  // If both are extractelement, indices must match
  if (isI64Half[0] && isI64Half[1] && extractIdx[0] != extractIdx[1])
    return false;

  uint32_t idx = isI64Half[0] ? extractIdx[0] : extractIdx[1];

  e_opcode opCode;
  if (!matchBitwiseOpcode(I.getOpcode(), opCode))
    return false;

  BitwiseI64HalvesPattern *pattern = new (m_allocator) BitwiseI64HalvesPattern();
  for (unsigned i = 0; i < 2; ++i) {
    pattern->isI64Half[i] = isI64Half[i];
    if (isI64Half[i]) {
      pattern->sources[i] = GetSource(i64Srcs[i], false, false, IsSourceOfSample(&I));
    } else {
      pattern->sources[i] = GetSource(I.getOperand(i), false, false, IsSourceOfSample(&I));
    }
  }
  pattern->extractIdx = idx;
  pattern->opCode = opCode;
  AddPattern(pattern);
  return true;
}

// Match insertelement→bitcast pack of two i32 values into i64, where both
// halves are produced by single-use bitwise ALU ops (and/or/xor).
// The ALU ops are folded into the pack, writing directly into the i64
// destination with stride-2 regioning and eliminating intermediate movs.
//
// Match the following sequence:
//   %lo = {and|or|xor} i32 %a, %b   ; single-use
//   %hi = {and|or|xor} i32 %c, %d   ; single-use
//   %vec0 = insertelement <2 x i32> undef, i32 %lo, i32 0
//   %vec1 = insertelement <2 x i32> %vec0, i32 %hi, i32 1
//   %dst  = bitcast <2 x i32> %vec1 to i64
// And generate:
//   {and|or|xor} (M1_NM, 1) dst_0(0,0)<2> a(0,0)<0;1,0> b(0,0)<0;1,0>
//   {and|or|xor} (M1_NM, 1) dst_0(0,1)<2> c(0,0)<0;1,0> d(0,0)<0;1,0>
//
// When an operand of a folded op is itself an extractelement from a
// bitcast i64→<2 x i32>, stride-2 source regioning is used to read
// the lo/hi half directly from the i64 source, avoiding an intermediate mov.
//

// Check if val is extractelement from bitcast i64 to <2 x i32>.
// On match, populate pattern fields at the given index and return true.
static bool tryMatchI64Half(CodeGenPatternMatch *matcher, Value *val, unsigned idx, bool *isI64Half,
                            uint32_t *extractIdx, SSource *sources, Instruction &I) {
  using namespace llvm::PatternMatch;
  Value *src = nullptr;
  ConstantInt *idxCI = nullptr;
  if (!match(val, m_ExtractElt(m_BitCast(m_Value(src)), m_ConstantInt(idxCI))))
    return false;
  if (!src->getType()->isIntegerTy(64) || isa<Constant>(src))
    return false;
  auto *extElt = cast<ExtractElementInst>(val);
  auto *bc = cast<BitCastInst>(extElt->getVectorOperand());
  // All matched instructions must be in the same basic block.
  if (extElt->getParent() != I.getParent() || bc->getParent() != I.getParent())
    return false;
  ...
  if (!vecTy || vecTy->getNumElements() != 2 || !vecTy->getElementType()->isIntegerTy(32))
    return false;
  isI64Half[idx] = true;
  extractIdx[idx] = int_cast<uint32_t>(idxCI->getZExtValue());
  sources[idx] = matcher->GetSource(src, false, false, matcher->IsSourceOfSample(&I));
  return true;
}

bool CodeGenPatternMatch::MatchPackI64(llvm::Instruction &I) {
  using namespace llvm::PatternMatch;
  ...
  struct PackI64Pattern : public Pattern {
    SSource sources[4]; // [half*2+op]
    bool isI64Half[4] = {};
    uint32_t extractIdx[4] = {};
    e_opcode foldedOp[2];
    virtual void Emit(EmitPass *pass, const DstModifier &modifier) {
      pass->EmitPackI64(sources, isI64Half, extractIdx, foldedOp, modifier);
    }
  };
  ...
  if (!I.getType()->isIntegerTy(64))
    return false;
  auto *srcVecTy = dyn_cast<IGCLLVM::FixedVectorType>(I.getOperand(0)->getType());
  if (!srcVecTy || srcVecTy->getNumElements() != 2 || !srcVecTy->getElementType()->isIntegerTy(32))
    return false;

  // Match insertelement chain:
  //   insertelement(insertelement(undef, %innerVal, innerI), %outerVal, outerI)
  Value *outerVal = nullptr, *innerVal = nullptr;
  ConstantInt *outerIdx = nullptr, *innerIdx = nullptr;
  if (!match(I.getOperand(0),
             m_OneUse(m_InsertElt(m_OneUse(m_InsertElt(m_Undef(), m_Value(innerVal), m_ConstantInt(innerIdx))),
                                  m_Value(outerVal), m_ConstantInt(outerIdx)))))
    return false;

  // All matched instructions must be in the same basic block.
  auto *outerInsert = cast<InsertElementInst>(I.getOperand(0));
  auto *innerInsert = cast<InsertElementInst>(outerInsert->getOperand(0));
  BasicBlock *BB = I.getParent();
  if (outerInsert->getParent() != BB || innerInsert->getParent() != BB)
    return false;

  uint64_t outerI = outerIdx->getZExtValue();
  uint64_t innerI = innerIdx->getZExtValue();
  if (outerI > 1 || innerI > 1 || outerI == innerI)
    return false;

  // Identify lo (index 0) and hi (index 1) values.
  // Both must be single-use bitwise BinaryOperators.
  Value *vals[2];
  vals[innerI] = innerVal;
  vals[outerI] = outerVal;

  e_opcode foldedOp[2];
  BinaryOperator *binOps[2];
  for (unsigned half = 0; half < 2; ++half) {
    auto *binOp = dyn_cast<BinaryOperator>(vals[half]);
    if (!binOp || !binOp->hasOneUse())
      return false;
    if (binOp->getParent() != BB)
      return false;
    if (!matchBitwiseOpcode(binOp->getOpcode(), foldedOp[half]))
      return false;
    binOps[half] = binOp;
  }

  // Special case: both halves are XOR with all-ones (NOT).
  // Each half's NOT operand can independently be an extractelement from a
  // bitcast i64 → <2 x i32> (using stride-2 source regioning) or a plain
  // i32 value. NOT uses stride-2 destination regioning.
  {
    struct PackI64NotPattern : public Pattern {
      SSource sources[2];
      bool isI64Half[2] = {};
      uint32_t extractIdx[2] = {};
      virtual void Emit(EmitPass *pass, const DstModifier &modifier) {
        pass->EmitPackI64Not(sources, isI64Half, extractIdx, modifier);
      }
    };

    // Check if both halves are NOT: xor %v, -1
    Value *notSrcs[2] = {};
    if (match(binOps[0], m_Not(m_Value(notSrcs[0]))) && match(binOps[1], m_Not(m_Value(notSrcs[1])))) {
      auto *pattern = new (m_allocator) PackI64NotPattern();
      for (unsigned half = 0; half < 2; ++half) {
        if (!tryMatchI64Half(this, notSrcs[half], half, pattern->isI64Half, pattern->extractIdx, pattern->sources, I))
          pattern->sources[half] = GetSource(notSrcs[half], false, false, IsSourceOfSample(&I));
      }
      AddPattern(pattern);
      return true;
    }
  ...
  PackI64Pattern *pattern = new (m_allocator) PackI64Pattern();
  pattern->foldedOp[0] = foldedOp[0];
  pattern->foldedOp[1] = foldedOp[1];

  for (unsigned half = 0; half < 2; ++half) {
    for (unsigned op = 0; op < 2; ++op) {
      unsigned idx = half * 2 + op;
      if (!tryMatchI64Half(this, binOps[half]->getOperand(op), idx, pattern->isI64Half, pattern->extractIdx,
                           pattern->sources, I))
        pattern->sources[idx] = GetSource(binOps[half]->getOperand(op), false, false, IsSourceOfSample(&I));
    }
  }
...
// Match insertelement→bitcast pack of two i32 values into i64, where both
// halves are produced by a chain of two nested binary logic ops that can be
// folded into a single BFN instruction per half. Each BFN writes directly
// into the i64 destination with stride-2 regioning.
//
// Match the following sequence:
//   %t0 = {and|or|xor} i32 %a, %b   ; inner op, single-use
//   %lo = {and|or|xor} i32 %t0, %c  ; root op, single-use
//   %t1 = {and|or|xor} i32 %d, %e   ; inner op, single-use
//   %hi = {and|or|xor} i32 %t1, %f  ; root op, single-use
//   %vec0 = insertelement <2 x i32> undef, i32 %lo, i32 0
//   %vec1 = insertelement <2 x i32> %vec0, i32 %hi, i32 1
//   %dst  = bitcast <2 x i32> %vec1 to i64
// And generate:
//   bfn.xNN (M1, 32) dst_0v(0,0)<2> a<2;1,0> b<2;1,0> c<2;1,0>
//   bfn.xNN (M1, 32) dst_0v(0,1)<2> d<2;1,0> e<2;1,0> f<2;1,0>
//
bool CodeGenPatternMatch::MatchPackBfnI64(llvm::Instruction &I) {
  using namespace llvm::PatternMatch;
  ...
  struct PackBfnI64Pattern : public Pattern {
    SSource sources[6]; // [half*3+srcIdx]
    bool isI64Half[6] = {};
    uint32_t extractIdx[6] = {};
    uint8_t booleanFuncCtrl[2];
    virtual void Emit(EmitPass *pass, const DstModifier &modifier) {
      pass->EmitPackBfnI64(sources, isI64Half, extractIdx, booleanFuncCtrl, modifier);
    }
  };

  // Check platform support for BFN
  ...
  auto isBinaryLogic = [](unsigned op) {
    return op == Instruction::Or || op == Instruction::And || op == Instruction::Xor;
  };

  auto applyBfnOp = [](uint8_t a, uint8_t b, unsigned opcode) -> uint8_t {
    switch (opcode) {
    case Instruction::And:
      return a & b;
    case Instruction::Or:
  ...
  const uint8_t bfnSrc[3] = {0xAA, 0xCC, 0xF0};
  const int useThreshold = 4;

  // For each half, try to match a BFN pattern (3 sources from 2 nested binary logic ops)
  Value *bfnSources[2][3];
  uint8_t bfnCtrl[2];

  for (unsigned half = 0; half < 2; ++half) {
    auto *rootOp = dyn_cast<BinaryOperator>(vals[half]);
    if (!rootOp || !rootOp->hasOneUse() || !isBinaryLogic(rootOp->getOpcode()))
      return false;
    if (rootOp->getParent() != BB)
      return false;

    bool foundBfn = false;
    for (unsigned side = 0; side < 2 && !foundBfn; ++side) {
      auto *innerOp = dyn_cast<BinaryOperator>(rootOp->getOperand(side));
      if (!innerOp || !isBinaryLogic(innerOp->getOpcode()) || innerOp->hasNUsesOrMore(useThreshold))
        continue;
      if (innerOp->getParent() != BB)
        continue;

      if (side == 0) {
        // Tree: (innerOp.lhs innerOpcode innerOp.rhs) rootOpcode rootOp.rhs
        bfnSources[half][0] = innerOp->getOperand(0);
        bfnSources[half][1] = innerOp->getOperand(1);
        bfnSources[half][2] = rootOp->getOperand(1);
        bfnCtrl[half] =
            applyBfnOp(applyBfnOp(bfnSrc[0], bfnSrc[1], innerOp->getOpcode()), bfnSrc[2], rootOp->getOpcode());
      } else {
        // Tree: rootOp.lhs rootOpcode (innerOp.lhs innerOpcode innerOp.rhs)
        bfnSources[half][0] = rootOp->getOperand(0);
        bfnSources[half][1] = innerOp->getOperand(0);
        bfnSources[half][2] = innerOp->getOperand(1);
        bfnCtrl[half] =
            applyBfnOp(bfnSrc[0], applyBfnOp(bfnSrc[1], bfnSrc[2], innerOp->getOpcode()), rootOp->getOpcode());
      }
      foundBfn = true;
    }
    if (!foundBfn)
  ...
  PackBfnI64Pattern *pattern = new (m_allocator) PackBfnI64Pattern();
  pattern->booleanFuncCtrl[0] = bfnCtrl[0];
  pattern->booleanFuncCtrl[1] = bfnCtrl[1];

  for (unsigned half = 0; half < 2; ++half) {
    for (unsigned s = 0; s < 3; ++s) {
      unsigned idx = half * 3 + s;
      if (!tryMatchI64Half(this, bfnSources[half][s], idx, pattern->isI64Half, pattern->extractIdx, pattern->sources,
                           I)) {
        pattern->sources[idx] = GetSource(bfnSources[half][s], false, false, IsSourceOfSample(&I));
        // BFN can use imm16 in src0 and src2; src1 must always go through
        // the constant pool. Mirror the legalization from MatchBfn.
        if (auto *CI = dyn_cast<ConstantInt>(bfnSources[half][s])) {
          if (s == 1 || !CI->getType()->isIntegerTy(16)) {
            AddToConstantPool(I.getParent(), bfnSources[half][s]);
            pattern->sources[idx].fromConstantPool = true;
          }
        }
      }
    }
