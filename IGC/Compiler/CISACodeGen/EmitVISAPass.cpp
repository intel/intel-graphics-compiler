...
void EmitPass::emitReductionAllAdd(
    e_opcode op, uint64_t identityValue, VISA_Type type, bool negate, CVariable* src, CVariable* dst)
{
    ...
    if ((type != ISA_TYPE_F) && src->IsImmediate() && src->GetImmediateValue() == 1)
    {
        if (negate)
        {
            m_encoder->SetSrcModifier(0, EMOD_NEG);
        }
        m_encoder->Cast(dst, numActiveLanes);
        m_encoder->Push();
    }
    else
    {
        ...
        m_encoder->Mul(dst, src, castNumActiveLanes);
        m_encoder->Push();
    ...
    e_opcode op, uint64_t identityValue, VISA_Type type, bool negate, CVariable* src, CVariable* dst)
{
    ...
    if (op == EOPCODE_ADD && src->IsUniform() && getCurrInstNumInstances() == 1)
    {
        emitReductionAllAdd(op, identityValue, type, negate, src, dst);
        return;
    }
...
void EmitPass::emitScalarAtomics(llvm::Instruction *pInst,
    ...
    if ((op == EOPCODE_AND || op == EOPCODE_OR) && pSrc->IsUniform())
    {
