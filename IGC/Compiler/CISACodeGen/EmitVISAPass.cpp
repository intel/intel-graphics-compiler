...
void EmitPass::emitStackCall(llvm::CallInst* inst)
{
    ...
    else
    {
        ...
        else
        {
            ...
            CVariable* tEM = m_currShader->GetNewVariable(eMask);  // don't write eMask as it is cached
            m_encoder->IndirectStackCall(nullptr, uniformAddr, argSizeInGRF, retSizeInGRF);
            m_encoder->Copy(tEM, eMask);
            m_encoder->Push();
            eMask = tEM;
...
{
    // execution mask is cached for each BB.
    SBasicBlock* currBlk = getCurrentBlock();
    if (currBlk && currBlk->m_execMask != nullptr)
    {
        // reuse the already-calculated var.
        return currBlk->m_execMask;
    }
