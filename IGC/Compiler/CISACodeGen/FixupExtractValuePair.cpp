...
public:
  ...
  ExtractValuePairFixup() : FunctionPass(ID) { initializeExtractValuePairFixupPass(*PassRegistry::getPassRegistry()); }
...
FunctionPass *IGC::createExtractValuePairFixupPass() { return new ExtractValuePairFixup(); }
...
bool ExtractValuePairFixup::fixup(Function &F) const {
  ...
  for (auto &BB : F) {
    for (auto BI = BB.begin(), BE = BB.end(); BI != BE; /*EMPTY*/) {
      ...
      for (++II; II != BI; ++II) {
        if (isa<ExtractValueInst, PHINode>(II))
          continue;
