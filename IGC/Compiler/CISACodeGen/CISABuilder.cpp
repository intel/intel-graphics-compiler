...
void CEncoder::InitVISABuilderOptions(TARGET_PLATFORM VISAPlatform, bool canAbortOnSpill, bool hasStackCall,
  ...
  // enabled by default for 3D (graphics pipeline) shaders on the platforms listed
  // in enableNewSpillCostFunctionDefault(). It can still be force-enabled for any
  // shader via the NewSpillCostFunction regkey, the compiler option or the
  // compute-shader metadata opt-in, and the default-on behavior can be turned off
  // via the DisableNewSpillCostFunction kill-switch.
  ...
  const bool newSpillCostDefaultOnFor3D = context->isGraphicsShaderType() &&
                                          m_program->m_Platform->enableNewSpillCostFunctionDefault() &&
                                          !IGC_IS_FLAG_ENABLED(DisableNewSpillCostFunction);
