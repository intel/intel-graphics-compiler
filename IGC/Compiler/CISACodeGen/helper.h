...
inline bool isEntryFunc(const IGC::ModuleMetaData *modMD, const llvm::Function *CF) {
  ...
  if (Info == modMD->FuncMD.end())
    return false;
  bool isEntryType = Info->second.functionType == FunctionTypeMD::KernelFunction ||
                     Info->second.functionType == FunctionTypeMD::CallableShader;
  // A linked-in BiF builtin (marked with !igc_bif metadata) is a callee, never
  // a shader entry. After the FunctionType migration its FuncMD entry can be
  // created late as the default KernelFunction; without this guard, entry-gated
  // passes (e.g. PixelShaderLowering) would run on the builtin and corrupt codegen.
  if (isEntryType && F->getMetadata("igc_bif") != nullptr)
    return false;
  return isEntryType;
}
