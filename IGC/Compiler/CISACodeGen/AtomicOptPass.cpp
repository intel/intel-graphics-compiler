...
Instruction *AtomicOptPass::createReduce(Instruction *Pos, Instruction *Atomic) {
    IRBuilder<> Builder(Pos);

    Value *ValueForReduce = nullptr;
    if (isa<GenIntrinsicInst>(Atomic))
        ValueForReduce = Atomic->getOperand(2);
    else
        ValueForReduce = Atomic->getOperand(1);
...
bool AtomicOptPass::checkFloatAtomicEmulation(Instruction *Inst) {
    ...
    auto BitCastPat =  m_BitCast(m_FAdd(m_BitCast(m_Instruction(Inst)), m_Value()));
    if (!match(cast<Value>(SecondBitcastInstr), BitCastPat))
        return false;
...
bool AtomicOptPass::runOnFunction(Function &F)
{
    Changed = false;
    M = F.getParent();
    llvm::SmallVector<std::tuple<Instruction*, BasicBlock*, BasicBlock*, Instruction*>, 32> AtomicsEmulationToProcess;
    ...
    for (auto &B : F) {
        for (auto &I : B) {
            ...
            if (checkFloatAtomicEmulation(&I)) {
                ...
                std::tuple instrTuple = std::make_tuple(&I, BackBb, ExitBb, MainInstr);
    ...
    for (auto T : AtomicsEmulationToProcess)
    {
        ...
        Instruction *NewReduce = createReduce(BbTerm, AtomicMain);
        if (!NewReduce)
            continue;

        IRBuilder<> Builder(BbTerm);
        AtomicMain->setOperand(1, NewReduce);
