...
/*static*/ bool DebugMetadataInfo::hasAnyDebugInfo(CodeGenContext *ctx,
                                                   bool &fullDebugInfo,
  ...
  if (Utils::hasDebugInfo(*ctx->getModule())) {
    ...
    for (auto funcIt = funcList.begin();
         funcIt != funcList.end() && !hasFullDebugInfo; funcIt++) {
      auto &func = (*funcIt);

      for (auto bbIt = func.begin(); bbIt != func.end() && !hasFullDebugInfo;
           bbIt++) {
        auto &bb = (*bbIt);

        for (auto instIt = bb.begin(); instIt != bb.end() && !hasFullDebugInfo;
             instIt++) {
          auto &inst = (*instIt);
...
VISAScalarModule::VISAScalarModule(CShader *TheShader,
...
VISAScalarModule::BuildNew(CShader *S, llvm::Function *F, bool IsPrimary) {
  auto *n = new VISAScalarModule(S, F, IsPrimary);
  return std::unique_ptr<IGC::VISAModule>(n);
}

unsigned VISAScalarModule::getPrivateBaseReg() const {
  auto pVar = privateBase;
  unsigned privateBaseRegNum =
      m_pShader->GetDebugInfoData().getVISADclId(pVar, 0);
  return privateBaseRegNum;
  ;
}

int VISAScalarModule::getPTOReg() const {
  ...
  IGC_ASSERT_MESSAGE(pVarPTO->GetVarType() == EVARTYPE_GENERAL,
                     "Unexpected VISA register type!");

  int regPTO = getDeclarationID(pVarPTO, false);
  return regPTO;
}
int VISAScalarModule::getFPReg() const {
...
llvm::StringRef VISAScalarModule::GetVISAFuncName() const {
  // since device enqueue no longer used, llvm's function name
  // matches that used by VISA.
  return getFunction()->getName();
}
uint64_t VISAScalarModule::getFPOffset() const {
  return EmitPass::getFPOffset();
}

bool VISAScalarModule::usesSlot1ScratchSpill() const {
  return m_pShader->ProgramOutput()->getScratchSpaceUsageInSlot1();
}

unsigned VISAScalarModule::getPointerSize() const {
  return IGC::getPointerSize((llvm::Module &)(*GetModule()));
}

uint64_t VISAScalarModule::getTypeSizeInBits(Type *Ty) const {
...
void VISAScalarModule::UpdateVisaId() {
  auto *Kernel = m_pShader->GetEncoder().GetVISAKernel();
  SetVISAId(Kernel->getvIsaInstCount());
}

void VISAScalarModule::ValidateVisaId() {
  IGC_ASSERT_MESSAGE(
      GetCurrentVISAId() ==
          m_pShader->GetEncoder().GetVISAKernel()->getvIsaInstCount(),
      "Missed emitted pattern!");
}

uint16_t VISAScalarModule::GetSIMDSize() const {
...
const Argument *
VISAScalarModule::GetTracedArgument64Ops(const Value *pVal) const {
  ...
  %25 = call { i32, i32 }
  @llvm.genx.GenISA.ptr.to.pair.p1opencl.image2d_t(%opencl.image2d_t
  addrspace(1)* %Input) %26 = extractvalue { i32, i32 } %25, 0 %27 =
  extractvalue { i32, i32 } %25, 1 %28 = insertelement <2 x i32> undef, i32 %26,
  i32 0 %29 = insertelement <2 x i32> %28, i32 %27, i32 1 %30 = bitcast i64* %24
  to <2 x i32>* store <2 x i32> %29, <2 x i32>* %30, align 8 call void
  @llvm.dbg.declare(metadata %opencl.image2d_t addrspace(1)**
  %.privateBuffer111, metadata !82, metadata !80), !dbg !83*/
  ...
  // % 24 = bitcast %opencl.image2d_t addrspace(1)** %.privateBuffer111 to i64*
  for (auto i = pBaseValue->user_begin(), e = pBaseValue->user_end(); i != e;
       ++i) {
    const Value *pUser = *i;
...
const Argument *VISAScalarModule::GetTracedArgument(const Value *pVal,
  ...
  while (true) {
    if (isAddress) {
      ...
      for (auto i = pBaseValue->user_begin(), e = pBaseValue->user_end();
           i != e; ++i) {
        const Value *pUser = *i;
...
VISAScalarModule::GetVariableLocation(const llvm::Instruction *pInst) const {
  ...
  if (pArgument) {
    ...
    IGC::IGCMD::MetaDataUtils::FunctionsInfoMap::iterator itr =
        m_pShader->GetMetaDataUtils()->findFunctionsInfoItem(
            const_cast<Function *>(curFunc));
    ...
    if (itr != m_pShader->GetMetaDataUtils()->end_FunctionsInfo() &&
        modMD->FuncMD.find(const_cast<Function *>(curFunc)) !=
            modMD->FuncMD.end()) {
      unsigned int explicitArgsNum =
          curFunc->arg_size() - itr->second->size_ImplicitArgInfoList();
      if (pArgument->getArgNo() < explicitArgsNum &&
          modMD->FuncMD[const_cast<Function *>(curFunc)]
                  .m_OpenCLArgBaseTypes.size() > pArgument->getArgNo()) {
        const std::string typeStr =
            modMD->FuncMD[const_cast<Function *>(curFunc)]
                .m_OpenCLArgBaseTypes[pArgument->getArgNo()];
        KernelArg::ArgType argType = KernelArg::calcArgType(pArgument, typeStr);
        if (argType == KernelArg::ArgType::SAMPLER) {
          // SAMPLER and NOT_TO_ALLOCATE have same enum values so disambiguate
          // these
        ...
        IGC_ASSERT_MESSAGE(resAllocMD->argAllocMDList.size() ==
                               curFunc->arg_size(),
                           "Invalid ArgAllocMDList");
        ArgAllocMD *argAlloc =
            &resAllocMD->argAllocMDList[pArgument->getArgNo()];
        ...
        case KernelArg::ArgType::IMAGE_2D_MSAA_DEPTH_ARRAY:
          ...
          case UAVResourceType:
            ...
            IGC_ASSERT_MESSAGE(index < TEXTURE_REGISTER_NUM,
                               "Bad texture index");
  ...
  if (isDbgDclInst) {
    if (!pType->isPointerTy()) {
      // TODO: Re-enable this assert once -O2 fixes bug where llvm.dbg.declare
      // points to a non-address value.
      // IGC_ASSERT_MESSAGE(0, "DBG declare intrinsic must point to an
      // address");
      return VISAVariableLocation(this);
    }
  ...
  if (isa<GlobalVariable>(pValue)) {
    ...
    if (isInSurface) {
      return VISAVariableLocation(surfaceReg, offset, false, isDbgDclInst, 0,
                                  false, this);
    }
  ...
  if (!globalSubCVar) {
    pVar =
        m_pShader->GetDebugInfoData().getMapping(*pInst->getFunction(), pValue);
    if (!pVar) {
      return VISAVariableLocation(this);
    }
  } else
  ...
  IGC_ASSERT_MESSAGE(false == pVar->IsImmediate(),
                     "Do not expect an immediate value at this level");
  ...
  case EVARTYPE_GENERAL: {
    ...
    if (pType->isVectorTy()) {
      vectorNumElements =
          (unsigned)cast<IGCLLVM::FixedVectorType>(pType)->getNumElements();
    } else if (!pVar->IsUniform()) {
      vectorNumElements = 1;
    }

    if (isInSurface) {
      return VISAVariableLocation(surfaceReg, reg, true /*isRegister*/,
                                  isDbgDclInst, vectorNumElements,
                                  !pVar->IsUniform(), this);
    }
    VISAVariableLocation genReg(reg, true /*isRegister*/, isDbgDclInst,
                                vectorNumElements, !pVar->IsUniform(),
                                isGlobalAddrSpace, this);
    // SIMD32 locations can't into one register. See
    // VISAVariableLocation::m_locationSecondReg field description for more
    // information
...
bool VISAScalarModule::IsCatchAllIntrinsic(
    const llvm::Instruction *pInst) const {
  return ((isa<GenIntrinsicInst>(pInst) &&
           cast<GenIntrinsicInst>(pInst)->getIntrinsicID() ==
               GenISAIntrinsic::GenISA_CatchAllDebugLine));
}

// OpenCL keyword constant is used as qualifier to variables whose values remain
// the same throughout the program. clang inlines constants in to LLVM IR and no
// metadata is emitted to LLVM IR for such constants. This function iterates
// over all globals and constants to emit metadata per function.
void insertOCLMissingDebugConstMetadata(CodeGenContext *ctx) {
  ...
  for (auto &func : *M) {
    ...
    for (auto global_it = M->global_begin(); global_it != M->global_end();
         global_it++) {
      ...
      if (g->isConstant()) {
        ...
        for (unsigned int j = 0; j < GVs.size(); j++) {
          auto GVExp =
              llvm::dyn_cast_or_null<llvm::DIGlobalVariableExpression>(GVs[j]);
          if (GVExp) {
            auto GV = GVExp->getVariable();
            auto gblNodeScope = GV->getScope();
            if (isa<DISubprogram>(gblNodeScope)) {
              auto subprogramName =
                  cast<DISubprogram>(gblNodeScope)->getName().data();
        ...
        if (!GlobalValue::isLocalLinkage(g->getLinkage()) ||
            isConstForThisFunc) {
          Utils::updateGlobalVarDebugInfo(
              g, init, &func.getEntryBlock().getInstList().front(), false);
        }
      }
    }
  }
...
bool VISAScalarModule::IsIntelSymbolTableVoidProgram() const {
  return IGC::isIntelSymbolTableVoidProgram(
      const_cast<llvm::Function *>(GetEntryFunction()));
}

CVariable *VISAScalarModule::GetSymbol(const llvm::Instruction *pInst,
  ...
  return m_pShader->GetDebugInfoData().getMapping(*pInst->getFunction(),
                                                  pValue);
}

int VISAScalarModule::getDeclarationID(CVariable *pVar,
                                       bool isSecondSimd32Instruction) const {
