...
public:
  ...
  static bool hasAnyDebugInfo(CodeGenContext *ctx, bool &fullDebugInfo,
                              bool &lineNumbersOnly);
};

class VISAScalarModule final : public IGC::VISAModule {
...
public:
  ...
  bool isLineTableOnly() const override { return isLineTableOnly(m_pShader); }
  ...
  llvm::ArrayRef<char> getGenDebug() const override {
    ...
    return llvm::ArrayRef<char>((const char *)PO.m_debugDataGenISA,
                                PO.m_debugDataGenISASize);
  }
  llvm::ArrayRef<char> getGenBinary() const override {
    const auto &PO = *m_pShader->ProgramOutput();
    return llvm::ArrayRef<char>((const char *)PO.m_programBin,
                                PO.m_programSize);
  }
  ...
  static bool isLineTableOnly(CShader *s) {
    ...
    DebugMetadataInfo::hasAnyDebugInfo(s->GetContext(), fullDebugInfo,
                                       lineTableOnly);
  ...
  CVariable *GetSymbol(const llvm::Instruction *pInst,
  ...
  llvm::Instruction *getPerThreadOffset() const { return m_perThreadOffset; }

  bool hasPTO() const override { return getPerThreadOffset() != nullptr; }
  ...
  void setPrivateBase(void *V) override { privateBase = (IGC::CVariable *)V; }
...
private:
  ...
  VISAScalarModule(CShader *TheShader, llvm::Function *TheFunction,
                   bool IsPrimary);
  ...
  /// @param LLVM Argument if the origin value is an argument, nullptr
  /// otherwsie.
  const llvm::Argument *GetTracedArgument(const llvm::Value *pVal,
