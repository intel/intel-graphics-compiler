...
private:
  template <typename SampleOrGather4Intrinsic> bool isSampleLorBAndIsNotHalfType(SampleOrGather4Intrinsic *inst);
  bool isSampleWithHalfType(SampleIntrinsic *sampleInst);
  template <typename SampleOrGather4Intrinsic> bool isSamplingFromCubeSurface(SampleOrGather4Intrinsic *inst);
  bool FixCubeHFPrecisionBug(SampleIntrinsic *sampleInst);
  template <typename SampleOrGather4Intrinsic> bool DoAIParameterCombiningWithLODBias(SampleOrGather4Intrinsic *inst);
...
SamplerPerfOptPass::SamplerPerfOptPass() : FunctionPass(ID) {
  initializeSamplerPerfOptPassPass(*PassRegistry::getPassRegistry());
}

namespace IGC {
llvm::FunctionPass *createSamplerPerfOptPass() { return new SamplerPerfOptPass(); }
} // namespace IGC
...
bool SamplerPerfOptPass::isSampleLorBAndIsNotHalfType(SampleOrGather4Intrinsic *inst) {
  if (inst->getIntrinsicID() == GenISAIntrinsic::GenISA_gather4Bptr ||
...
bool SamplerPerfOptPass::FixCubeHFPrecisionBug(SampleIntrinsic *sampleInst) {
  if (isSampleWithHalfType(sampleInst)) {
    ...
    //    2.    Do the math operation in float, (inv and multiplication) - the part of the cube coords normalization
    //    code
...
bool SamplerPerfOptPass::DoAIParameterCombiningWithLODBias(SampleOrGather4Intrinsic *inst) {
  if (isSampleLorBAndIsNotHalfType(inst)) {
    ...
    Value *lodAi =
        CombineSampleOrGather4Params(builder, inst->getOperand(aiOffset), inst->getOperand(lodOrBiasOffset), numBits,
                                     std::string("AI"), std::string(inst->hasBias() ? "Bias" : "Lod"));
...
bool SamplerPerfOptPass::ConvertToSampleMlod(SampleIntrinsic *inst) {
  ...
  if (isa<Constant>(mlod) && cast<Constant>(mlod)->isNullValue()) {
    return false;
  }

  GenISAIntrinsic::ID id =
      inst->hasRef() ? GenISAIntrinsic::GenISA_sampleCMlodptr : GenISAIntrinsic::GenISA_sampleMlodptr;
  llvm::Type *types[] = {inst->getType(), inst->getOperand(0)->getType(), inst->getPairedTextureValue()->getType(),
                         inst->getTextureValue()->getType(), inst->getSamplerValue()->getType()};
  Function *newFunc = GenISAIntrinsic::getDeclaration(inst->getModule(), id, types);
...
bool SamplerPerfOptPass::ConvertLdToLdl(SamplerLoadIntrinsic *inst) {
  ...
  llvm::Type *types[] = {inst->getType(), inst->getPairedTextureValue()->getType(), inst->getTextureValue()->getType(),
    ...
    for (auto BI = F.begin(), BE = F.end(); BI != BE; BI++) {
      for (auto II = BI->begin(), IE = BI->end(); II != IE; II++) {
       ...
       // 32bit versions of messages no longer have the AI param:
          ...
          if (ctx->platform.hasSampleMlodMessage() && IGC_IS_FLAG_ENABLED(EnablePromotionToSampleMlod) &&
