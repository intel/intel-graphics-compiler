...
public:
  static std::unique_ptr<IGC::VISAModule> BuildNew(CShader *S, llvm::Function *F, bool IsPrimary = false,
                                                   bool IsCallerStackCall = false);

  unsigned int getUnpaddedProgramSize() const override { return m_pShader->ProgramOutput()->m_unpaddedProgramSize; }
  bool isLineTableOnly() const override { return isLineTableOnly(m_pShader); }
  unsigned getPrivateBaseReg() const override;

  unsigned getGRFSizeInBytes() const override { return m_pShader->getGRFSize(); }

  unsigned getNumGRFs() const override { return m_pShader->ProgramOutput()->m_numGRFTotal; }
  ...
  CVariable *GetGlobalCVar(llvm::Value *pValue) { return m_pShader->GetGlobalCVar(pValue); }
  ...
  CVariable *getPerThreadOffset() const { return m_perThreadOffset; }
...
private:
  ...
  ScalarVisaModule(CShader *TheShader, llvm::Function *TheFunction, bool IsPrimary, bool IsStackCallContext);
