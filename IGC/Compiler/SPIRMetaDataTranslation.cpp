...
void SPIRMetaDataTranslation::translateKernelMetadataIntoOpenCLKernelsMD(Module &M) {
  ...
  for (uint i = 0; i < opencl_kernels->getNumOperands(); i++) {
    if (auto pMdNode = opencl_kernels->getOperand(i)) {
      if (pMdNode->getOperand(0)) {
        llvm::Function *opFunc = mdconst::dyn_extract<llvm::Function>(pMdNode->getOperand(0));
  ...
  for (auto &Func : M) {
    if (Functions.find(&Func) == Functions.end() && Func.getCallingConv() == CallingConv::SPIR_KERNEL) {
...
bool SPIRMetaDataTranslation::runOnModule(Module &M) {
  ...
  for (; ki != ke; ++ki) {
    ...
    if (reqdWorkGroupSize->hasValue()) {
      funcMD.threadGroupSize.dim0 = reqdWorkGroupSize->getXDim();
      funcMD.threadGroupSize.dim1 = reqdWorkGroupSize->getYDim();
      funcMD.threadGroupSize.dim2 = reqdWorkGroupSize->getZDim();
    }
  ...
  if (!spirMDUtils.empty_CompilerOptions()) {
    ...
    for (; coi != coe; ++coi) {
      std::string co = *coi;
      // Compiler options that originate from OpenCL/L0 are represented by the same name, without the "-cl"/"-ze"
      // prefixes. L0 supports only "-opt-disable" from the options below
      ...
      case DENORM_ARE_ZERO:
        modMD->compOpt.DenormsAreZero = true;
        break;
      case BF_TF_DENORMS_ARE_ZERO:
        modMD->compOpt.BFTFDenormsAreZero = true;
        break;
      case CORRECTLY_ROUNDED_SQRT:
        modMD->compOpt.CorrectlyRoundedDivSqrt = true;
        break;
      case OPT_DISABLE:
        modMD->compOpt.OptDisable = true;
        break;
      case MAD_ENABLE:
        modMD->compOpt.MadEnable = true;
        break;
      case NO_SIGNED_ZERO:
        modMD->compOpt.NoSignedZeros = true;
        break;
      case UNSAFE_MATH:
        modMD->compOpt.UnsafeMathOptimizations = true;
        break;
      case FINITE_MATH:
        modMD->compOpt.FiniteMathOnly = true;
        break;
      ...
      case RELAXED_BUILTINS:
        ...
        break;
      case DASH_G:
        modMD->compOpt.DashGSpecified = true;
        break;
      case MATCH_SINCOSPI:
        modMD->compOpt.MatchSinCosPi = true;
        break;
      default:
        break;
      }
