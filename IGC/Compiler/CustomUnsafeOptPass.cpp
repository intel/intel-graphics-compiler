...
static bool allowUnsafeMathOpt(CodeGenContext *ctx, llvm::BinaryOperator &op) {
  ...
  if (llvm::isa<llvm::FPMathOperator>(op)) {
    auto FMF = op.getFastMathFlags();
    if (FMF.allowContract() && FMF.allowReassoc() && FMF.allowReciprocal() && FMF.approxFunc() && FMF.noSignedZeros())
      return true;
  }
...
void CustomUnsafeOptPass::strengthReducePowOrExpLog(IntrinsicInst *intrin, Value *base, Value *exponent, bool isPow) {
  ...
  if (exponent == ConstantFP::get(exponent->getType(), 0.5)) {
    // pow(x, 0.5) -> sqrt(x)
    llvm::Function *sqrtIntr = IGCLLVM::getOrInsertDeclaration(m_ctx->getModule(), Intrinsic::sqrt, base->getType());
    ...
    Function *logf = IGCLLVM::getOrInsertDeclaration(m_ctx->getModule(), Intrinsic::log2, base->getType());
    Function *expf = IGCLLVM::getOrInsertDeclaration(m_ctx->getModule(), Intrinsic::exp2, base->getType());
