...
void CustomSafeOptPass::visitShuffleIndex(llvm::CallInst *I) {
  ...
  */
  ...
  auto insertShuffleXor = [](IRBuilder<> &builder, Value *value, uint32_t xorValue) {
    Function *simdShuffleXorFunc = GenISAIntrinsic::getDeclaration(
        builder.GetInsertBlock()->getParent()->getParent(), GenISAIntrinsic::GenISA_simdShuffleXor, value->getType());

    return builder.CreateCall(simdShuffleXorFunc, {value, builder.getInt32(xorValue)}, "simdShuffleXor");
  };
...
Value *CustomSafeOptPass::analyzeTreeForTrunc64bto32b(const Use &OperandUse,
                                                      SmallVector<BinaryOperator *, 8> &OpsToDelete) {
...
void CustomSafeOptPass::visitAllocaInst(AllocaInst &I) {
  ...
  // %27 = getelementptr[356 x float] * %outarray_x1, i32 0, i32 352      %35 = getelementptr[2 x float] * %31, i32 0,
  // i32 0 store float 0.000000e+00, float* %27, align 4                        store float 0.000000e+00, float* %35,
  // align 4 %28 = getelementptr[356 x float] * %outarray_y2, i32 0, i32 352      %36 = getelementptr[2 x float] * %32,
  // i32 0, i32 0 store float 0.000000e+00, float* %28, align 4                        store float 0.000000e+00, float*
  // %36, align 4 %43 = add nsw i32 %selRes_s, 354 %44 = getelementptr[356 x float] * %outarray_x1, i32 0, i32 %43 %51 =
  // getelementptr[2 x float] * %31, i32 0, i32 %selRes_s %45 = load float* %44, align 4 %52 = load float* %51, align 4

  llvm::Type *pType = I.getAllocatedType();
  if (!pType->isArrayTy() || static_cast<ADDRESS_SPACE>(I.getType()->getAddressSpace()) != ADDRESS_SPACE_PRIVATE) {
    return;
  }

  if (!(pType->getArrayElementType()->isFloatingPointTy() || pType->getArrayElementType()->isIntegerTy() ||
      ...
      for (Value::user_iterator use_it = pGEP->user_begin(), use_e = pGEP->user_end(); use_it != use_e; ++use_it) {
        if (llvm::dyn_cast<llvm::LoadInst>(*use_it)) {
          ...
          if (dyn_cast<ConstantInt>(pGEP->getOperand(2)) && pGEP->getOperand(2)->getType()->isIntegerTy(32)) {
            int currentIndex = int_cast<int>(dyn_cast<ConstantInt>(pGEP->getOperand(2))->getZExtValue());
  ...
  for (Value::user_iterator it = I.user_begin(), e = I.user_end(); it != e; ++it) {
    if (GetElementPtrInst *pGEP = llvm::dyn_cast<GetElementPtrInst>(*it)) {
      if (dyn_cast<ConstantInt>(pGEP->getOperand(2))) {
        ...
        int newIndex = int_cast<int>(dyn_cast<ConstantInt>(pGEP->getOperand(2))->getZExtValue()) - index_lb;
  ...
  if (GetElementPtrInst *gep = dyn_cast<GetElementPtrInst>(ptr)) {
    ...
    for (uint i = 0; i < gep->getNumIndices(); ++i) {
      ...
      if (!isa<ConstantInt>(gepIdx)) {
        ...
        if (!found && sel && isa<ConstantInt>(sel->getOperand(1)) && isa<ConstantInt>(sel->getOperand(2))) {
          found = true;
          selIdx = i;
        } else {
...
void CustomSafeOptPass::visitf32tof16(llvm::CallInst *inst) {
  ...
  if (!shiftOrMul || (shiftOrMul->getOpcode() != Instruction::Shl && shiftOrMul->getOpcode() != Instruction::Mul)) {
    return;
  }
  ...
  if (extInstLo && extInstHi && extInstLo->getOperand(0)->getType()->isHalfTy() &&
...
void CustomSafeOptPass::matchDp4a(BinaryOperator &I) {
  ...
  // different.
  auto checkIfValuesComeFromCharType = [](auto &range, OriginSignedness &retSign) {
    ...
    IGC_ASSERT_MESSAGE(range.size() == NUM_DP4A_COMPONENTS, "Range too big in dp4a pattern match!");
    ...
    for (auto I : range) {
      ...
      case Instruction::ZExt:
        ...
        signs[counter] =
            (I->getOpcode() == Instruction::SExt) ? OriginSignedness::originSigned : OriginSignedness::originUnsigned;
        break;
      ...
      case Instruction::LShr:
        ...
        signs[counter] =
            (I->getOpcode() == Instruction::AShr) ? OriginSignedness::originSigned : OriginSignedness::originUnsigned;
        break;
      case Instruction::And:
    ...
    retSign = signs[0];
    return std::all_of(signs.begin(), signs.end(), [&signs](OriginSignedness v) { return v == signs[0]; });
  };

  OriginSignedness ABranch = OriginSignedness::originUnknown, BBranch = OriginSignedness::originUnknown;
  bool canMatch = AccVal->getType()->isIntegerTy(32) && checkIfValuesComeFromCharType(ExtA, ABranch) &&
                  checkIfValuesComeFromCharType(ExtB, BBranch);
  if (!canMatch)
    return;

  GenISAIntrinsic::ID IntrinsicID{};

  static_assert(ExtA.size() == ArrA.size() && ExtB.size() == ArrB.size() && ExtA.size() == NUM_DP4A_COMPONENTS,
                "array sizes must match!");

  auto getInt8Origin = [&](Instruction *I) {
    if (I->getOpcode() == Instruction::SExt || I->getOpcode() == Instruction::ZExt) {
  ...
  case OriginSignedness::originSigned:
    IntrinsicID =
        (BBranch == OriginSignedness::originSigned) ? GenISAIntrinsic::GenISA_dp4a_ss : GenISAIntrinsic::GenISA_dp4a_su;
    break;
  case OriginSignedness::originUnsigned:
    IntrinsicID =
        (BBranch == OriginSignedness::originSigned) ? GenISAIntrinsic::GenISA_dp4a_us : GenISAIntrinsic::GenISA_dp4a_uu;
    break;
  default:
    IGC_ASSERT(0);
  }
...
void CustomSafeOptPass::hoistDp3(BinaryOperator &I) {
  ...
  // dp3
  bool isdp3 =
      match(cast<Instruction>(&I), m_FAdd(m_FMul(m_Value(Z1), m_Value(Z2)),
                                          m_FAdd(m_FMul(m_Value(X1), m_Value(X2)), m_FMul(m_Value(Y1), m_Value(Y2)))));

  // need to try matching for when fadd and fmul are switched
  if (!isdp3) {
    isdp3 =
        match(cast<Instruction>(&I), m_FAdd(m_FAdd(m_FMul(m_Value(X1), m_Value(X2)), m_FMul(m_Value(Y1), m_Value(Y2))),
  ...
  if (I.getOpcode() == Instruction::Or) {
    if (BinaryOperator *OrOp0 = dyn_cast<BinaryOperator>(I.getOperand(0))) {
      if (OrOp0->getOpcode() == Instruction::Shl) {
        ...
        // If we have a constant Mul and a constant Or. The Mul constant needs to be divisible by the rounded up 2^n of
        // Or value.
...
void CustomSafeOptPass::visitBinaryOperator(BinaryOperator &I) {
  ...
  if (I.getType()->isIntegerTy()) {
    if ((I.getOpcode() == Instruction::Add || isEmulatedAdd(I)) && I.hasOneUse()) {
...
void CustomSafeOptPass::visitTruncInst(TruncInst &I) {
  ...
  */
  ...
  if (I.getSrcTy()->isIntegerTy(32) && I.getDestTy()->isIntegerTy(16)) {
    ...
    if (auto *genIntr = dyn_cast<GenIntrinsicInst>(I.getOperand(0));
        genIntr && isSubGroupShuffleVariant(genIntr) && genIntr->hasOneUse()) {
      if (auto *ZI = dyn_cast<ZExtInst>(genIntr->getOperand(0));
          ZI && ZI->getSrcTy()->isIntegerTy(16) && ZI->getDestTy()->isIntegerTy(32)) {
        IRBuilder<> builder(&I);
        ...
        Function *demotedFuncDeclaration =
            GenISAIntrinsic::getDeclaration(I.getModule(), genIntr->getIntrinsicID(), builder.getInt16Ty());
...
void IGC::CustomSafeOptPass::visitLdptr(llvm::SamplerLoadIntrinsic *inst) {
  ...
  // change
  // % 10 = call fast <4 x float> @llvm.genx.GenISA.ldptr.v4f32.p196608v4f32(i32 %_s1.i, i32 %_s14.i, i32 0, i32 0, <4 x
  // float> addrspace(196608)* null, i32 0, i32 0, i32 0), !dbg !123 to % 10 = call fast <4 x float>
  // @llvm.genx.GenISA.typedread.p196608v4f32(<4 x float> addrspace(196608)* null, i32 %_s1.i, i32 %_s14.i, i32 0, i32
  // 0), !dbg !123 when the index comes directly from threadid
  ...
  Function *pLdIntrinsic =
      llvm::GenISAIntrinsic::getDeclaration(M, GenISAIntrinsic::GenISA_typedread, inst->getOperand(4)->getType());
  ...
  llvm::CallInst *pNewCallInst = builder.CreateCall(pLdIntrinsic, ld_FunctionArgList);
  ...
  if (inst->getType() != pNewCallInst->getType()) {
    ...
    IGC_ASSERT_MESSAGE(cast<IGCLLVM::FixedVectorType>(inst->getType())->getElementType()->isIntegerTy(32),
                       "expect int4 here");
    IGC_ASSERT_MESSAGE(cast<IGCLLVM::FixedVectorType>(inst->getType())->getNumElements() == 4, "expect int4 here");
    ...
    if (auto EE = dyn_cast<ExtractElementInst>(inst->user_back())) {
      if (auto constIndex = dyn_cast<ConstantInt>(EE->getIndexOperand())) {
        ...
        llvm::SmallVector<llvm::Value *, 4> new_args{
            inst->getOperand(0), builder.CreateAdd(inst->getOperand(1), builder.getInt32((unsigned)new_offset)),
            inst->getOperand(2), inst->getOperand(3)};

        Function *pLdraw_indexed_intrinsic =
            llvm::GenISAIntrinsic::getDeclaration(inst->getModule(), GenISAIntrinsic::GenISA_ldraw_indexed, ovldtypes);
      ...
      // %16 = shl i32 %13, 1
      // %17 = call <4 x i16> @llvm.genx.GenISA.ldrawvector.indexed.v4i16.p2490368v4f32(<4 x float> addrspace(2490368)*
      // %u0, i32 % 16, i32 2, i1 false) %18 = shufflevector <4 x i16> %17, <4 x i16> undef, <1 x i32> zeroinitializer
      // ====>
      // %15 = shl i32 %12, 1
      // %16 = call <1 x i16> @llvm.genx.GenISA.ldraw.indexed.v1i16.p2490368v4f32(<4 x float> addrspace(2490368) * %u0,
      // i32 % 15, i32 2, i1 false)

      if (SV->getOperand(0) == inst && SV->getShuffleMask().size() == 1 && SV->getShuffleMask().front() == 0) {
        llvm::IRBuilder<> builder(inst);
        ...
        llvm::SmallVector<llvm::Value *, 4> new_args{inst->getOperand(0), inst->getOperand(1), inst->getOperand(2),
                                                     inst->getOperand(3)};
...
void CustomSafeOptPass::dp4WithIdentityMatrix(ExtractElementInst &I) {
  ...
  convert dp4 with identity matrix icb ( ex: "dp4 r[6].x, cb[2][8].xyzw, icb[ r[4].w].xyzw") from
      ...
      %190 = extractelement <20 x float> <float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float
  0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float
  0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00>, i32
  %189, !dbg !326 %191 = or i32 %189, 1, !dbg !326 %192 = extractelement <20 x float> <float 1.000000e+00, float
  0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00,
  float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float
  0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 0.000000e+00, float 0.000000e+00>, i32 %191, !dbg !326 %193 = or i32 %189, 2, !dbg !326 %194 = extractelement
  <20 x float> <float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00>, i32 %193, !dbg
  !326 %195 = or i32 %189, 3, !dbg !326 %196 = extractelement <20 x float> <float 1.000000e+00, float 0.000000e+00,
  float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float
  0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00,
  float 0.000000e+00, float 0.000000e+00, float 1.000000e+00, float 0.000000e+00, float 0.000000e+00, float
  0.000000e+00, float 0.000000e+00>, i32 %195, !dbg !326 %s01_s.chan0141 = fmul fast float %181, %190, !dbg !326 %197 =
  fmul fast float %182, %192, !dbg !326 %198 = fadd fast float %197, %s01_s.chan0141, !dbg !326 %199 = fmul fast float
  %183, %194, !dbg !326 %200 = fadd fast float %199, %198, !dbg !326 %201 = fmul fast float %184, %196, !dbg !326 %202 =
  fadd fast float %201, %200, !dbg !326 to %533 = icmp eq i32 %532, 0, !dbg !434 %534 = icmp eq i32 %532, 1, !dbg !434
  ...
  */
  ...
  if (addI[0] == nullptr || addI[1] == nullptr || addI[2] == nullptr || !isAddInst(addI[0]) || !isAddInst(addI[1]) ||
      !isAddInst(addI[2]) || !addI[0]->hasOneUse() || !addI[1]->hasOneUse() ||
      *addI[0]->user_begin() != *mulI[2]->user_begin() || *addI[1]->user_begin() != *mulI[3]->user_begin()) {
    return;
  }
  ...
  Value *cond0 = builder.CreateICmp(ICmpInst::ICMP_EQ, offset[0]->getOperand(0),
                                    ConstantInt::get(offset[0]->getOperand(0)->getType(), 0));
  Value *cond1 = builder.CreateICmp(ICmpInst::ICMP_EQ, offset[0]->getOperand(0),
                                    ConstantInt::get(offset[0]->getOperand(0)->getType(), 1));
  Value *cond2 = builder.CreateICmp(ICmpInst::ICMP_EQ, offset[0]->getOperand(0),
                                    ConstantInt::get(offset[0]->getOperand(0)->getType(), 2));
  Value *cond3 = builder.CreateICmp(ICmpInst::ICMP_EQ, offset[0]->getOperand(0),
                                    ConstantInt::get(offset[0]->getOperand(0)->getType(), 3));
...
////////////////////////////////////////////////////////////////////////////////
IGC_INITIALIZE_PASS_BEGIN(TrivialUnnecessaryTGMFenceElimination, "TrivialUnnecessaryTGMFenceElimination",
                          "TrivialUnnecessaryTGMFenceElimination", false, false)
IGC_INITIALIZE_PASS_END(TrivialUnnecessaryTGMFenceElimination, "TrivialUnnecessaryTGMFenceElimination",
                        "TrivialUnnecessaryTGMFenceElimination", false, false)
...
void GenSpecificPattern::visitMul(llvm::BinaryOperator &I) {
  ...
  if (ConstOp->isPowerOf2()) {
    Value *Shl = builder.CreateShl(ValOp, (uint64_t)ConstOp->exactLogBase2(), "",
  ...
  Value *Shl = builder.CreateShl(ValOp, (uint64_t)ConstOpAbs.exactLogBase2(), "",
                                 /*bool NUW=*/HasNUW, /*bool NSW=*/HasNSW);
...
*/
template <typename MaskType> void CustomSafeOptPass::matchReverse(BinaryOperator &I) {
  using namespace llvm::PatternMatch;
  ...
  auto patternBfrevFirst = m_Or(m_Shl(m_Value(nextOrShl), m_ConstantInt(currentShiftShl)),
                                m_LShr(m_Value(nextOrShr), m_ConstantInt(currentShiftShr)));

  auto patternBfrev =
      m_Or(m_And(m_Shl(m_Value(nextOrShl), m_ConstantInt(currentShiftShl)), m_ConstantInt(currentMaskShl)),
           m_And(m_LShr(m_Value(nextOrShr), m_ConstantInt(currentShiftShr)), m_ConstantInt(currentMaskShr)));
  ...
  nextOrShl = &I;
  if (match(nextOrShl, patternBfrevFirst) && nextOrShl == nextOrShr && currentShiftShl == currentShift &&
  ...
  while (currentShift > 0) {
    if (match(nextOrShl, patternBfrev) && nextOrShl == nextOrShr && currentShiftShl == currentShift &&
        currentShiftShr == currentShift && currentMaskShl == mask && currentMaskShr == (MaskType)~mask) {
      // NextOrShl is assigned to next one by match().
      if (currentShift == 1) {
  ...
  if (isBfrevMatchFound) {
    llvm::IRBuilder<> builder(&I);
    Function *bfrevFunc = GenISAIntrinsic::getDeclaration(I.getParent()->getParent()->getParent(),
                                                          GenISAIntrinsic::GenISA_bfrev, builder.getInt32Ty());
    if (bitWidth == 16) {
 ...
 */
void GenSpecificPattern::createBitcastExtractInsertPattern(BinaryOperator &I, Value *OpLow, Value *OpHi,
                                                           unsigned extractNum1, unsigned extractNum2) {
  ...
  auto zeroextorNot = [&](Value *Op, unsigned num) {
    ...
    if (auto ZextInst = dyn_cast<ZExtInst>(Op)) {
      ...
      if (opType->isVectorTy() && cast<VectorType>(opType)->getElementType()->isIntegerTy(32) &&
          cast<IGCLLVM::FixedVectorType>(opType)->getNumElements() == 2) {
...
void GenSpecificPattern::createAddcIntrinsicPattern(Instruction &ZEI, Value *val1, Value *val2, Value *val3,
                                                    Instruction &addInst) {
  ...
  Function *addcIntrinsic =
      llvm::GenISAIntrinsic::getDeclaration(ZEI.getModule(), GenISAIntrinsic::GenISA_uaddc, types);
  ...
  if (match(&I, fabs_on_int_pattern1)) {
    ...
    */
    ...
    if (match(&I, pattern_And_0xFF) && I.getType()->isIntegerTy(32) && AndSrc && AndSrc->getType()->isIntegerTy(32)) {
      ...
      bool LShrMatch = match(AndSrc, LShr_Pattern) && LhsSrc && LhsSrc->getType()->isIntegerTy(32) && CI &&
                       (CI->getZExtValue() % 8 == 0);
...
void GenSpecificPattern::visitOr(BinaryOperator &I) {
  ...
  */
  ...
  auto pattern1 = m_Or(m_And(m_Value(AndOp1), m_SpecificInt(0xFFFFFFFF)), m_Shl(m_Value(EltOp1), m_SpecificInt(32)));
  Value *AndOp2 = nullptr, *EltOp2 = nullptr, *VecOp = nullptr;
  auto pattern2 = m_Or(m_And(m_Value(AndOp2), m_SpecificInt(0xFFFFFFFF)),
  ...
  if (match(&I, pattern1) && AndOp1 && AndOp1->getType()->isIntegerTy(64)) {
    ...
    if (cVec) {
      ...
      if (vector_type && isa<ConstantInt>(cVec->getOperand(0)) && cast<ConstantInt>(cVec->getOperand(0))->isZero() &&
          vector_type->getElementType()->isIntegerTy(32) && vector_type->getNumElements() == 2) {
    ...
    */
    ...
    if (OrConstant) {
      ...
      if (ShlInst && ShlInst->getOpcode() == Instruction::Shl) {
        ...
        if (ShlConstant) {
          // if the constant bit width is larger than 64, we cannot store ShlIntValue and OrIntValue rawdata as
          // uint64_t. will need a fix then
...
void GenSpecificPattern::visitSelectInst(SelectInst &I) {
  ...
  */
  ...
  if (Cint && Cint->isZero()) {
    if (I.getType()->isIntegerTy(1)) {
      ...
      if (cmpInst && cmpInst->getOpcode() == Instruction::ICmp && I.getOperand(1) != cmpInst->getOperand(0)) {
    ...
    if (Cfp && Cfp->isZero()) {
      llvm::Instruction *cmpInst = llvm::dyn_cast<llvm::Instruction>(I.getOperand(0));
      if (cmpInst && cmpInst->getOpcode() == Instruction::FCmp && I.getOperand(1) != cmpInst->getOperand(0)) {
        // disable the cases for csel or where we can optimize the instructions to such as add.ge.* later in vISA
  ...
  */
  if (auto *compInst = dyn_cast<ICmpInst>(I.getOperand(0))) {
    ...
    if (selOp1->getType()->isIntegerTy() && icmpType->isIntegerTy() &&
        selOp1->getType()->getIntegerBitWidth() < icmpType->getIntegerBitWidth()) {
      Value *newSelOp1 = NULL;
      Value *newSelOp2 = NULL;
      if (trunc1 && (trunc1->getOperand(0) == cmpOp0 || trunc1->getOperand(0) == cmpOp1)) {
        newSelOp1 = (trunc1->getOperand(0) == cmpOp0) ? cmpOp0 : cmpOp1;
      }

      if (trunc2 && (trunc2->getOperand(0) == cmpOp0 || trunc2->getOperand(0) == cmpOp1)) {
        newSelOp2 = (trunc2->getOperand(0) == cmpOp0) ? cmpOp0 : cmpOp1;
      }

      if (isa<llvm::ConstantInt>(selOp1) && isa<llvm::ConstantInt>(cmpOp0) &&
          (cast<llvm::ConstantInt>(selOp1)->getZExtValue() == cast<llvm::ConstantInt>(cmpOp0)->getZExtValue())) {
        IGC_ASSERT(newSelOp1 == NULL);
        newSelOp1 = cmpOp0;
      }

      if (isa<llvm::ConstantInt>(selOp1) && isa<llvm::ConstantInt>(cmpOp1) &&
          (cast<llvm::ConstantInt>(selOp1)->getZExtValue() == cast<llvm::ConstantInt>(cmpOp1)->getZExtValue())) {
        IGC_ASSERT(newSelOp1 == NULL);
        newSelOp1 = cmpOp1;
      }

      if (isa<llvm::ConstantInt>(selOp2) && isa<llvm::ConstantInt>(cmpOp0) &&
          (cast<llvm::ConstantInt>(selOp2)->getZExtValue() == cast<llvm::ConstantInt>(cmpOp0)->getZExtValue())) {
        IGC_ASSERT(newSelOp2 == NULL);
        newSelOp2 = cmpOp0;
      }

      if (isa<llvm::ConstantInt>(selOp2) && isa<llvm::ConstantInt>(cmpOp1) &&
          (cast<llvm::ConstantInt>(selOp2)->getZExtValue() == cast<llvm::ConstantInt>(cmpOp1)->getZExtValue())) {
        IGC_ASSERT(newSelOp2 == NULL);
        newSelOp2 = cmpOp1;
      }
  ...
  if (I.getType()->isDoubleTy() && I.getOperand(0)->getType()->isIntegerTy(64)) {
    ...
    if ((binOperator = dyn_cast<BinaryOperator>(I.getOperand(0))) && binOperator->getOpcode() == Instruction::Or) {
      if (isa<BitCastInst>(binOperator->getOperand(0)) && isa<ZExtInst>(binOperator->getOperand(1))) {
        ...
        if (zExtInst->getOperand(0)->getType()->isIntegerTy(32) && isa<InsertElementInst>(bitCastInst->getOperand(0)) &&
          ...
          if (isa<Constant>(insertElementInst->getOperand(0)) &&
            ...
            vectorValue =
                builder.CreateInsertElement(vectorValue, insertElementInst->getOperand(1), builder.getInt32(1));
...
/*
    ...
    %6 = call %union._XReq addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)*
   addrspace(1)* addrspace(1)* @llvm.genx.GenISA.pair.to.ptr.p1p1p1p1p1p1p1p1union._XReq(i32 %4, i32 %5) %7 = bitcast
   %union._XReq addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)* addrspace(1)*
   addrspace(1)* %6 to i64 addrspace(1)* %8 = bitcast i64 addrspace(1)* %7 to <2 x i32> addrspace(1)* %9 = load <2 x
   i32>, <2 x i32> addrspace(1)* %8, align 64
...
void GenSpecificPattern::visitLoadInst(LoadInst &LI) {
  ...
  bool IsPairToPtrInst = (isa<GenIntrinsicInst>(PO) &&
                          cast<GenIntrinsicInst>(PO)->getIntrinsicID() == GenISAIntrinsic::GenISA_pair_to_ptr);
  ...
  bool PointerComesFromALoad =
      (Op0 && Op1 && isa<ConstantInt>(Op0->getIndexOperand()) && isa<ConstantInt>(Op1->getIndexOperand()) &&
       cast<ConstantInt>(Op0->getIndexOperand())->getZExtValue() == 0 &&
       cast<ConstantInt>(Op1->getIndexOperand())->getZExtValue() == 1 && isa<LoadInst>(Op0->getVectorOperand()) &&
       isa<LoadInst>(Op1->getVectorOperand()) && Op0->getVectorOperand() == Op1->getVectorOperand());
...
void GenSpecificPattern::visitTruncInst(llvm::TruncInst &I) {
  ...
  */
  ...
  using namespace llvm::PatternMatch;
  ConstantInt *CI = nullptr;
  if (match(&I, m_Trunc(m_LShr(m_Value(LHS), m_ConstantInt(CI)))) && I.getType()->isIntegerTy(32) &&
      LHS->getType()->isIntegerTy(64) && CI->getZExtValue() >= 32) {
  ...
  */
  if (match(&I, m_Trunc(m_PtrToInt(m_IntToPtr(m_Value(Src)))))) {
    ...
    if (!I2P)
      return;
    if (DL.getPointerSizeInBits(I2P->getType()->getPointerAddressSpace()) < I2P->getSrcTy()->getIntegerBitWidth())
      return;
    if (DL.getPointerSizeInBits(P2I->getSrcTy()->getPointerAddressSpace()) != P2I->getType()->getIntegerBitWidth())
      return;
...
static cl::opt<bool> overrideEnableSimplifyGEP("override-enable-simplify-gep", cl::init(false), cl::Hidden,
...
IGCConstProp::IGCConstProp(bool enableSimplifyGEP)
    : FunctionPass(ID), m_enableSimplifyGEP(enableSimplifyGEP), m_TD(nullptr), m_TLI(nullptr) {
      ...
      if (modMD->immConstant.data.size() &&
        ...
        if (inst->getType()->isVectorTy()) {
          ...
          for (uint i = 0; i < srcNElts; i++) {
            ...
            vectorValue =
                builder.CreateInsertElement(vectorValue, GetConstantValue(srcEltTy, pEltValue), builder.getInt32(i));
          }
  ...
  if (inst) {
    ...
    case llvm_canonicalize: {
      ...
      if (C0) {
        ...
        flushVal = flushVal ||
                   (compOpt.FloatDenormMode32 == ::IGC::FLOAT_DENORM_FLUSH_TO_ZERO && inst->getType()->isFloatTy());
        flushVal = flushVal ||
                   (compOpt.FloatDenormMode64 == ::IGC::FLOAT_DENORM_FLUSH_TO_ZERO && inst->getType()->isDoubleTy());
  ...
  if (EEI && isa<Constant>(EEI->getVectorOperand())) {
    ...
    for (unsigned i = 0; i < N; ++i) {
      ...
      if (isa<UndefValue>(Opnd))
        continue;
      Constant *Result = ConstantFoldCompareInstOperands(CI->getPredicate(), Opnd, cast<Constant>(RHS),
                                                         CI->getFunction()->getParent()->getDataLayout());
  ...
  if (isLHSAdd && isRHSAdd) {
    ...
    if (ConstantInt *CLHS = dyn_cast<ConstantInt>(LHS)) {
      if (ConstantInt *CRHS = dyn_cast<ConstantInt>(RHS)) {
        Value *newC = ConstantFoldBinaryOpOperands(Instruction::Add, CLHS, CRHS, *m_TD);
...
public:
  static char ID;
  ClampICBOOBAccess() : FunctionPass(ID) { initializeClampICBOOBAccessPass(*PassRegistry::getPassRegistry()); }
      ...
      if (llvm::LoadInst *inst = llvm::dyn_cast<llvm::LoadInst>(&(*BI++))) {
        ...
        if (isICBOffseted(inst, modMD->pushInfo.inlineConstantBufferOffset, offsetIntoMergedBuffer)) {
          ...
          if (GetElementPtrInst *gep = dyn_cast<GetElementPtrInst>(ptrVal)) {
            ...
            Value *isOOB =
                m_builder.CreateICmp(ICmpInst::ICMP_UGE, eltIdx, llvm::ConstantInt::get(eltIdx->getType(), arrSize));
            unsigned zeroIndex = modMD->immConstant.zeroIdxs[offsetIntoMergedBuffer];
            Value *eltIdxClampped =
                m_builder.CreateSelect(isOOB, llvm::ConstantInt::get(eltIdx->getType(), zeroIndex), eltIdx);
        ...
        if (llvm::LoadInst *inst = llvm::dyn_cast<llvm::LoadInst>(&(*BI++))) {
          ...
          bool bICBNoOffset =
              (IGC::INVALID_CONSTANT_BUFFER_INVALID_ADDR == modMD->pushInfo.inlineConstantBufferOffset &&
               bufType == CONSTANT_BUFFER && directBuf && bufId == modMD->pushInfo.inlineConstantBufferSlot);
          bool bICBOffseted =
              (IGC::INVALID_CONSTANT_BUFFER_INVALID_ADDR != modMD->pushInfo.inlineConstantBufferOffset &&
               ADDRESS_SPACE_CONSTANT == as &&
               isICBOffseted(inst, modMD->pushInfo.inlineConstantBufferOffset, offsetIntoMergedBuffer));
          if (bICBNoOffset || bICBOffseted) {
            ...
            if (size_in_bytes) {
              uint maxImmConstantSizePushed = !modMD->immConstant.sizes.empty()
                                                  ? modMD->immConstant.sizes[offsetIntoMergedBuffer]
                                                  : modMD->immConstant.data.size();
...
IGC_INITIALIZE_PASS_BEGIN(IGCIndirectICBPropagaion, "IGCIndirectICBPropagaion", "IGCIndirectICBPropagaion", false,
                          false)
...
public:
  ...
  VectorBitCastOpt() : FunctionPass(ID) { initializeVectorBitCastOptPass(*PassRegistry::getPassRegistry()); };
  ~VectorBitCastOpt() {}

  virtual llvm::StringRef getPassName() const override { return "VectorBitCastOptPass"; }
...
IGC_INITIALIZE_PASS_BEGIN(VectorBitCastOpt, VECTOR_BITCAST_OPT_PASS_FLAG, VECTOR_BITCAST_OPT_PASS_DESCRIPTION,
                          VECTOR_BITCAST_OPT_PASS_CFG_ONLY, VECTOR_BITCAST_OPT_PASS_ANALYSIS)
IGC_INITIALIZE_PASS_END(VectorBitCastOpt, VECTOR_BITCAST_OPT_PASS_FLAG, VECTOR_BITCAST_OPT_PASS_DESCRIPTION,
                        VECTOR_BITCAST_OPT_PASS_CFG_ONLY, VECTOR_BITCAST_OPT_PASS_ANALYSIS)
...
public:
  static char ID;
  GenStrengthReduction() : FunctionPass(ID) { initializeGenStrengthReductionPass(*PassRegistry::getPassRegistry()); }
...
bool GenStrengthReduction::processInst(Instruction *Inst) {
  ...
  if (opc == Instruction::Select) {
    ...
    if (oprd1 == oprd2 || (CF1 && CF2 && CF1->isExactlyValue(CF2->getValueAPF()))) {
      Inst->replaceAllUsesWith(oprd1);
      Inst->eraseFromParent();
      return true;
    }
  }
  if (Op && Op->hasNoNaNs() && Op->hasNoInfs() && Op->hasNoSignedZeros()) {
    switch (opc) {
  ...
  if (isCandidateFDiv(Inst)) {
    ...
    for (auto UI = Src1->user_begin(), UE = Src1->user_end(); UI != UE; ++UI) {
      Value *Val = *UI;
      Instruction *I = dyn_cast<Instruction>(Val);
      if (I && I != Inst && I->getOpcode() == Instruction::FDiv && I->getOperand(1) == Src1 && isCandidateFDiv(I)) {
        // special case
...
IGC_INITIALIZE_PASS_BEGIN(GenStrengthReduction, "GenStrengthReduction", "GenStrengthReduction", false, false)
IGC_INITIALIZE_PASS_END(GenStrengthReduction, "GenStrengthReduction", "GenStrengthReduction", false, false)
...
public:
  static char ID;
  FlattenSmallSwitch() : FunctionPass(ID) { initializeFlattenSmallSwitchPass(*PassRegistry::getPassRegistry()); }
...
bool FlattenSmallSwitch::processSwitchInst(SwitchInst *SI) {
  ...
  for (auto *Phi : PhiNodes) {
    Value *vTemp = Phi->getIncomingValueForBlock(DefaultMergeBlock ? SI->getParent() : Default);
  ...
  */
  {
    ...
    if (Cfp1 && Cfp1->getValueAPF().isFiniteNonZero() && Cfp2 && Cfp2->isZero()) {
      isSelWithConstants = true;
    }
    if (Cfp1 && Cfp1->isZero() && Cfp2 && Cfp2->getValueAPF().isFiniteNonZero()) {
      isSelWithConstants = true;
      swapNodesFromSel = true;
    }
    if (isSelWithConstants && dyn_cast<FCmpInst>(I.getOperand(0))) {
      for (auto bitCastI : I.users()) {
        if (BitCastInst *bitcastInst = dyn_cast<BitCastInst>(bitCastI)) {
          for (auto cmpI : bitcastInst->users()) {
            ...
            if (iCmpInst && iCmpInst->isEquality()) {
...
public:
  static char ID;
  SplitIndirectEEtoSel() : FunctionPass(ID) { initializeSplitIndirectEEtoSelPass(*PassRegistry::getPassRegistry()); }
...
protected:
  ...
  bool isSafeToConvert(Instruction *cond0, Instruction *cond1, smallvector<Instruction *, 8> &moveInsts);

  void convertAndToBranch(Instruction *opAnd, Instruction *cond0, Instruction *cond1, BasicBlock *&newBB);
};
} // namespace
...
bool LogicalAndToBranch::scheduleUp(BasicBlock *bb, Value *V, Instruction *&insertPos) {
...
//       ...
void LogicalAndToBranch::convertAndToBranch(Instruction *opAnd, Instruction *cond0, Instruction *cond1,
                                            BasicBlock *&newBB) {
...
bool LogicalAndToBranch::isSafeToConvert(Instruction *cond0, Instruction *cond1,
...
bool LogicalAndToBranch::runOnFunction(Function &F) {
  ...
  for (auto BI = F.begin(), BE = F.end(); BI != BE;) {
    ...
    for (auto II = bb->begin(), IE = bb->end(); II != IE;) {
      ...
      // search for "and i1"
      if (inst->getOpcode() == BinaryOperator::And && inst->getType()->isIntegerTy(1)) {
        Instruction *s0 = dyn_cast<Instruction>(inst->getOperand(0));
        Instruction *s1 = dyn_cast<Instruction>(inst->getOperand(1));
        if (s0 && s1 && !isa<PHINode>(s0) && !isa<PHINode>(s1) && s0->hasOneUse() && s1->hasOneUse() &&
          ...
          auto distance = std::count_if(is0, is1, [](const auto &i) { return !isDbgIntrinsic(&i); });
...
FunctionPass *IGC::createCleanPHINodePass() { return new CleanPHINode(); }

CleanPHINode::CleanPHINode() : FunctionPass(ID) { initializeCleanPHINodePass(*PassRegistry::getPassRegistry()); }
...
ex: Change
    Label-1337.i:                                     ; preds = %Label-1339.i, %Label-1780.i
      ...
      call void @llvm.genx.GenISA.typedwrite.p2490368__2D_DIM_Resource(%__2D_DIM_Resource addrspace(2490368)* %494, i32
%17, i32 %18, i32 0, i32 0, float %491, float %492, float %493, float %Temp-2360.i163) br label %Output Label-1313.i: ;
preds = %Label-1577.i %495 = inttoptr i32 %0 to %__2D_DIM_Resource addrspace(2490368)* call void
@llvm.genx.GenISA.typedwrite.p2490368__2D_DIM_Resource(%__2D_DIM_Resource addrspace(2490368)* %495, i32 %17, i32 %18,
i32 0, i32 0, float %scalar39, float %scalar40, float %scalar41, float %scalar42) br label %Output Output: ; preds =
%Label-1313.i, %Label-1337.i ret void to Output:                                           ; preds = %Label-1313.i,
%Label-1337.i %494 = phi float [ %scalar39, %Label-1313.i ], [ %491, %Label-1337.i ] %495 = phi float [ %scalar40,
%Label-1313.i ], [ %492, %Label-1337.i ] %496 = phi float [ %scalar41, %Label-1313.i ], [ %493, %Label-1337.i ] %497 =
phi float [ %scalar42, %Label-1313.i ], [ %Temp-2360.i163, %Label-1337.i ] %498 = inttoptr i32 %0 to %__2D_DIM_Resource
addrspace(2490368)* call void @llvm.genx.GenISA.typedwrite.p2490368__2D_DIM_Resource(%__2D_DIM_Resource
addrspace(2490368)* %498, i32 %17, i32 %18, i32 0, i32 0, float %494, float %495, float %496, float %497) ret void
    }
*/

char MergeMemFromBranchOpt::ID = 0;
FunctionPass *IGC::createMergeMemFromBranchOptPass() { return new MergeMemFromBranchOpt(); }
          ...
          if (br && br->getPrevNode()) {
            if (llvm::GenIntrinsicInst *giInst = llvm::dyn_cast<GenIntrinsicInst>(br->getPrevNode())) {
              if (giInst->getIntrinsicID() == GenISAIntrinsic::GenISA_typedwrite ||
                  giInst->getIntrinsicID() == GenISAIntrinsic::GenISA_typedwriteMS) {
                IntToPtrInst *itp0 = dyn_cast<IntToPtrInst>(inst->getOperand(0));
                IntToPtrInst *itp1 = dyn_cast<IntToPtrInst>(giInst->getOperand(0));

                if (itp0 && itp1 &&
                    (itp0->getType()->getPointerAddressSpace() == itp1->getType()->getPointerAddressSpace()) &&
                    (itp0->getOperand(0) == itp1->getOperand(0)) && (giInst->getOperand(1) == inst->getOperand(1)) &&
                    (giInst->getOperand(2) == inst->getOperand(2)) && (giInst->getOperand(3) == inst->getOperand(3)) &&
...
public:
  ...
  virtual StringRef getPassName() const override { return IGCOpts::SinkLoadOptPass; }
...
FunctionPass *IGC::createSinkLoadOptPass() { return new SinkLoadOpt(); }
...
bool SinkLoadOpt::sinkLoadInstruction(Function &F) {
  ...
  for (auto &BB : F) {
    for (auto BI = BB.begin(), BE = BB.end(); BI != BE; ++BI) {
      ...
      if (CI && (CI->getIntrinsicID() == GenISAIntrinsic::GenISA_ldraw_indexed ||
...
FunctionPass *IGC::createInsertBranchOptPass() { return new InsertBranchOpt(); }
...
bool InsertBranchOpt::HasSrcFromEE(Instruction *I, uint selNum, Instruction *&loadInst) {
  ...
  while (itemID != traceBackInst.size()) {
    ...
    if (ExtractElementInst *eetemp = dyn_cast<ExtractElementInst>(tbInst)) {
      ...
      if (load->getParent() == I->getParent() && !load->hasNUsesOrMore(1 + selNum) && eetemp->hasOneUse()) {
        loadInst = load;
        return true;
      }
    } else {
...
void InsertBranchOpt::findOptCases(SelectInst *I) {
  ...
  if (lg.selInst[1][0]) {
    ...
    if (lg.dclInst0->getIntrinsicID() != GenISAIntrinsic::GenISA_DCL_inputVec ||
        ...
        !dyn_cast<Constant>(lg.cmpInst0->getOperand(1)) || !dyn_cast<Constant>(lg.cmpInst1->getOperand(1)))
      return;
    ...
    for (uint i = 0; i < numSel; i++) {
      ...
      if (temp0 && temp1 && temp2 && HasSrcFromEE(temp0, numSel, load0) && HasSrcFromEE(temp1, numSel, load1) &&
          HasSrcFromEE(temp2, numSel, load2)) {
...
void InsertBranchOpt::CreateBranchBlock(Function &F, Value *cmpI, loadGroup &lg, Instruction *inst[], int groupID) {
  ...
  for (uint i = 0; i < lg.num; i++) {
    ...
    if (groupID == 0)
      lg.newPhi[groupID][i]->addIncoming(inst[i]->getType()->isFloatingPointTy() ? zeroF : zeroI,
                                         termatorInst->getParent()->getSinglePredecessor());
    else
...
void InsertBranchOpt::atomicSplitOpt(Function &F, int mode) {
  ...
  auto createReadFromAtomic = [=](IRBuilder<> &builder, Instruction *inst, bool isTyped) {
    ...
    if (isTyped) {
      Function *pLdIntrinsic = llvm::GenISAIntrinsic::getDeclaration(
          inst->getModule(), GenISAIntrinsic::GenISA_typedread, inst->getOperand(0)->getType());
    ...
    // Stateless atomic
      ...
      // Generate send.smpl ld_lz instruction
        ...
        pLdIntrinsic = GenISAIntrinsic::getDeclaration(inst->getModule(), GenISAIntrinsic::GenISA_ldptr, types);
  ...
  auto splitBBAndName = [](Instruction *condInst, Instruction *inst, Instruction **ThenTerm, Instruction **ElseTerm,
...
bool isRematerializable(Value *V, const Instruction *dest, const DominatorTree &DT) {
...
/// does not match.
static bool getUniqueNonZeroPhiValue(PHINode *phi, Value *&nonZeroVal, BasicBlock *&nonZeroBB) {
  nonZeroVal = nullptr;
...
static Value *rematerializeChainIterative(Value *rootVal, Instruction *insertBefore, const DominatorTree &DT) {
...
void typedWriteZeroStoreCheck(Function &F, DominatorTree &DT, PostDominatorTree &PDT) {
  for (auto &BB : F) {
      ...
      if (GenIntrinsicInst *tyWrite = dyn_cast<GenIntrinsicInst>(&I)) {
        ...
        if (tyWrite->getIntrinsicID() == GenISAIntrinsic::GenISA_typedwrite ||
          ...
          for (uint32_t i = 0; i < numActiveChannels; i++) {
            ...
            faddInst[i] = dyn_cast<Instruction>(tyWrite->getOperand(i + paramCnt));
            ...
            if (!fmulInst[i] || fmulInst[i]->getOpcode() != Instruction::FMul || fmulInst[i]->getNumUses() > 1) {
              checksAreGreen = false;
              break;
            }
            ...
            if (!isa<ConstantInt>(eeInst[i]->getIndexOperand()) ||
                cast<ConstantInt>(eeInst[i]->getIndexOperand())->getZExtValue() != i) {
              checksAreGreen = false;
              break;
            }
            ...
            if (!tyRead || (tyR != nullptr && tyR != tyRead) || tyRead->getNumUses() > numArgs ||
                (tyWrite->getIntrinsicID() == GenISAIntrinsic::GenISA_typedwrite &&
                 tyRead->getIntrinsicID() != GenISAIntrinsic::GenISA_typedread) ||
                (tyWrite->getIntrinsicID() == GenISAIntrinsic::GenISA_typedwriteMS &&
                 tyRead->getIntrinsicID() != GenISAIntrinsic::GenISA_typedreadMS)) {
              checksAreGreen = false;
              break;
            }
            tyR = tyRead;
          ...
          for (uint32_t i = numActiveChannels; i < numArgs && checksAreGreen; i++) {
            bool found = false;
          ...
          for (uint32_t i = 0; i < numActiveChannels && !breakOut; i++) {
            ...
            if (PHINode *p = dyn_cast<PHINode>(fmul->getOperand(1))) {
              breakOut = !getUniqueNonZeroPhiValue(p, incomingNonZeroValues[i], fromBB);
            } else {
              breakOut = true;
              break;
            }
          ...
          for (int i = 0; i < paramCnt && !breakOut; i++) {
            breakOut = tyR->getOperand(i) != tyW->getOperand(i) ||
                       !isRematerializable(tyR->getOperand(i), fromBB->getTerminator(), DT);
          }
          ...
          for (int i = 0; i < paramCnt; i++) {
            rematerializedVals[i] = rematerializeChainIterative(tyR->getOperand(i), fromBB->getTerminator(), DT);
...
bool InsertBranchOpt::runOnFunction(Function &F) {
  pContext = getAnalysis<CodeGenContextWrapper>().getCodeGenContext();
  int mode = IGC_IS_FLAG_ENABLED(EnableAtomicBranch) ? IGC_GET_FLAG_VALUE(EnableAtomicBranch)
  ...
  if (IGC_IS_FLAG_DISABLED(DisableTypedWriteZeroStoreCheck)) {
    typedWriteZeroStoreCheck(F, getAnalysis<DominatorTreeWrapperPass>().getDomTree(),
...
bool CanonicalizeMulAdd::isSupported(Instruction &I) {
