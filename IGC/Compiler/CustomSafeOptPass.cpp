...
MergeMemFromBranchOpt::MergeMemFromBranchOpt() : FunctionPass(ID)
{
...
void MergeMemFromBranchOpt::visitTypedWrite(llvm::CallInst* inst)
{
    ...
    if (dyn_cast<BranchInst>(inst->getNextNode()))
    {
        ...
        if (BasicBlock* succBB = BB->getSingleSuccessor())
        {
            ...
            {
                BB2 = *PI;
                if (BB2 != BB && BB2->getInstList().size() > 1)
                {
                    BranchInst* br = dyn_cast<BranchInst>(&BB2->getInstList().back());
                    if (br && br->getPrevNode())
                    {
                        if (llvm::GenIntrinsicInst* giInst = llvm::dyn_cast<GenIntrinsicInst>(br->getPrevNode()))
                        {
                            ...
                            {
                                if (((giInst->getOperand(0))->getType()->getPointerAddressSpace() ==
            ...
            if (succBB->hasNPredecessorsOrMore(callSet.size() + 1))
                return;
            ...
            if (inst->getOperand(0) && inst->getOperand(0)->hasOneUse())
            {
                if (Instruction* itp = dyn_cast<Instruction>(inst->getOperand(0)))
                {
                    itp->removeFromParent();
                    itp->insertAfter(p8);
                }
            }
