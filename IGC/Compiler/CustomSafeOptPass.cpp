...
void IGC::CustomSafeOptPass::visitLdRawVec(llvm::CallInst* inst)
{
    ...
    if (inst->hasOneUse())
    {
        // allow a bitcast in between the ldrawvectorand the extractElement.
        ExtractElementInst* EE = dyn_cast<ExtractElementInst>(inst->user_back());
        BitCastInst* bitCastInst = dyn_cast<BitCastInst>(inst->user_back());
        if (!EE)
        {
            if (bitCastInst && bitCastInst->hasOneUse())
            {
                EE = dyn_cast<ExtractElementInst>(bitCastInst->user_back());
            }
            if (!EE)
            {
                return;
        ...
        // make sure the inst, bitCastInst, and EE are next to each other.
        // so that the new offset calculation can be right above the ldraw and
        // no need to worry about def before use for the offset calcuation.
        if (bitCastInst)
        {
            bitCastInst->moveBefore(EE);
            inst->moveBefore(bitCastInst);
        }
        else
        {
            inst->moveBefore(EE);
        ...
        llvm::Value* new_offset = builder.CreateMul(EE->getIndexOperand(),
            builder.getInt32((uint32_t)(DL.getTypeAllocSize(EE->getType()))));

        llvm::SmallVector<llvm::Value*, 4> new_args{
            inst->getOperand(0),
            builder.CreateAdd(inst->getOperand(1), new_offset),
            inst->getOperand(2),
            inst->getOperand(3)
        };
