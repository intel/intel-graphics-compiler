    ...
    for (auto BI = F.begin(), BE = F.end(); BI != BE; ++BI)
    {
        for (auto II = BI->begin(); II != BI->end(); ++II) {
            ...
            if (intrinsicInst && intrinsicInst->getIntrinsicID() == Intrinsic::pow) {
                ...
                IntrinsicInst* maxInst = nullptr;
                Instruction* fsubInst = nullptr;
                if (minInst == dyn_cast<IntrinsicInst>(intrinsicInst->getOperand(0))) {
                    if (maxInst == dyn_cast<IntrinsicInst>(minInst->getOperand(0))) {
                        auto minInstID = minInst->getIntrinsicID();
                        auto maxInstID = maxInst->getIntrinsicID();
                        fsubInst = dyn_cast<Instruction>(maxInst->getOperand(0));
                        if (minInstID == Intrinsic::minnum && maxInstID == Intrinsic::maxnum &&
