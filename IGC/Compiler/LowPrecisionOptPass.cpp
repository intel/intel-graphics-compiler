        ...
        */
        ...
        if (m_func_llvm_GenISA_DCL_inputVec_f32 == nullptr) {
          m_func_llvm_GenISA_DCL_inputVec_f32 = llvm::GenISAIntrinsic::getDeclaration(
              m_currFunction->getParent(), ID, Type::getFloatTy(m_builder->getContext()));
        }

        m_builder->SetInsertPoint(callInst);
        Value *v = m_builder->CreateCall2(m_func_llvm_GenISA_DCL_inputVec_f32, callInst->getOperand(0),
...
void LowPrecisionOpt::visitFPTruncInst(llvm::FPTruncInst &I) {
  ...
  if (cInst && cInst->getIntrinsicID() == GenISAIntrinsic::GenISA_RuntimeValue) {
    if (!IGC_IS_FLAG_ENABLED(HoistPSConstBufferValues) || shdrType != ShaderType::PIXEL_SHADER)
      return;
    moveBundle bundle;
...
bool LowPrecisionOpt::propagateSamplerType(llvm::GenIntrinsicInst &I) {
  ...
  if (eltTy->isFloatingPointTy()) {
    ...
    newDstType = isFloatType ? m_builder->getHalfTy() : m_builder->getFloatTy();
  ...
  } else if (eltTy == m_builder->getInt32Ty() && IGC_IS_FLAG_ENABLED(DownConvertI32Sampler)) {
  ...
  Function *newSample = GenISAIntrinsic::getDeclaration(m_currFunction->getParent(), I.getIntrinsicID(), overloadTys);
...
void LowPrecisionOpt::visitIntrinsicInst(llvm::IntrinsicInst &I) {
  ...
  if (I.getIntrinsicID() != llvm::Intrinsic::floor || I.getType() != Type::getHalfTy(m_builder->getContext()))
    return;
