...
void Subgroup2DBlockIoResolution::visitCallInst(CallInst &CI) {
  ...
  m_SimdSize = IGC::getSIMDSize(getAnalysis<MetaDataUtilsWrapper>().getMetaDataUtils(), CI.getParent()->getParent());
  if (Pattern2DBlockSPV.match(FuncName, &Matches)) {
    visit2DBlockCallInst(CI, Matches, true);
  } else if (Pattern2DBlockOCL.match(FuncName, &Matches)) {
    if (m_SimdSize != 16)
...
void Subgroup2DBlockIoResolution::visit2DBlockCallInst(CallInst &CI, const SmallVector<StringRef, 3> &Matches,
  ...
  if (IntrinsicID == GenISAIntrinsic::GenISA_LSC2DBlockRead) {
    auto *StoreDataPtr =
        Builder.CreatePointerCast(MemoryDesc.PrivateMem, NewInst->getType()->getPointerTo(), "storeDataPtr");
...
SmallVector<Value *, 14> Subgroup2DBlockIoResolution::getArgs(CallInst &CI, const MemoryDesc &MemDesc,
  ...
  if (OpType == Op::Write) {
    ...
    auto *StoreDataPtr = Builder.CreatePointerCast(MemDesc.PrivateMem, LoadedType->getPointerTo(), "storeDataPtr");
