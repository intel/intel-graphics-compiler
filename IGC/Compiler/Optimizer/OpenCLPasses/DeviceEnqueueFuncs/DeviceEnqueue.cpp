...
void DeviceEnqueueFuncsAnalysis::visitCallInst(CallInst &CI) {
  ...
  if (funcName == GET_DEFAULT_DEVICE_QUEUE) {
    ...
    IGC_ASSERT_MESSAGE(IGCLLVM::getNumArgOperands(&CI) == 1,
                       "get_object_id function is expected to have only one argument");
    ...
    // Extract the arg num and add it to the appropriate data structure
    IGC_ASSERT_MESSAGE(IGCLLVM::getNumArgOperands(&CI) == 1,
                       "get_block_simd_size function is expected to have only one argument");
...
void DeviceEnqueueFuncsResolution::visitCallInst(CallInst &CI) {
  ...
  if (funcName == GET_DEFAULT_DEVICE_QUEUE) {
    ...
    enqueueResource = m_implicitArgs.getNumberedImplicitArg(
        F, ImplicitArg::GET_OBJECT_ID, (int)(cast<ConstantInt>(CI.getArgOperand(0))->getZExtValue()));
    IGC_ASSERT(enqueueResource != NULL);
  } else if (funcName == GET_BLOCK_SIMD_SIZE) {
    enqueueResource = m_implicitArgs.getNumberedImplicitArg(
        F, ImplicitArg::GET_BLOCK_SIMD_SIZE, (int)(cast<ConstantInt>(CI.getArgOperand(0))->getZExtValue()));
    IGC_ASSERT(enqueueResource != NULL);
  }
