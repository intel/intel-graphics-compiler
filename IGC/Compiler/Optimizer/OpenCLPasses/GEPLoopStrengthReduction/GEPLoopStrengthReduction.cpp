...
{
public:
    Reducer(const DataLayout &DL, DominatorTree &DT, Loop &L, LoopInfo &LI, RegisterPressureEstimate &RPE, ScalarEvolution &SE, unsigned MaxPressure, bool AllowLICM)
        : DL(DL), DT(DT), L(L), LI(LI), RPE(RPE), SE(SE), E(SE, DL, "gep-loop-strength-reduction"), MaxPressure(MaxPressure), AllowLICM(AllowLICM) {}
...
// Updates group's base to candidate with smallest SCEV expression. Used to
// select cheapest GEP to reduce to loop's preheader.
void ReductionCandidateGroup::selectBestBase()
{
    if (Others.empty())
        return;

    // Iterate other candidates in group and find the cheapest one.
    auto BestCandidate = Others.begin();
    for (auto It = Others.begin() + 1; It != Others.end(); ++It)
    {
        if (It->S->getExpressionSize() < BestCandidate->S->getExpressionSize())
            BestCandidate = It;
    }

    // Compare with base.
    if (BestCandidate->S->getExpressionSize() < Base.S->getExpressionSize())
    {
        // Move previous base to the rest of the group.
        Others.push_back(Base);

        // Update previous candidates with new delta.
        auto Delta = BestCandidate->Delta;
        for (auto &o : Others)
            o.Delta -= Delta;

        Base = { BestCandidate->GEP, BestCandidate->S, 0 };
        Others.erase(BestCandidate);
    }
}
...
uint32_t ReductionCandidateGroup::getPressure()
{
    ...
    return Base.S->getExpressionSize();
}
...
void Reducer::filterCandidates()
{
    ...
    // When pressure threshold is reached, reduce only index calculation for
    // GEPs grouped under the same base address. Possible only if group has
    ...
    // Drop remaining candidates, as there is nothing to reduce.
...
bool GEPLoopStrengthReduction::runOnFunction(llvm::Function &F)
{
    ...
    {
        changed |= Reducer(DL, DT, *L, LI, RPE, SE, MaxPressure, AllowLICM).reduce();
    }
