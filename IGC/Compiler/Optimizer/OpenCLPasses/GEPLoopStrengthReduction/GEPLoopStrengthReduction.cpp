...
// Sets group's base GEP to provided candidate. Moves previous base
...
void ReductionCandidateGroup::swapBase(ReductionCandidate &C)
{
    ...
    auto *NewBase = std::find(Others.begin(), Others.end(), C);
    IGC_ASSERT(NewBase != Others.end());

    // Move previous base at the end of the group.
    append(Base);

    // Update candidates with new delta.
    auto Delta = NewBase->Delta;
    for (auto &O : Others)
        O.Delta -= Delta;

    Base = *NewBase;
    Others.erase(NewBase);

    IGC_ASSERT(Base.Delta == 0);
}
