...
private:
  ...
  // Collects the format string and value args (up to its specifier count) into
  // m_argDescriptors.
  void preprocessPrintfArgs(llvm::CallInst &printfCall);

  // Classifies, lowers and records a value argument.
  void appendPrintfValueArg(llvm::CallInst &printfCall, llvm::Value *arg);

  // As appendPrintfValueArg for the format string; a dynamic OCL format is kept
  // inline. Returns the globals it resolved to (for the specifier count).
  llvm::SmallVector<llvm::GlobalVariable *, 2> appendPrintfFormatArg(llvm::CallInst &printfCall, llvm::Value *arg);

  // Records the descriptor for an already-lowered value.
  void addArgDescriptor(llvm::Value *value, IGC::SHADER_PRINTF_TYPE argDataType, llvm::Type *originalType,
                        bool inlineFmtString);

  // Format specifier count, or UINT_MAX when not a single constant literal.
  unsigned formatStringSpecifierCount(const llvm::SmallVectorImpl<llvm::GlobalVariable *> &formatGlobals);
