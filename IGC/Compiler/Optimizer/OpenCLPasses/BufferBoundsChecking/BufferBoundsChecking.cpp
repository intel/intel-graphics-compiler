...
void BufferBoundsChecking::visitLoadInst(LoadInst &load) { loadsAndStoresToCheck.push_back(&load); }

void BufferBoundsChecking::visitStoreInst(StoreInst &store) { loadsAndStoresToCheck.push_back(&store); }
...
std::tuple<Value *, Value *, Value *> BufferBoundsChecking::getLocalIds(Function &function) {
  auto getOrCreateBuiltin = [&function](const char *name) {
    return function.getParent()->getOrInsertFunction(name,
                                                     FunctionType::get(Type::getInt32Ty(function.getContext()), false));
  };
...
std::tuple<Value *, Value *, Value *> BufferBoundsChecking::getGlobalIds(Function &function) {
  auto builtin = function.getParent()->getOrInsertFunction(
      ...
      FunctionType::get(Type::getInt32Ty(function.getContext()), {Type::getInt32Ty(function.getContext())}, false));
...
bool BufferBoundsChecking::argumentQualifiesForChecking(const Argument *argument) {
  ...
  return pointerType && (pointerType->getPointerAddressSpace() == ADDRESS_SPACE_CONSTANT ||
...
void BufferBoundsChecking::createAssertCall(const AccessInfo &accessInfo, Instruction *insertBefore) {
  ...
  auto assert = cast<Function>(
      M->getOrInsertFunction(compileUnit ? "__bufferoutofbounds_assert" : "__bufferoutofbounds_assert_nodebug",
...
SmallVector<Value *, 4> BufferBoundsChecking::createAssertArgs(const AccessInfo &accessInfo,
...
Value *BufferBoundsChecking::createBufferSizePlaceholder(uint32_t implicitArgBufferSizeIndex,
                                                         Instruction *insertBefore) {
  if (!bufferSizePlaceholderFunction) {
    auto M = insertBefore->getModule();
    bufferSizePlaceholderFunction =
        cast<Function>(M->getOrInsertFunction(BufferBoundsCheckingPatcher::BUFFER_SIZE_PLACEHOLDER_FUNCTION_NAME,
