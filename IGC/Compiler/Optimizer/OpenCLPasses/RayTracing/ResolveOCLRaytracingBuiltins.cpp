...
static Value *castToRTGlobalsTy(RTBuilder &RTB, Value *V) {
  ...
  auto GlobalPtrTy = RTB.getRayDispatchGlobalDataPtrTy(*M, ADDRESS_SPACE_GLOBAL);
...
bool ResolveOCLRaytracingBuiltins::runOnModule(Module &M) {
  ...
  // For now just loop through all the kernels (that we have metadata for) and assume that any one of them may be the
  // caller. Marking them all as having ray tracing calls seems to be not harmful.
  // TODO: In future implementation go back up the call graph from the regular function with hasSyncRTCalls until you
  // find the parent kernel.
...
*/
void ResolveOCLRaytracingBuiltins::handleGetThreadBTDStack(CallInst &callInst) { handleGetBTDStack(callInst, false); }
...
global_extra = rt_dispatch_globals->rtMemBasePtr +
                 ...
                 DSS_COUNT;
*/
void ResolveOCLRaytracingBuiltins::handleGetGlobalBTDStack(CallInst &callInst) { handleGetBTDStack(callInst, true); }
...
void ResolveOCLRaytracingBuiltins::handleInitRayQuery(llvm::CallInst &callInst) {
  ...
  auto storeToAlloca = [&](unsigned argIndex) {
    auto ptr = m_builder->CreateGEP(alloca->getAllocatedType(), alloca,
                                    {m_builder->getInt32(0), m_builder->getInt32(argIndex)});
