...
PrivateMemoryToSLM::PrivateMemoryToSLM(bool enableOptReport /* = false */)
    : ModulePass(ID), m_ForceAll(true), m_EnableOptReport(enableOptReport) {
  initializePrivateMemoryToSLMPass(*PassRegistry::getPassRegistry());
}

PrivateMemoryToSLM::PrivateMemoryToSLM(std::string forcedBuffers, bool enableOptReport)
    : ModulePass(ID), m_ForceAll(false), m_EnableOptReport(enableOptReport) {
  ...
  if (pos < forcedBuffers.length())
    m_ForcedBuffers.push_back(forcedBuffers.substr(pos));
...
bool PrivateMemoryToSLM::runOnModule(Module &M) {
  ...
  for (Module::iterator I = M.begin(); I != M.end(); ++I) {
    ...
    for (auto pAI : allocaInsts) {
      bool isForcedBuffer =
          std::find(m_ForcedBuffers.begin(), m_ForcedBuffers.end(), pAI->getName()) != m_ForcedBuffers.end();
      ...
      if (m_ForceAll || isForcedBuffer) {
        ...
        if (newOffset > slmSizePerSubslice) {
          if (m_EnableOptReport) {
            std::stringstream report;
            report << "Skip moving a memory allocation " << pAI->getName().str() << " of " << allocSize << " bytes"
        ...
        if (m_EnableOptReport) {
          std::stringstream report;
          report << "Moving a memory allocation " << pAI->getName().str() << " of " << allocSize << " bytes"
        ...
        auto slmVar = new GlobalVariable(M, newType,
                                         /* isConstant */ false, GlobalValue::ExternalLinkage, UndefValue::get(newType),
                                         F->getName() + "." + pAI->getName(),
                                         /* InsertBefore */ nullptr, GlobalVariable::ThreadLocalMode::NotThreadLocal,
                                         ADDRESS_SPACE_LOCAL);
        ...
        if (CodeGenCtx->type == ShaderType::OPENCL_SHADER) {
          localIdX = ZExtInst::CreateIntegerCast(
              implicitArgs.getImplicitArgValue(*F, ImplicitArg::LOCAL_ID_X, CodeGenCtx->getModuleMetaData()), typeInt32,
              false, VALUE_NAME("localIdX"), IGCLLVM::insertPosition(pEntryPoint));
          localIdY = ZExtInst::CreateIntegerCast(
              implicitArgs.getImplicitArgValue(*F, ImplicitArg::LOCAL_ID_Y, CodeGenCtx->getModuleMetaData()), typeInt32,
              false, VALUE_NAME("localIdY"), IGCLLVM::insertPosition(pEntryPoint));
          localIdZ = ZExtInst::CreateIntegerCast(
              implicitArgs.getImplicitArgValue(*F, ImplicitArg::LOCAL_ID_Z, CodeGenCtx->getModuleMetaData()), typeInt32,
              false, VALUE_NAME("localIdZ"), IGCLLVM::insertPosition(pEntryPoint));
        }

        Value *xOffset = localIdX;
        Value *yOffset = builder.CreateMul(dimX, localIdY, VALUE_NAME(pAI->getName() + ".yOffset"));
        Value *zOffset = builder.CreateMul(dimXY, localIdZ, VALUE_NAME(pAI->getName() + ".zOffset"));

        Value *totalOffset = builder.CreateAdd(xOffset, builder.CreateAdd(yOffset, zOffset),
        ...
        localOffset.m_Offset =
            useAsPointerOnly(slmVar) ? (offset & LOW_BITS_MASK) : ((offset & LOW_BITS_MASK) | VALID_LOCAL_HIGH_BITS);
