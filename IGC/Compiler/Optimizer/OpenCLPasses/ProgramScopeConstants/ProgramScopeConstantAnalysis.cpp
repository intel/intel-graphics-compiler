...
bool ProgramScopeConstantAnalysis::runOnModule(Module& M)
{
    ...
    for (Module::global_iterator I = M.global_begin(), E = M.global_end(); I != E; ++I)
    {
        ...
        if (!globalVar->hasInitializer())
        {
            ...
            DataVector* inlineProgramScopeBuffer = getOrAllocateGlobalBuffer();
            inlineProgramScopeOffsets[globalVar] = inlineProgramScopeBuffer->size();
            // Fill the placeholder entry with zeros.
            const unsigned zeroSize = int_cast<unsigned>(m_DL->getTypeAllocSize(globalVar->getType()));
