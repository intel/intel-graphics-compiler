...
///    Note: Read instructions for the fence are considered only if any of their usages are not post-dominated by the fence. This reduces
///    the numbers of instructions which can influence determination of the obligation of the fence.
...
private:
    ...
    ////////////////////////////////////////////////////////////////////////
    void FilterVisibleMemoryInstructions(
        const llvm::Instruction* pSourceInst,
        bool forwardDirection,
        std::vector<const llvm::Instruction*>& memoryInstructions) const;
    ...
    std::vector<const llvm::Instruction*>& memoryInstructions) const
{
    ...
    FilterVisibleMemoryInstructions(pSourceInst, forwardDirection, memoryInstructions);
}

////////////////////////////////////////////////////////////////////////
/// @brief Filters such read instruction which are used before the fence
/// instruction out of visible memory instruction set.
/// @param pSourceInst the source synchronization instruction
/// @param forwardDirection the direction of searching
/// @param memoryInstructionMask determines which memory instruction can be collected
/// @param output the output collection of reachable memory instructions
void SynchronizationObjectCoalescingAnalysis::FilterVisibleMemoryInstructions(
    const llvm::Instruction* pSourceInst,
    bool forwardDirection,
    std::vector<const llvm::Instruction*>& memoryInstructions) const
{
    if (!forwardDirection && IsFenceOperation(pSourceInst))
    {
        llvm::PostDominatorTree& PDT = getAnalysis<llvm::PostDominatorTreeWrapperPass>().getPostDomTree();
        const llvm::BasicBlock* pSourceBB = pSourceInst->getParent();
        std::vector<const llvm::Instruction*> filteredMemoryInstructions;
        for (const llvm::Instruction* pInst : memoryInstructions)
        {
            InstructionMask instructionBit = GetInstructionMask(pInst);
            if ((instructionBit & sc_MemoryReadInstructionMask) != 0 &&
                instructionBit != InstructionMask::AtomicOperation)
            {
                bool sourcePostDominatesReadInstruction = pInst->getNumUses() == 0;
                for (const llvm::User* pUser : pInst->users())
                {
                    const llvm::BasicBlock* pUserBB = llvm::cast<llvm::Instruction>(pUser)->getParent();
                    bool sharesBB = pUserBB == pSourceBB;
                    if (sharesBB)
                    {
                        for (auto it = llvm::cast<llvm::Instruction>(pUser)->getIterator(); it != pUserBB->end(); it++)
                        {
                            const llvm::Instruction& currentInst = *it;
                            if (&currentInst == pSourceInst)
                            {
                                sourcePostDominatesReadInstruction = true;
                                break;
                            }
                        }
                    }
                    ...
                    {
                        sourcePostDominatesReadInstruction = PDT.dominates(pSourceBB, pUserBB);
                    }

                    if (sourcePostDominatesReadInstruction)
                    {
                        break;
                    }
                }

                // if a user of the read instruction is post-dominated by the fence instruction, such a read instruction can be removed from the memory instruction set
                // because the read instruction is already synchronized by one of its user.
                if (sourcePostDominatesReadInstruction)
                {
                    continue;
                }
            }
            filteredMemoryInstructions.push_back(pInst);
        }
        if (filteredMemoryInstructions.size() != memoryInstructions.size())
        {
            memoryInstructions.assign(filteredMemoryInstructions.begin(), filteredMemoryInstructions.end());
        }
    }
}
