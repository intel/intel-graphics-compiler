...
struct ExplanationEntry {
  ////////////////////////////////////////////////////////////////////////
  enum Cause { GlobalMemoryRedundancy, L1CacheInvalidationRedundancy, StrictRedundancy, FastStrictRedundancy };
...
constexpr InstructionMask AllInstructionsMask = InstructionMask{((InstructionMask::LastMaskPlusOne - 1) << 1) - 1};

constexpr InstructionMask AllNoAtomicMask = InstructionMask{AllInstructionsMask & ~InstructionMask::AtomicOperation};
...
///    - (only for barriers) any atomic instruction, this synchronization instruction, any atomic instruction (RAW, WAW
///    or WAR).
///    * in this context read and write instructions contribute to the same type of memory.
///    ** [Only for the strict redundancy case] If a fence neighbors with barriers forwardly (there is no path between
///    this fence and a barrier which
///      crosses a substitute of this fence), the area of searching is increased to next synchronization area which can
///      be synchronized (there is a substitute of this fence). This area can be depicted in this parenthesis: this
///      fence -> ((barrier -> substitute*** -> barrier***))
///    *** this elements can be missing in the shader
///    **** dependent fences means that such fences neighbor with barriers forwardly (there is no path between these
///    fences and a barrier which
///      crosses a substitute of these fences).
/// 2) [a partial redundancy] removing the global memory association with untyped memory fences (for platforms with
/// dependent SLM fences)
...
///      load float 5.000000e-01, float addrspace(3)* %load_address, align 4
///      call void @llvm.genx.GenISA.memoryfence(i1 true, i1 false, i1 false, i1 false, i1 false, i1 false, i1 false) ;
///      an unneeded instruction call void @llvm.genx.GenISA.threadgroupbarrier() ; an unneeded instruction load
///      float 5.000000e-01, float addrspace(3)* %load_address1, align 4
/// e) an unneeded thread group barrier (write-barrier-no read/write):
///      store float 5.000000e-01, float addrspace(3)* %store_address, align 4
///      call void @llvm.genx.GenISA.memoryfence(i1 true, i1 false, i1 false, i1 false, i1 false, i1 false, i1 false) ;
///      an unneeded instruction call void @llvm.genx.GenISA.threadgroupbarrier() ; an unneeded instruction
/// f) an unneeded thread group barrier (no read/write-barrier-write):
///      call void @llvm.genx.GenISA.memoryfence(i1 true, i1 false, i1 false, i1 false, i1 false, i1 false, i1 false) ;
///      an unneeded instruction call void @llvm.genx.GenISA.threadgroupbarrier() ; an unneeded instruction store
///      float 5.000000e-01, float addrspace(3)* %store_address1, align 4
...
/// 1) synchronization area - this is such an area where all threads in the same workgroup can be run independently.
/// However the start point and
///    end point is the same for the bunch of threads. Such synchronization area can be drawn by an entry point, thread
///    group barriers and end of thread instructions.
/// 2) unsynchronized instruction - this is such a memory instruction which can finish after an imaginatively removed
/// synchronization point. Read
///    instructions must be finished before the synchronization point or first usage. Writes instructions are only
///    synchronized with the first encountered synchronization point. The missing synchronization point for such an
///    instruction can be destructive for the expected order of memory operations.
/// 3) boundary instruction - this is such a synchronization instruction which draws a border for searching.
/// 4) substitute instruction - this is such a synchronization instruction which work at least so extensively as the
/// reference instruction. It
///    means that the substitute instruction can replace the reference instruction without any regressive functional
///    changes in the shader.
...
private:
  static constexpr InstructionMask sc_MemoryWriteInstructionMask = static_cast<InstructionMask>(
      AtomicOperation | TypedWriteOperation | UrbWriteOperation | BufferWriteOperation | SharedMemoryWriteOperation);

  static constexpr InstructionMask sc_MemoryReadInstructionMask = static_cast<InstructionMask>(
      AtomicOperation | TypedReadOperation | OutputUrbReadOperation | BufferReadOperation | SharedMemoryReadOperation);

  static constexpr SynchronizationCaseMask sc_FullSynchronizationCaseMask =
  ...
  ////////////////////////////////////////////////////////////////////////
  void GetVisibleMemoryInstructions(const llvm::Instruction *pSourceInst, bool forwardDirection,
                                    std::vector<const llvm::Instruction *> &boundaryInstructions,
                                    std::vector<const llvm::Instruction *> &memoryInstructions) const;

  ////////////////////////////////////////////////////////////////////////
  void GetAllUnsynchronizedMemoryInstructions(const llvm::Instruction *pSourceInst,
                                              std::vector<const llvm::Instruction *> &threadGroupBarriers,
                                              std::vector<const llvm::Instruction *> &memoryInstructions) const;

  ////////////////////////////////////////////////////////////////////////
  InstructionMask GetInstructionMask(const llvm::Instruction *pSourceInst, bool forwardDirection) const;

  ////////////////////////////////////////////////////////////////////////
  InstructionMask GetAtomicInstructionMaskFromPointer(const llvm::Instruction *pSourceInst) const;

  ////////////////////////////////////////////////////////////////////////
  InstructionMask GetUnsynchronizedForwardInstructionMask(const llvm::Instruction *pSourceInst) const;
  ...
  ////////////////////////////////////////////////////////////////////////
  InstructionMask GetInstructionMask(const std::vector<const llvm::Instruction *> &input) const;

  ////////////////////////////////////////////////////////////////////////
  InstructionMask GetDefaultWriteMemoryInstructionMask(const llvm::Instruction *pSourceInst) const;
  ...
  ////////////////////////////////////////////////////////////////////////
  SynchronizationCaseMask GetSynchronizationMask(InstructionMask localForwardMemoryInstructionMask,
                                                 InstructionMask localBackwardMemoryInstructionMask,
                                                 InstructionMask readBit, InstructionMask writeBit) const;

  ////////////////////////////////////////////////////////////////////////
  SynchronizationCaseMask GetStrictSynchronizationMask(llvm::Instruction *pInst) const;

  ////////////////////////////////////////////////////////////////////////
  SynchronizationCaseMask GetL1CacheInvalidatioSynchronizationMask() const;

  ////////////////////////////////////////////////////////////////////////
  SynchronizationCaseMask
  GetSynchronizationMaskForAllResources(InstructionMask localForwardMemoryInstructionMask,
...
inline constexpr SynchronizationObjectCoalescing::SynchronizationCaseMask
operator|(SynchronizationObjectCoalescing::SynchronizationCaseMask a,
          SynchronizationObjectCoalescing::SynchronizationCaseMask b) {
...
////////////////////////////////////////////////////////////////////////////
SynchronizationObjectCoalescing::SynchronizationObjectCoalescing() : llvm::FunctionPass(ID) {
...
bool SynchronizationObjectCoalescing::ProcessFunction() {
  ...
  bool isDisabled = IsStage1FastestCompile(pContext->m_CgFlag, pContext->m_StagingCtx) ||
...
void SynchronizationObjectCoalescing::EraseRedundantInst(llvm::Instruction *pInst) {
  ...
  llvm::DenseMap<llvm::BasicBlock *, std::map<uint32_t, llvm::Instruction *>> &container =
      isFence ? m_OrderedFenceInstructionsInBasicBlockCache : m_OrderedBarrierInstructionsInBasicBlockCache;
...
void SynchronizationObjectCoalescing::EraseRedundantL1CacheInvalidation(llvm::Instruction *pInst) {
  ...
  case llvm::GenISAIntrinsic::GenISA_memoryfence: {
    ...
    pGenIntrinsicInst->setOperand(
        L1CacheInvalidateArg,
        llvm::ConstantInt::getFalse(pGenIntrinsicInst->getOperand(L1CacheInvalidateArg)->getType()));
    break;
  }
  ...
  case llvm::GenISAIntrinsic::GenISA_LSCFence: {
    ...
    pGenIntrinsicInst->setOperand(fenceOpArg, llvm::ConstantInt::get(type, LSC_FENCE_OP_NONE));
    break;
  }
  default:
    IGC_ASSERT(0);
...
void SynchronizationObjectCoalescing::EraseRedundantGlobalScope(llvm::Instruction *pInst) {
  ...
  case llvm::GenISAIntrinsic::GenISA_memoryfence: {
    ...
    pGenIntrinsicInst->setOperand(
        globalMemFenceArg, llvm::ConstantInt::getFalse(pGenIntrinsicInst->getOperand(globalMemFenceArg)->getType()));
    constexpr uint32_t scopeMemFenceArg = 8;
    pGenIntrinsicInst->setOperand(scopeMemFenceArg,
                                  llvm::ConstantInt::get(pGenIntrinsicInst->getOperand(scopeMemFenceArg)->getType(),
                                                         static_cast<uint32_t>(LSC_SCOPE::LSC_SCOPE_GROUP)));
    break;
  }
  case llvm::GenISAIntrinsic::GenISA_LSCFence: {
    constexpr uint32_t globalMemFenceArg = 0;
    pGenIntrinsicInst->setOperand(globalMemFenceArg,
                                  llvm::ConstantInt::get(pGenIntrinsicInst->getOperand(globalMemFenceArg)->getType(),
                                                         static_cast<uint32_t>(LSC_SFID::LSC_SLM)));
    constexpr uint32_t scopeMemFenceArg = 1;
    pGenIntrinsicInst->setOperand(scopeMemFenceArg,
                                  llvm::ConstantInt::get(pGenIntrinsicInst->getOperand(scopeMemFenceArg)->getType(),
                                                         static_cast<uint32_t>(LSC_SCOPE::LSC_SCOPE_GROUP)));
    constexpr uint32_t fenceOpArg = 2;
    pGenIntrinsicInst->setOperand(fenceOpArg,
                                  llvm::ConstantInt::get(pGenIntrinsicInst->getOperand(fenceOpArg)->getType(),
                                                         static_cast<uint32_t>(LSC_FENCE_OP_NONE)));
    break;
  }
  default:
    IGC_ASSERT(0);
...
bool SynchronizationObjectCoalescing::FindRedundancies() {
  ...
  auto GatherRedundancies = [this, &isModified](std::vector<llvm::Instruction *> &synchronizationOperations) {
    ...
    for (llvm::Instruction *pInst : synchronizationOperations) {
      ...
      if (IsTypedMemoryFenceOperation(pInst) || IsTypedMemoryLscFenceOperation(pInst)) {
        instType |= SyncInstMask::TypedMemoryFence;
      }
      if (IsUntypedMemoryFenceOperationForSharedMemoryAccess(pInst) || IsSharedLscFenceOperation(pInst)) {
        instType |= SyncInstMask::SharedMemoryFence;
      }
      if (IsUrbFenceOperation(pInst) || IsUrbLscFenceOperation(pInst)) {
      ...
      bool isDisabled =
          instType != SyncInstMask::None && (IGC_GET_FLAG_VALUE(DisableCoalescingSynchronizationObjectMask) &
                                             static_cast<DWORD>(instType)) == static_cast<DWORD>(instType);
      if (isDisabled) {
        continue;
      }
      ...
      auto SetLocalMemoryInstructionMask = [this, pInst, forwardDirection, backwardDirection,
        ...
        localForwardMemoryInstructionMask =
            GetDefaultMemoryInstructionMask(pInst) & GetInstructionMask(pInst, forwardDirection);
        localBackwardMemoryInstructionMask =
            GetDefaultMemoryInstructionMask(pInst) & GetInstructionMask(pInst, backwardDirection);
      };
      ...
      if (IsUntypedMemoryFenceOperationForGlobalAccess(pInst) ||
        ...
        if (IsUntypedMemoryFenceOperationWithInvalidationFunctionality(pInst)) {
          ...
          if (!m_HasTypedMemoryFenceFunctionality) {
            syncCaseMask |=
                GetSynchronizationMask(localForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
                                       TypedReadOperation, TypedWriteOperation);
          }
        ...
        if (!m_HasIndependentSharedMemoryFenceFunctionality) {
          ...
          if (verifyUnsynchronizedInstructions) {
            ...
            SynchronizationCaseMask syncCaseMaskForUnsynchronizedInstructions =
                GetSynchronizationMask(unsynchronizedForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
                                       BufferReadOperation, BufferWriteOperation);
            if (!m_HasTypedMemoryFenceFunctionality) {
              syncCaseMaskForUnsynchronizedInstructions |=
                  GetSynchronizationMask(unsynchronizedForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
                                         TypedReadOperation, TypedWriteOperation);
            }
            if (!m_HasUrbFenceFunctionality) {
      ...
      if (IsLscFenceOperation(pInst)) {
        ...
        if (op == LSC_FENCE_OP_INVALIDATE && (mem == LSC_UGM || mem == LSC_TGM)) {
          SynchronizationCaseMask syncCaseMask =
              GetSynchronizationMask(localForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
      ...
      /// - (only for barriers or dependent fences) any read instruction, this synchronization instruction, any write
      /// instruction,
      ...
      if (verifyUnsynchronizedInstructions) {
        InstructionMask unsynchronizedForwardMemoryInstructionMask = GetUnsynchronizedForwardInstructionMask(pInst);
        SynchronizationCaseMask syncCaseMaskForUnsynchronizedInstructions = GetSynchronizationMaskForAllResources(
            unsynchronizedForwardMemoryInstructionMask, localBackwardMemoryInstructionMask);
        syncCaseMask = syncCaseMaskForUnsynchronizedInstructions;
        isObligatory = (syncCaseMask & referenceSyncCaseMask) != 0;
      }
...
SynchronizationObjectCoalescing::GetDefaultWriteMemoryInstructionMask(const llvm::Instruction *pSourceInst) const {
  ...
  if (IsUntypedMemoryFenceOperation(pSourceInst)) {
    ...
    if (hasSharedMemoryInfluence) {
      result = static_cast<InstructionMask>(result | SharedMemoryWriteOperation);
      if (m_ShaderType == ShaderType::HULL_SHADER && !m_HasUrbFenceFunctionality &&
          m_HasIndependentSharedMemoryFenceFunctionality) {
    ...
    if (hasGlobalInfluence) {
      result = static_cast<InstructionMask>(result | BufferWriteOperation);
      if (!m_HasTypedMemoryFenceFunctionality) {
...
/// graph searching.
InstructionMask
SynchronizationObjectCoalescing::GetDefaultMemoryInstructionMask(const llvm::Instruction *pSourceInst) const {
  // All the rules stems from assumptions in the main comment of this analysis (the paragraph about a strict
  // redundancy).
  InstructionMask result = InstructionMask::None;
  if (IsUntypedMemoryFenceOperation(pSourceInst)) {
    const bool hasSharedMemoryInfluence = IsUntypedMemoryFenceOperationForSharedMemoryAccess(pSourceInst);
    ...
    if (hasSharedMemoryInfluence) {
      result = static_cast<InstructionMask>(result | SharedMemoryWriteOperation | SharedMemoryReadOperation);
      if (m_ShaderType == ShaderType::HULL_SHADER && !m_HasUrbFenceFunctionality &&
          m_HasIndependentSharedMemoryFenceFunctionality) {
        // This is for ICL+ but should not harm on pre-ICL
        result = static_cast<InstructionMask>(result | UrbWriteOperation | OutputUrbReadOperation);
      }
    }

    if (hasGlobalInfluence) {
      result = static_cast<InstructionMask>(result | BufferWriteOperation | BufferReadOperation);
      if (!m_HasTypedMemoryFenceFunctionality) {
    ...
    result = static_cast<InstructionMask>(result | OutputUrbReadOperation | UrbWriteOperation | AtomicOperation);
  } else if (IsThreadBarrierOperation(pSourceInst)) {
...
/// @tparam BasicBlockterator a basic block iterator type indicating the scan direction
template <typename BasicBlockterator>
auto GetMapRange(const InstLookupTableInBasicBlockT &container, const InstIdxLookupTableT &lookupTable,
                 BasicBlockterator it, BasicBlockterator end) {
  IGC_ASSERT(it != end);
  auto GetEndIt = [](const llvm::BasicBlock *pBasicBlock) -> BasicBlockterator {
    constexpr bool isForwardDirection = std::is_same_v<BasicBlockterator, llvm::BasicBlock::const_iterator>;
    if constexpr (isForwardDirection) {
  ...
  auto lowIt = currBeg != GetEndIt(pBB) ? lookupTable.find(&(*(currBeg))) : lookupTable.end();
  auto lowBound = lowIt == lookupTable.end() ? container.begin() : container.lower_bound(lowIt->second);
  auto currEnd = !isForwardDirection ? it : end;
  auto highIt = currEnd != GetEndIt(pBB) ? lookupTable.find(&(*(currEnd))) : lookupTable.end();
  auto highBound = highIt == lookupTable.end() ? container.end() : container.upper_bound(highIt->second);
...
auto GetBoundaryFunc(const OrderedInstructionLookupTableT &container, const InstIdxLookupTableT &lookupTable,
                     std::function<bool(const llvm::Instruction *)> &IsBoundaryInst,
                     std::vector<const llvm::Instruction *> &boundaryInstructions) {
  BoundaryCallbackT GetBoundary{};
  GetBoundary = [&container, &IsBoundaryInst, &boundaryInstructions, &lookupTable](auto beg,
                                                                                   auto end) -> decltype(beg) {
    if (beg == end) {
      return end;
    }
...
/// @param instructions the container of the visited expected instructions from the container filled by the output
/// callback
auto GetIterationFunc(const OrderedInstructionLookupTableT &container, const InstIdxLookupTableT &lookupTable,
...
IterationCallbackT GetIterationFunc(const OrderedInstructionLookupTableT &container,
                                    const InstIdxLookupTableT &lookupTable,
                                    const InstMaskLookupTableT &instMaskLookupTable,
                                    const BasicBlockMaskLookupTableT &bbLookupTable, InstructionMask &instructionMask) {
  IterationCallbackT IterateFunc{};
...
void SynchronizationObjectCoalescing::GetVisibleMemoryInstructions(
    const llvm::Instruction *pSourceInst, bool forwardDirection,
    std::vector<const llvm::Instruction *> &boundaryInstructions,
    std::vector<const llvm::Instruction *> &memoryInstructions) const {
  ...
  std::function<bool(const llvm::Instruction *)> IsMemoryInst =
      [this, memoryInstructionMask](const llvm::Instruction *pEvaluatedInst) {
  ...
  auto GetBoundaries = GetBoundaryFunc(IsFenceOperation(pSourceInst) ? m_OrderedFenceInstructionsInBasicBlockCache
                                       ...
                                       m_InstIdxLookupTable, IsBoundaryInst, boundaryInstructions);
  auto GetMemInsts = GetIterationFunc(m_OrderedMemoryInstructionsInBasicBlockCache, m_InstIdxLookupTable, IsMemoryInst,
                                      memoryInstructions);

  if (forwardDirection) {
    llvm::BasicBlock::const_iterator firstIt = ++pSourceInst->getIterator();
    std::list<llvm::BasicBlock::const_iterator> workList{firstIt};
    SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardIterationCallbackT &>(GetMemInsts),
    ...
    SearchInstructions(workList, visitedBasicBlocks, static_cast<BackwardIterationCallbackT &>(GetMemInsts),
...
void SynchronizationObjectCoalescing::GetAllUnsynchronizedMemoryInstructions(
    const llvm::Instruction *pSourceInst, std::vector<const llvm::Instruction *> &threadGroupBarriers,
    std::vector<const llvm::Instruction *> &memoryInstructions) const {
  std::function<bool(const llvm::Instruction *)> IsBoundaryInst =
      [this, pSourceInst](const llvm::Instruction *pEvaluatedInst) {
        return IsSubstituteInstruction(pEvaluatedInst, pSourceInst);
  ...
  auto GetBarrierInsts =
      GetIterationFunc(m_OrderedBarrierInstructionsInBasicBlockCache, m_InstIdxLookupTable, threadGroupBarriers);
  ...
  // gather all visible thread group barriers
  {
    ...
    llvm::BasicBlock::const_iterator firstIt = ++pSourceInst->getIterator();
    std::list<llvm::BasicBlock::const_iterator> workList{firstIt};
    SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardIterationCallbackT &>(GetBarrierInsts),
                       static_cast<ForwardBoundaryCallbackT &>(GetBoundaries));
  }

  // gather all memory instructions between thread group barriers which goes through a substitute (thread group barrier
  // -> fence -> thread group barrier)
  if (!threadGroupBarriers.empty()) {
    ...
    auto GetMemInsts = GetIterationFunc(m_OrderedMemoryInstructionsInBasicBlockCache, m_InstIdxLookupTable,
                                        IsMemoryInst, memoryInstructions);
    llvm::DenseSet<const llvm::BasicBlock *> visitedBasicBlocks;
    ...
    auto GetBoundariesAfterBarriers = GetBoundaryFunc(m_OrderedFenceInstructionsInBasicBlockCache, m_InstIdxLookupTable,
                                                      IsBoundaryInst, visibleSubstitues);
    SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardIterationCallbackT &>(GetMemInsts),
    ...
    auto GetBarrierBoundaries = GetBoundaryFunc(m_OrderedBarrierInstructionsInBasicBlockCache, m_InstIdxLookupTable);
    SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardIterationCallbackT &>(GetMemInsts),
...
/// @param pSourceInst the source synchronization instruction
/// @param forwardDirection the direction of searching
InstructionMask SynchronizationObjectCoalescing::GetInstructionMask(const llvm::Instruction *pSourceInst,
  ...
  auto GetBoundaries = GetBoundaryFunc(IsFenceOperation(pSourceInst) ? m_OrderedFenceInstructionsInBasicBlockCache
                                                                     : m_OrderedBarrierInstructionsInBasicBlockCache,
                                       m_InstIdxLookupTable, IsBoundaryInst);
  auto GetMemInsts = GetIterationFunc(m_OrderedMemoryInstructionsInBasicBlockCache, m_InstIdxLookupTable,
                                      m_InstMaskLookupTable, m_BasicBlockMemoryInstructionMaskCache, result);
  if (forwardDirection) {
    llvm::BasicBlock::const_iterator firstIt = ++pSourceInst->getIterator();
    std::list<llvm::BasicBlock::const_iterator> workList{firstIt};
    SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardIterationCallbackT &>(GetMemInsts),
...
IGC::InstructionMask
SynchronizationObjectCoalescing::GetAtomicInstructionMaskFromPointer(const llvm::Instruction *pSourceInst) const {
...
SynchronizationObjectCoalescing::SynchronizationCaseMask
SynchronizationObjectCoalescing::GetSynchronizationMask(InstructionMask localForwardMemoryInstructionMask,
                                                        InstructionMask localBackwardMemoryInstructionMask,
                                                        InstructionMask readBit, InstructionMask writeBit) const {
  SynchronizationCaseMask result = SynchronizationCaseMask::Empty;
  // write -> barrier/fence -> read
  bool isWriteSyncReadCase =
      ((localBackwardMemoryInstructionMask & writeBit) != 0 && (localForwardMemoryInstructionMask & readBit) != 0);
  ...
  bool isWriteSyncWriteCase =
      ((localBackwardMemoryInstructionMask & writeBit) != 0 && (localForwardMemoryInstructionMask & writeBit) != 0);
  if (isWriteSyncWriteCase) {
    result = static_cast<SynchronizationCaseMask>(result | SynchronizationCaseMask::WriteSyncWrite);
  }

  // write -> fence -> ret
  bool requiresFlushWrites =
      static_cast<uint32_t>(writeBit & (SharedMemoryWriteOperation | BufferWriteOperation | TypedWriteOperation)) != 0;
  ...
  bool isAtomicSyncWriteCase = ((localBackwardMemoryInstructionMask & AtomicOperation) != 0 &&
                                (localForwardMemoryInstructionMask & writeBit) != 0);
  ...
  bool isWriteSyncAtomicCase = ((localBackwardMemoryInstructionMask & writeBit) != 0 &&
                                (localForwardMemoryInstructionMask & AtomicOperation) != 0);
  if (isWriteSyncAtomicCase) {
    result = static_cast<SynchronizationCaseMask>(result | SynchronizationCaseMask::WriteSyncAtomic);
  }

  // read -> barrier -> atomic
  bool isReadSyncAtomicCase = ((localBackwardMemoryInstructionMask & readBit) != 0 &&
                               (localForwardMemoryInstructionMask & AtomicOperation) != 0);
  ...
  // read -> barrier -> write
  bool isReadSyncWriteCase =
      ((localBackwardMemoryInstructionMask & readBit) != 0 && (localForwardMemoryInstructionMask & writeBit) != 0);
  ...
  bool isAtomicSyncAtomicCase = ((localBackwardMemoryInstructionMask & AtomicOperation) != 0 &&
                                 (localForwardMemoryInstructionMask & AtomicOperation) != 0);
...
/// redundancy.
SynchronizationObjectCoalescing::SynchronizationCaseMask
SynchronizationObjectCoalescing::GetStrictSynchronizationMask(llvm::Instruction *pInst) const {
  ...
  if (IsFenceOperation(pInst)) {
    ...
    strictSynchronizationCaseMask = static_cast<SynchronizationCaseMask>((~SynchronizationCaseMask::AtomicSyncAtomic) &
    ...
    if (disableReadFenceWriteCase) {
      strictSynchronizationCaseMask = static_cast<SynchronizationCaseMask>((~SynchronizationCaseMask::ReadSyncWrite) &
                                                                           strictSynchronizationCaseMask);
    }
  }
...
IGC::SynchronizationObjectCoalescing::SynchronizationCaseMask
SynchronizationObjectCoalescing::GetL1CacheInvalidatioSynchronizationMask() const {
  constexpr SynchronizationCaseMask L1CacheInvalidationCaseMask = static_cast<SynchronizationCaseMask>(
      SynchronizationCaseMask::WriteSyncRead | SynchronizationCaseMask::WriteSyncAtomic |
...
SynchronizationObjectCoalescing::SynchronizationCaseMask
SynchronizationObjectCoalescing::GetSynchronizationMaskForAllResources(
    InstructionMask localForwardMemoryInstructionMask, InstructionMask localBackwardMemoryInstructionMask) const {
  // buffer access
  SynchronizationCaseMask syncCaseMask = GetSynchronizationMask(
      localForwardMemoryInstructionMask, localBackwardMemoryInstructionMask, BufferReadOperation, BufferWriteOperation);
  // shared memory access
  syncCaseMask = static_cast<SynchronizationCaseMask>(
      syncCaseMask | GetSynchronizationMask(localForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
                                            SharedMemoryReadOperation, SharedMemoryWriteOperation));
  // typed access
  syncCaseMask = static_cast<SynchronizationCaseMask>(
      syncCaseMask | GetSynchronizationMask(localForwardMemoryInstructionMask, localBackwardMemoryInstructionMask,
                                            TypedReadOperation, TypedWriteOperation));
...
/// @param pSourceInst the source synchronization instruction
InstructionMask
SynchronizationObjectCoalescing::GetUnsynchronizedForwardInstructionMask(const llvm::Instruction *pSourceInst) const {
  std::vector<const llvm::Instruction *> boundaryInstructions;
...
bool SynchronizationObjectCoalescing::IsRequiredForAtomicOperationsOrdering(
  ...
  for (const llvm::Instruction *pInst : memoryInstructions) {
    ...
    // store float %value, float addrspace(1)* %global_address
    {
      ...
      BoundaryCallbackT GetBoundaries = GetBoundaryFunc(m_OrderedFenceInstructionsInBasicBlockCache,
                                                        m_InstIdxLookupTable, IsBoundaryInst, boundaryInstructions);
      ProcessCallbackT ProcessInstructions{};
      ProcessInstructions = [&GetBoundaries](auto beg, auto end) { return GetBoundaries(beg, end) == end; };
      ...
      // forward direction.
      SearchInstructions(workList, visitedBasicBlocks, static_cast<ForwardProcessCallbackT &>(ProcessInstructions));
...
InstructionMask
SynchronizationObjectCoalescing::GetInstructionMask(const std::vector<const llvm::Instruction *> &input) const {
  InstructionMask result = InstructionMask::None;
...
bool SynchronizationObjectCoalescing::IsSubstituteInstruction(const llvm::Instruction *pEvaluatedInst,
  ...
  if (IsUntypedMemoryFenceOperation(pEvaluatedInst) && IsUntypedMemoryFenceOperation(pReferenceInst)) {
    ...
    bool isDuplicate = pEvaluatedInst->getOperand(commitEnableArg) == pReferenceInst->getOperand(commitEnableArg);
    isDuplicate &= pEvaluatedInst->getOperand(L3FlushRWDataArg) == pReferenceInst->getOperand(L3FlushRWDataArg);
    isDuplicate &=
        pEvaluatedInst->getOperand(L3FlushConstantDataArg) == pReferenceInst->getOperand(L3FlushConstantDataArg);
    isDuplicate &=
        pEvaluatedInst->getOperand(L3FlushTextureDataArg) == pReferenceInst->getOperand(L3FlushTextureDataArg);
    isDuplicate &=
        pEvaluatedInst->getOperand(L3FlushInstructionsArg) == pReferenceInst->getOperand(L3FlushInstructionsArg);

    isDuplicate &= ((IsUntypedMemoryFenceOperationForGlobalAccess(pEvaluatedInst) &&
                     IsUntypedMemoryFenceOperationForGlobalAccess(pReferenceInst)) ||
                    (m_HasIndependentSharedMemoryFenceFunctionality &&
                     (IsUntypedMemoryFenceOperationForSharedMemoryAccess(pEvaluatedInst) &&
                      IsUntypedMemoryFenceOperationForSharedMemoryAccess(pReferenceInst)))) ||
                   (!m_HasIndependentSharedMemoryFenceFunctionality &&
                    (IsUntypedMemoryFenceOperationForGlobalAccess(pEvaluatedInst) ||
                     !IsUntypedMemoryFenceOperationForGlobalAccess(pReferenceInst)));
    ...
    bool cond3 = !evaluatedIsOpNone && opReference == LSC_FENCE_OP_INVALIDATE &&
                 (opEvaluated == LSC_FENCE_OP_INVALIDATE || opEvaluated == LSC_FENCE_OP_EVICT);
    isDuplicate &= (cond1 || cond2 || cond3);

    return isDuplicate;
  } else if (IsTypedMemoryFenceOperation(pEvaluatedInst) && IsTypedMemoryFenceOperation(pReferenceInst)) {
    ...
    return isDuplicate;
  } else if (IsUrbFenceOperation(pEvaluatedInst) && IsUrbFenceOperation(pReferenceInst)) {
    return true;
  } else if (IsThreadBarrierOperation(pEvaluatedInst) && IsThreadBarrierOperation(pReferenceInst)) {
    return true;
  }
...
////////////////////////////////////////////////////////////////////////////////
inline llvm::Type *GetReadOperationPointerType(const llvm::Instruction *pInst) {
  if (llvm::isa<llvm::GenIntrinsicInst>(pInst)) {
    const llvm::GenIntrinsicInst *pGenIntrinsicInst = llvm::cast<llvm::GenIntrinsicInst>(pInst);
...
////////////////////////////////////////////////////////////////////////////////
inline llvm::Type *GetWriteOperationPointerType(const llvm::Instruction *pInst) {
  if (llvm::isa<llvm::GenIntrinsicInst>(pInst)) {
    const llvm::GenIntrinsicInst *pGenIntrinsicInst = llvm::cast<llvm::GenIntrinsicInst>(pInst);
...
////////////////////////////////////////////////////////////////////////
bool SynchronizationObjectCoalescing::IsUntypedMemoryFenceOperationWithInvalidationFunctionality(
    const llvm::Instruction *pInst) const {
  constexpr uint32_t L1CacheInvalidateArg = 6;
...
////////////////////////////////////////////////////////////////////////
bool SynchronizationObjectCoalescing::IsUntypedMemoryFenceOperationForSharedMemoryAccess(
    const llvm::Instruction *pInst) const {
  constexpr uint32_t globalMemFenceArg = 5;
  return IsUntypedMemoryFenceOperation(pInst) &&
         (!m_HasIndependentSharedMemoryFenceFunctionality ||
          llvm::cast<llvm::ConstantInt>(pInst->getOperand(globalMemFenceArg))->getValue().getBoolValue() == false);
}

////////////////////////////////////////////////////////////////////////
bool SynchronizationObjectCoalescing::IsUntypedMemoryFenceOperationForGlobalAccess(
    const llvm::Instruction *pInst) const {
  constexpr uint32_t globalMemFenceArg = 5;
  return IsUntypedMemoryFenceOperation(pInst) &&
...
////////////////////////////////////////////////////////////////////////
bool SynchronizationObjectCoalescing::IsUntypedMemoryLscFenceOperationForGlobalAccess(
    const llvm::Instruction *pInst) const {
  if (IsLscFenceOperation(pInst)) {
    LSC_SFID mem = GetLscMem(pInst);
...
////////////////////////////////////////////////////////////////////////
bool SynchronizationObjectCoalescing::IsTypedMemoryFenceOperationWithInvalidationFunctionality(
    const llvm::Instruction *pInst) const {
  constexpr uint32_t L1CacheInvalidateArg = 0;
...
bool SynchronizationObjectCoalescing::IsFenceOperation(const llvm::Instruction *pInst) {
  return IsTypedMemoryFenceOperation(pInst) || IsUrbFenceOperation(pInst) || IsLscFenceOperation(pInst) ||
...
bool SynchronizationObjectCoalescing::IsGlobalWritableResource(llvm::Type *pResourePointerType) {
...
void SynchronizationObjectCoalescing::RegisterRedundancyExplanation(const llvm::Instruction *pInst,
                                                                    ExplanationEntry::Cause cause) {
  constexpr bool forwardDirection = true;
  constexpr bool backwardDirection = false;
  ...
  auto GetBoundaryInst = [this, GetIndex](const llvm::Instruction *pInst) -> ExplanationEntry::SyncInstDescription {
    ...
    return {pInst->getParent(), GetIndex(pInst), stringRepresentation};
  };
  ...
  GetVisibleMemoryInstructions(pInst, forwardDirection, forwardBoundaryInstructions,
                               explanationEntry.m_ForwardMemoryInstructions);
  GetVisibleMemoryInstructions(pInst, backwardDirection, backwardBoundaryInstructions,
                               explanationEntry.m_BackwardMemoryInstructions);
  if (!IsThreadBarrierOperation(pInst) && cause == ExplanationEntry::Cause::StrictRedundancy) {
    GetAllUnsynchronizedMemoryInstructions(pInst, threadGroupBarriersBoundaries,
                                           explanationEntry.m_ForwardMemoryInstructions);
  }
...
void SynchronizationObjectCoalescing::print(llvm::raw_ostream &stream,
                                            bool onlyMemoryInstructionMask /* = true*/) const {
  ...
  auto GetSyncInstName = [this, GetRedundancyCauseName](const ExplanationEntry::SyncInstDescription &syncInstDesc) {
    ...
    syncDescName << std::get<0>(syncInstDesc)->getName().str() << ", " << std::get<1>(syncInstDesc) << ", "
                 << std::get<2>(syncInstDesc);
  ...
  for (const ExplanationEntry &explanationEntry : m_ExplanationEntries) {
    ...
    stream << indentation << "The forward instruction mask: "
           << GetInstructionMaskName(GetInstructionMask(explanationEntry.m_ForwardMemoryInstructions)) << "\n";
    stream << indentation << "The backward instruction mask: "
           << GetInstructionMaskName(GetInstructionMask(explanationEntry.m_BackwardMemoryInstructions)) << "\n";
    stream << "\n";
...
////////////////////////////////////////////////////////////////////////
llvm::Pass *createSynchronizationObjectCoalescing() { return new SynchronizationObjectCoalescing(); }
