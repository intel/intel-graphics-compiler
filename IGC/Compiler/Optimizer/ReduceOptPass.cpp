...
bool ReduceOptPass::createReduceWI0(Instruction *ReduceInstr) {
    ...
    std::vector<std::string> OpSet = {
        "IAdd", "FAdd", "UMin",
        "FMin", "SMin", "FMax",
        "UMax", "SMax", "IMulKHR",
        "FMulKHR", "BitwiseAndKHR", "BitwiseOrKHR",
        "BitwiseXorKHR", "LogicalAndKHR", "LogicalOrKHR",
        "LogicalXorKHR"};
    ...
    FunctionType *NewReduceFuncT = FunctionType::get(
        ReduceInstr->getOperand(2)->getType(),
        ReduceInstr->getOperand(2)->getType(),
        false);
...
bool ReduceOptPass::checkGlobalId(Value *Val) {
    ...
    if (!checkBuiltInName(Bi1, GlobalSizeName) ||
        !checkBuiltInName(Bi4, GlobalSizeName)) {
        return false;
    }

    if (!checkBuiltInName(Bi2, GlobalInvocationIdName) ||
        !checkBuiltInName(Bi5, GlobalInvocationIdName) ||
        !checkBuiltInName(Bi7, GlobalInvocationIdName)) {
        return false;
    }

    if (!checkBuiltInName(Bi3, GlobalOffsetName) ||
        !checkBuiltInName(Bi6, GlobalOffsetName) ||
...
bool ReduceOptPass::checkLocalId(Value *Val) {
    ...
    if (!checkBuiltInName(Bi1, WorkgroupSizeName) ||
        !checkBuiltInName(Bi4,WorkgroupSizeName)) {
        return false;
    }

    if (!checkBuiltInName(Bi2, LocalInvocationIdName) ||
        !checkBuiltInName(Bi3, LocalInvocationIdName) ||
...
bool ReduceOptPass::checkCmp(Value *Val) {
    if (isa<CmpInst>(Val)) {
        ...
        if ((CmpConst->getSExtValue() == 0) &&
            (Cmp->getPredicate() == CmpInst::ICMP_EQ)) {
            return true;
        }
    }
    return false;
