...
Value* InstCombiner::SimplifyDemandedUseBits(Value* V, APInt DemandedMask,
    ...
    const uint32_t BitWidth = DemandedMask.getBitWidth();
    Type* const VTy = V->getType();

    IGC_ASSERT(nullptr != VTy);
    IGC_ASSERT_MESSAGE((!VTy->isIntOrIntVectorTy() || VTy->getScalarSizeInBits() == BitWidth), "Value *V, DemandedMask and Known must have same BitWidth");
    IGC_ASSERT_MESSAGE(Known.getBitWidth() == BitWidth, "Value *V, DemandedMask and Known must have same BitWidth");
    ...
    case Instruction::And: {
        ...
        if (SimplifyDemandedBits(I, 1, DemandedMask, RHSKnown, Depth + 1) ||
            SimplifyDemandedBits(I, 0, DemandedMask & ~RHSKnown.Zero, LHSKnown,
                Depth + 1))
            return I;
        IGC_ASSERT_MESSAGE(!RHSKnown.hasConflict(), "Bits known to be one AND zero?");
        IGC_ASSERT_MESSAGE(!LHSKnown.hasConflict(), "Bits known to be one AND zero?");
    ...
    case Instruction::Trunc: {
        ...
        IGC_ASSERT_MESSAGE(!Known.hasConflict(), "Bits known to be one AND zero?");
        break;
    }
    ...
    case Instruction::AShr: {
        ...
        if (match(I->getOperand(1), m_APInt(SA))) {
            ...
            IGC_ASSERT_MESSAGE(BitWidth > ShiftAmt, "Shift amount not saturated?");
...
Value* InstCombiner::SimplifyDemandedVectorElts(Value* V, APInt DemandedElts,
    ...
    case Instruction::Call: {
        ...
        case Intrinsic::x86_avx512_packuswb_512: {
            ...
            IGC_ASSERT_MESSAGE(VWidth == (InnerVWidth * 2), "Unexpected input size");
