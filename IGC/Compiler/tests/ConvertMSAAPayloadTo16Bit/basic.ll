...
;
; RUN: igc_opt -debugify -igc-ConvertMSAAPayloadTo16Bit -check-debugify -S < %s 2>&1 | FileCheck %s
; ------------------------------------------------
...
; CHECK:    [[TMP6:%.*]] = sext <4 x i16> [[TMP1]] to <4 x i32>
; CHECK:    [[TMP7:%.*]] = shufflevector <4 x i32> [[TMP6]], <4 x i32> undef, <2 x i32> <i32 0, i32 1>
; CHECK:    [[TMP9:%.*]] = extractelement <2 x i32> [[TMP7]], i32 1
...
; CHECK:    [[TMP3:%.*]] = shufflevector <4 x i32> [[TMP2]], <4 x i32> undef, <2 x i32> <i32 0, i32 1>
; CHECK:    store <2 x i32> [[TMP3]], <2 x i32>* [[DST:%.*]], align 8
; CHECK:    ret void
;
  %1 = call <2 x i32> @llvm.genx.GenISA.ldmcsptr.v2i32.i32.p2v4f32(i32 1, i32 2, i32 3, i32 4, <4 x float> addrspace(2)* %src, i32 0, i32 0, i32 0)
