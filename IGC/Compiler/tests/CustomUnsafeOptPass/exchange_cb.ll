...
define float @test1(float %x, ptr addrspace(65536) %cb1, ptr addrspace(65536) %cb2) #0 {
entry:
  %0 = load float, ptr addrspace(65536) %cb1
  %1 = fmul float %x, %0
  %2 = load float, ptr addrspace(65536) %cb2
  %3 = fmul float %x, %2
  %4 = fadd float %1, %3
  ret float %4
}
...
; CHECK: [[CB1:%[A-z0-9]*]] = load float, ptr addrspace(65536) %cb1
; CHECK-NOT: fmul float %x, [[CB1]]
; CHECK: [[CB2:%[A-z0-9]*]] = load float, ptr addrspace(65536) %cb2
