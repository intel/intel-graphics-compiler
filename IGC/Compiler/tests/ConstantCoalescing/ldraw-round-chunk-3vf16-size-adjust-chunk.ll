...
; RUN: igc_opt --opaque-pointers %s -S -o - -igc-constant-coalescing -instcombine -dce | FileCheck %s --check-prefixes=CHECK,%if llvm-22-plus %{CHECK-LLVM22%} %else %{CHECK-PRE22%}
...
entry:
 ...
 ; CHECK-PRE22: [[RESULT:%.*]] = shufflevector <4 x half> [[CHUNK]], <4 x half> undef, <3 x i32> <i32 0, i32 1, i32 2>
 ; CHECK-LLVM22: [[RESULT:%.*]] = shufflevector <4 x half> [[CHUNK]], <4 x half> poison, <3 x i32> <i32 0, i32 1, i32 2>
