...
; CHECK-LABEL: @foo(
; CHECK:       entry:
; CHECK:         [[IN0_SHIFTED:%.*]] = getelementptr <2 x double>, ptr addrspace(3) [[IN0:%.*]], i32 [[OFFSETIN0:%.*]]
; CHECK:         [[IN0_AS_INT:%.*]] = ptrtoint ptr addrspace(3) [[IN0_SHIFTED]] to i32
...
; CHECK:         [[L0_PTR:%.*]] = inttoptr i32 [[IN0_NEW]] to ptr addrspace(3)
...
; CHECK:         [[L0:%.*]] = load <2 x double>, ptr addrspace(3) [[L0_PTR]], align 16
...
;
entry:
  %in0_shifted = getelementptr <2 x double>, ptr addrspace(3) %in_out, i32 %offsetIn0
  %in0_as_int = ptrtoint ptr addrspace(3) %in0_shifted to i32
  %in0_new = add i32 %in0_as_int, 800
  br label %entry_preheader

entry_preheader:                                  ; preds = %entry
  %l0_ptr = inttoptr i32 %in0_new to ptr addrspace(3)
  %l0 = load <2 x double>, ptr addrspace(3) %l0_ptr, align 16
...
afterloop:                                        ; preds = %loop
  ...
  %out0_shifted = getelementptr double, ptr addrspace(3) %in_out, i32 2
  store double %acc, ptr addrspace(3) %out0_shifted, align 8
  ret void
}
