...
define i32 @foo(i32 %dim, <8 x i32> %r0, <8 x i32> %payloadHeader, <3 x i32> %localSize) nounwind {
  %id = call i32 @__builtin_IB_get_local_size(i32 1)
  ret i32 %id
}
...
; CHECK:         [[local_size:%[a-zA-Z0-9]+]] = extractelement <3 x i32> %localSize, i32 1

; CHECK-NOT:     call i32 @__builtin_IB_get_local_size(i32 1)
