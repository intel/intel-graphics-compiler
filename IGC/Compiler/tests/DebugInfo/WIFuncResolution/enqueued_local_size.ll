...
; {
;   int x  = get_enqueued_local_size(0);
;   dst[0] = x;
; }
;
; ------------------------------------------------
...
define spir_kernel void @test_wi(i32 addrspace(1)* %dst, <8 x i32> %r0, <8 x i32> %payloadHeader, <3 x i32> %enqueuedLocalSize, i8* %privateBase, i32 %bufferOffset) #0 !dbg !16 {
entry:
  ...
  %call.i = call spir_func i32 @__builtin_IB_get_enqueued_local_size(i32 0), !dbg !30
...
; Check MD:
;
; CHECK-DAG: [[FILE:![0-9]*]] = !DIFile(filename: "enqueued_local_size.ll", directory: "/")
...
!17 = !DIFile(filename: "enqueued_local_size.ll", directory: "/")
