...
; ------------------------------------------------
; CustomSafeOptPass: extractelement, dp4WithIdentityMatrix with Sext pattern
; ------------------------------------------------
;
; Test checks that sequence containing extractelements from
; constant vector with Sext pattern inserted (shr|or + sext)
; is still recognized by dp4WithIdentityMatrix and converted
; to efficient select-based pattern

define float @test_extract_identity16_with_sext(i32 %src1, float %arg0, float %arg1, float %arg2, float %arg3) {
; CHECK-LABEL: define float @test_extract_identity16_with_sext(
...
;
  ...
  ; Sext pattern for first element
  %sext0 = sext i32 %idx4 to i64
  ...
  %idx5 = or i32 %idx4, 1
  ; Sext pattern for second element5
  %sext1 = sext i32 %idx5 to i64
  ...
  %idx6 = or i32 %idx4, 2
  ; Sext pattern for third element
  %sext2 = sext i32 %idx6 to i64
  ...
  %idx7 = or i32 %idx4, 3
  ; Sext pattern for fourth element
  %sext3 = sext i32 %idx7 to i64
...
define i32 @test_extract_identity16_with_sext_i32(i32 %src1, i32 %arg0, i32 %arg1, i32 %arg2, i32 %arg3) {
; CHECK-LABEL: define i32 @test_extract_identity16_with_sext_i32(
...
;
  ...
  %idx5 = or i32 %idx4, 1
  ; Sext pattern for second element
  %sext1 = sext i32 %idx5 to i64
...
; This should now optimize with our improved sext pattern fix
define float @test_extract_identity16_no_nuw_with_sext(i32 %src1, float %arg0, float %arg1, float %arg2, float %arg3) {
; CHECK-LABEL: define float @test_extract_identity16_no_nuw_with_sext(
