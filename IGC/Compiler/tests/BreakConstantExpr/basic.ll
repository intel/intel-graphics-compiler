...
define spir_kernel void @test_breakexpr(ptr %dst, i32 %isrc, float %fsrc) {
; CHECK-LABEL: @test_breakexpr(
; CHECK:    [[TMP1:%.*]] = getelementptr inbounds i32, ptr addrspace(2) @x, i32 2
; CHECK:    [[TMP2:%.*]] = ptrtoint ptr addrspace(2) [[TMP1]] to i32
; CHECK:    [[TMP3:%.*]] = add i32 [[TMP2]], 1
; CHECK:    [[TMP4:%.*]] = insertelement <2 x i32> undef, i32 [[TMP3]], i32 0
; CHECK:    [[TMP5:%.*]] = ptrtoint ptr addrspace(1) @y to i32
...
; CHECK:    ret void
;
  %1 = extractelement <2 x i32> <i32 add (i32 ptrtoint (ptr addrspace(2) getelementptr inbounds (i32, ptr addrspace(2) @x, i32 2) to i32), i32 1), i32 ptrtoint (ptr addrspace(1) @y to i32)>, i32 1
