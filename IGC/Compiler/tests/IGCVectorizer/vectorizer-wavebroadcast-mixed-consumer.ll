...
; REQUIRES: regkeys
; RUN: igc_opt -S --opaque-pointers --igc-vectorizer --regkey=VectorizerAllowWAVEBROADCAST=1 --platformbmg < %s 2>&1 | FileCheck %s --check-prefix=NODCE
; RUN: igc_opt -S --opaque-pointers --igc-vectorizer --regkey=VectorizerAllowWAVEBROADCAST=1 -dce --platformbmg < %s 2>&1 | FileCheck %s --check-prefix=DCE

; the joint is created and no extractelement is ever taken out of it
; NODCE: [[SRC:%.*]] = call float @llvm.exp2.f32(float 0.000000e+00)
; NODCE: %vectorized_joint_wavebroadcast = call <8 x float> @llvm.genx.GenISA.JointWaveBroadcast.v8f32.f32.v8i32(float [[SRC]], <8 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>, i32 0)
; NODCE-NOT: extractelement <8 x float> %vectorized_joint_wavebroadcast

; the joint survives because a vector consumer uses it, the scalar lane keeps its
; own broadcast, and still no extractelement is taken out of the joint
; before we would extract lane 0 from joint and kill scalar
; DCE: [[SRC:%.*]] = call float @llvm.exp2.f32(float 0.000000e+00)
; DCE: %vectorized_joint_wavebroadcast = call <8 x float> @llvm.genx.GenISA.JointWaveBroadcast.v8f32.f32.v8i32(float [[SRC]], <8 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>, i32 0)
; DCE-NOT: extractelement <8 x float> %vectorized_joint_wavebroadcast, i32 0
; DCE: %vectorized_binary = fmul <8 x float> %tmp235, %vectorized_joint_wavebroadcast
; DCE: [[SCALAR:%.*]] = call float @llvm.genx.GenISA.WaveBroadcast.f32(float %tmp658, i32 0, i32 0)
; DCE-NEXT: %scalar_use = fadd float [[SCALAR]]

source_filename = "reduced.ll"
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v16:16:16-v24:32:32-v32:32:32-v48:64:64-v64:64:64-v96:128:128-v128:128:128-v192:256:256-v256:256:256-v512:512:512-v1024:1024:1024-n8:16:32"
target triple = "spir64-unknown-unknown"

define spir_kernel void @zot.12(ptr %out) {

bb:
  br label %loop

loop:                                            ; preds = %loop, %bb
  %tmp235 = phi <8 x float> [ zeroinitializer, %bb ], [ %tmp1842, %loop ]
  ...
  %scalar_use = fadd float %tmp1634, 1.000000e+00
  store float %scalar_use, ptr %out
