...
; RUN: igc_opt -S --opaque-pointers --igc-vectorizer --regkey=VectorizerEnableVirtualSeeds=1 -dce --regkey=VectorizerAllowBITCAST=0 --platformbmg < %s 2>&1 | FileCheck %s

; Same swizzle pattern as vectorizer-virtual-seed-soa-basic.ll, but with the
; bitcast opcode disallowed the speculative tree breaks and no seed is formed.
