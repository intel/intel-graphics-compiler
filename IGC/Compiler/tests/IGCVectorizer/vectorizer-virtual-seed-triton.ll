...
; RUN: igc_opt -S --opaque-pointers --igc-vectorizer --regkey=VectorizerAllowBITCAST=1 --regkey=VectorizerEnableVirtualSeeds=1 --platformCri -dce < %s 2>&1 | FileCheck %s

; CHECK-LABEL: _Z15__spirv_ocl_logf.exit100
; CHECK: fadd contract <8 x float>

; The scalar inputs of the GenISA_PredicatedStore group form a virtual seed: the
; naive swizzle collapses into a single wide bitcast whose extracts feed the
; predicated stores.
; CHECK: bitcast <8 x float> %{{.*}} to <8 x i32>
; CHECK: call void @llvm.genx.GenISA.PredicatedStore.p1.i32(ptr addrspace(1) %{{.*}}, i32 %vector_extract
