...
; RUN: igc_opt --opaque-pointers -S  --igc-vectorizer -dce --platformbmg --regkey=VectorizerAllowUniformSelect=2 --regkey=VectorizerAllowUniformCMP=1 --regkey=VectorizerDepWindowMultiplier=6 --regkey=VectorizerAllowSamePredSelect=1 --regkey=VectorizerLog=1 --regkey=VectorizerLogToErr=1 < %s 2>&1 | FileCheck %s

; CHECK: FCMP ONE is not supported by vector emitter
; CHECK: %vectorized_select = select i1 false, <8 x float> {{.*}}, <8 x float> {{.*}}
; CHECK-NOT: %vectorized_cmp = fcmp one <8 x float>
; CHECK-NOT: %vectorized_select{{.*}} = select
...
bb2:                                              ; preds = %bb2, %bb
  ...
  %tmp18 = select i1 false, float 0.000000e+00, float %tmp10
  %tmp19 = select i1 false, float 0.000000e+00, float %tmp11
  %tmp20 = select i1 false, float 0.000000e+00, float %tmp12
  %tmp21 = select i1 false, float 0.000000e+00, float %tmp13
  %tmp22 = select i1 false, float 0.000000e+00, float %tmp14
  %tmp23 = select i1 false, float 0.000000e+00, float %tmp15
  %tmp24 = select i1 false, float 0.000000e+00, float %tmp16
  %tmp25 = select i1 false, float 0.000000e+00, float %tmp17
...
bb59:                                             ; preds = %bb2
  %tmp60 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp61 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp62 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp63 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp64 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp65 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp66 = fcmp one float 0.000000e+00, 0.000000e+00
  %tmp67 = fcmp one float 0.000000e+00, 0.000000e+00
...
!11 = !{!"implicitArgInfoList", !8, !10}
!12 = !{!"FuncMDMap[0]", ptr @widget}
!13 = !{!"FuncMDValue[0]", !11, !16}
!14 = !{!"FuncMD", !12, !13}
!15 = !{!"ModuleMD", !14}
!IGCMetadata = !{!15}
!16 = !{!"requiredSubGroupSize", i32 16}
