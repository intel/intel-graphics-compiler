...
; The extracts feeding the stores come from two different source vectors, so the
; group is not a naive swizzle: profitability fails and no virtual seed is built.

; CHECK: Source operand differ between extract elements
...
entry:
  ...
  %src2 = call <8 x float> @llvm.genx.GenISA.sub.group.dpas.v8f32.v8f32.v8i16.v8i32(<8 x float> zeroinitializer, <8 x i16> %a, <8 x i32> %b, i32 1, i32 1, i32 1, i32 1, i1 false)
  ...
  %e4 = extractelement <8 x float> %src2, i64 4
  %e5 = extractelement <8 x float> %src2, i64 5
  %e6 = extractelement <8 x float> %src2, i64 6
  %e7 = extractelement <8 x float> %src2, i64 7
