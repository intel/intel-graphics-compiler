...
; Natural seeds have different widths (<8 x float> and <4 x float>), so
; PreferredVectorSize resolves to 0 and virtual seed formation is skipped.

; CHECK-NOT: Preferred Vec Size
...
entry:
  ...
  %m0 = extractelement <8 x float> %src, i64 0
  %s4 = insertelement <4 x float> zeroinitializer, float %m0, i64 3
  %u4 = fadd <4 x float> %s4, %s4
