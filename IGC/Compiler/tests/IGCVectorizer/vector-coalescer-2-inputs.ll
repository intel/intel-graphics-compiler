...
; CHECK: [[VECT:%.*]] = call <16 x float> @llvm.maxnum.v16f32(<16 x float> %coalesced_input, <16 x float> %coalesced_input1)
; CHECK: shufflevector <16 x float> [[VECT]], <16 x float> undef, <8 x i32> <i32 8, i32 9, i32 10, i32 11, i32 12, i32 13, i32 14, i32 15>
; CHECK: shufflevector <16 x float> [[VECT]], <16 x float> undef, <8 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
...
._crit_edge4000:                                  ; preds = %._crit_edge4000, %cond-add
  %vectorized_intrinsic0 = call <8 x float> @llvm.maxnum.v8f32(<8 x float> %in1, <8 x float> %in2)
  %vectorized_intrinsic1 = call <8 x float> @llvm.maxnum.v8f32(<8 x float> %in3, <8 x float> %in4)
  br label %end
