...
; RUN: igc_opt -S  --igc-vectorizer -dce --regkey=VectorizerLog=1 --regkey=VectorizerLogToErr=1 --platformbmg < %s 2>&1 | FileCheck %s

; CHECK: SIMD Size: 32
; CHECK: Operand [1]:  First:   %17 = fmul fast float %9, %1
; CHECK-NEXT: Not safe to vectorize
