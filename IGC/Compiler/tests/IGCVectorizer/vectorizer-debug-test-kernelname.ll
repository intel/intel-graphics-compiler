...
; REQUIRES: regkeys
; RUN: igc_opt --igc-vectorizer -S -dce --platformbmg --regkey=VectorizerNameFilter=quux < %s 2>&1 | FileCheck %s --check-prefix=MATCH
; RUN: igc_opt --igc-vectorizer -S -dce --platformbmg --regkey=VectorizerNameFilter=^q.*x$ < %s 2>&1 | FileCheck %s --check-prefix=MATCH
; RUN: igc_opt --igc-vectorizer -S -dce --platformbmg --regkey=VectorizerNameFilter=nomatch < %s 2>&1 | FileCheck %s --check-prefix=NOMATCH

; When VectorizerNameFilter matches the function name, the vectorizer runs
; and the scalar phis collapse into a single vector phi.
; MATCH-LABEL: @quux(
; MATCH: phi <8 x float>
; MATCH-NOT: phi float

; When VectorizerNameFilter does not match, the vectorizer is skipped and
; the original scalar phis remain untouched.
; NOMATCH-LABEL: @quux(
; NOMATCH: phi float
; NOMATCH-NOT: phi <8 x float>
