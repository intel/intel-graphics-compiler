...
; RUN: igc_opt -S --opaque-pointers --igc-vectorizer --regkey=VectorizerEnableVirtualSeeds=1 --regkey=VectorizerAllowBITCAST=1 -dce --platformbmg < %s 2>&1 | FileCheck %s

; The swizzle source and its extractelements live in a different basic block
; than the stored bitcasts. The virtual seed is still formed and vectorized.

; CHECK-LABEL: @k
; CHECK: %vectorized_cast = bitcast <8 x float> %src to <8 x i32>
; CHECK: call void @llvm.genx.GenISA.PredicatedStore.p1.i32(ptr addrspace(1) @g0, i32 %vector_extract
