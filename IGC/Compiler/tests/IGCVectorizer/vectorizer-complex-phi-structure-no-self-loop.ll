...
; RUN: igc_opt --opaque-pointers -S  --igc-vectorizer -dce --platformbmg --regkey=VectorizerLog=1 --regkey=VectorizerLogToErr=1 < %s 2>&1 | FileCheck %s

; CHECK-LABEL: loop:
; CHECK: [[VECT_PHI:%vectorized_phi.*]] = phi <8 x float> [ <float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000, float 0xFFF0000000000000>, %.lr.ph ], [ [[VECTOR:%vector.*]], %._crit_edge ]
; CHECK: br label %[[BB2:.*]]
...
loop:                                             ; preds = %.lr.ph, %._crit_edge
  %m0 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn0, %._crit_edge ]
  %m1 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn1, %._crit_edge ]
  %m2 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn2, %._crit_edge ]
  %m3 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn3, %._crit_edge ]
  %m4 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn4, %._crit_edge ]
  %m5 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn5, %._crit_edge ]
  %m6 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn6, %._crit_edge ]
  %m7 = phi float [ 0xFFF0000000000000, %.lr.ph ], [ %mn7, %._crit_edge ]
  %v0 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn0, %._crit_edge ]
  %v1 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn1, %._crit_edge ]
  %v2 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn2, %._crit_edge ]
  %v3 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn3, %._crit_edge ]
  %v4 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn4, %._crit_edge ]
  %v5 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn5, %._crit_edge ]
  %v6 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn6, %._crit_edge ]
  %v7 = phi float [ 0.000000e+00, %.lr.ph ], [ %vn7, %._crit_edge ]
  %o0 = phi float [ 0.000000e+00, %.lr.ph ], [ %on0, %._crit_edge ]
  %o1 = phi float [ 0.000000e+00, %.lr.ph ], [ %on1, %._crit_edge ]
  %o2 = phi float [ 0.000000e+00, %.lr.ph ], [ %on2, %._crit_edge ]
  %o3 = phi float [ 0.000000e+00, %.lr.ph ], [ %on3, %._crit_edge ]
  %o4 = phi float [ 0.000000e+00, %.lr.ph ], [ %on4, %._crit_edge ]
  %o5 = phi float [ 0.000000e+00, %.lr.ph ], [ %on5, %._crit_edge ]
  %o6 = phi float [ 0.000000e+00, %.lr.ph ], [ %on6, %._crit_edge ]
  %o7 = phi float [ 0.000000e+00, %.lr.ph ], [ %on7, %._crit_edge ]
  br label %.crit_edge1
...
._crit_edge:                                      ; preds = %.crit_edge1
  ...
  %mn0 = select i1 %cmp0, float %x0, float 0.000000e+00
  %mn1 = select i1 %cmp1, float %x1, float 0.000000e+00
  %mn2 = select i1 %cmp2, float %x2, float 0.000000e+00
  %mn3 = select i1 %cmp3, float %x3, float 0.000000e+00
  %mn4 = select i1 %cmp4, float %x4, float 0.000000e+00
  %mn5 = select i1 %cmp5, float %x5, float 0.000000e+00
  %mn6 = select i1 %cmp6, float %x6, float 0.000000e+00
  %mn7 = select i1 %cmp7, float %x7, float 0.000000e+00
