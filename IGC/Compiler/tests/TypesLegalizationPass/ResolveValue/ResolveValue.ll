...
; Should resolve value through insertelement instruction
...
; CHECK-NEXT:    [[VECTOR:%.*]] = load <2 x i16>, ptr [[VECTOR_PTR]], align 4
; CHECK-NEXT:    [[TMP1:%.*]] = insertelement <2 x i16> [[VECTOR]], i16 999, i32 0
; CHECK-NEXT:    store <2 x i16> [[TMP1]], ptr [[VECTOR_PTR]], align 1
; CHECK-NEXT:    ret void
;
{
  %vector.ptr = alloca <2 x i16>, align 1
  %vector = load <2 x i16>, ptr %vector.ptr
  %1 = insertelement <2 x i16> %vector, i16 999, i32 0
  store <2 x i16> %1, ptr %vector.ptr, align 1
  ret void
}

; Should resolve value from intrinsic instruction
...
; Should resolve value from select instruction
...
; CHECK-NEXT:    store <2 x i16> [[FINAL_VECTOR]], ptr [[VECTOR_PTR]], align 1
; CHECK-NEXT:    [[ARRAY_PTR:%.*]] = alloca [2 x <2 x i16>], align 1
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr [2 x <2 x i16>], ptr [[ARRAY_PTR]], i32 0, i32 1
; CHECK-NEXT:    [[TMP2:%.*]] = load <2 x i16>, ptr [[TMP1]], align 4
; CHECK-NEXT:    [[ARRAY:%.*]] = load [2 x <2 x i16>], ptr [[ARRAY_PTR]], align 4
...
; CHECK-NEXT:    [[TMP5:%.*]] = getelementptr [2 x <2 x i16>], ptr [[ARRAY_PTR]], i32 0, i32 0
; CHECK-NEXT:    store <2 x i16> [[TMP3]], ptr [[TMP5]], align 1
; CHECK-NEXT:    [[TMP6:%.*]] = getelementptr [2 x <2 x i16>], ptr [[ARRAY_PTR]], i32 0, i32 1
; CHECK-NEXT:    store <2 x i16> [[TMP4]], ptr [[TMP6]], align 1
; CHECK-NEXT:    ret void
;
{
  %vector.ptr = alloca <2 x i16>, align 1
  ...
  store <2 x i16> %final.vector, ptr %vector.ptr, align 1

  %array.ptr = alloca [2 x <2 x i16>], align 1
  %array = load [2 x <2 x i16>], ptr %array.ptr
  ...
  store [2 x <2 x i16>] %final.array, ptr %array.ptr, align 1
  ret void
}
...
define float @cover.ResolveValue.cast.to.SelectInst.ExtractValue(ptr byval(%nested.struct.type) %arg)
; CHECK-LABEL: define float @cover.ResolveValue.cast.to.SelectInst.ExtractValue(
; CHECK-SAME: ptr byval([[NESTED_STRUCT_TYPE:%.*]]) [[ARG:%.*]]) {
; CHECK-NEXT:    [[PTR:%.*]] = getelementptr [[NESTED_STRUCT_TYPE]], ptr [[ARG]], i32 0
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr [[NESTED_STRUCT_TYPE]], ptr [[PTR]], i32 0, i32 1
; CHECK-NEXT:    [[TMP2:%.*]] = getelementptr [[SIMPLE_STRUCT_TYPE:%.*]], ptr [[TMP1]], i32 0, i32 1
; CHECK-NEXT:    [[TMP3:%.*]] = load float, ptr [[TMP2]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = load [[SIMPLE_STRUCT_TYPE]], ptr [[TMP1]], align 4
; CHECK-NEXT:    [[NESTED_STRUCT_INSTANCE:%.*]] = load [[NESTED_STRUCT_TYPE]], ptr [[PTR]], align 4
; CHECK-NEXT:    [[RESULT:%.*]] = fmul float [[TMP3]], 4.000000e+00
; CHECK-NEXT:    ret float [[RESULT]]
;
{
  %ptr = getelementptr %nested.struct.type, ptr %arg, i32 0
  %nested.struct.instance = load %nested.struct.type, ptr %ptr
