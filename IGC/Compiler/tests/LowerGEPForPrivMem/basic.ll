...
; RUN: igc_opt --opaque-pointers -enable-debugify -igc-priv-mem-to-reg -S < %s 2>&1 | FileCheck %s
; ------------------------------------------------
; LowerGEPForPrivMem
; ------------------------------------------------
...
; CHECK:    [[TMP2:%.*]] = load <4 x i32>, ptr [[TMP1]]
...
; CHECK:    store <4 x i32> [[TMP19]], ptr [[B:%.*]], align 16
; CHECK:    [[TMP20:%.*]] = alloca <4 x i32>
; CHECK:    [[TMP21:%.*]] = call <4 x i32> @llvm.genx.GenISA.vectorUniform.v4i32()
; CHECK:    store <4 x i32> [[TMP21]], ptr [[TMP20]]
; CHECK:    [[TMP22:%.*]] = load <4 x i32>, ptr [[TMP20]]
; CHECK:    [[TMP23:%.*]] = insertelement <4 x i32> [[TMP22]], i32 13, i32 3
; CHECK:    store <4 x i32> [[TMP23]], ptr [[TMP20]]
; CHECK:    ret void
;
  ...
  %4 = getelementptr [4 x i32], ptr %3, i32 0, i32 3
