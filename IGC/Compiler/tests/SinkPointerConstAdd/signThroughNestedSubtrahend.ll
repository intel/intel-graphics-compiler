...
; ------------------------------------------------
;
; Sign must accumulate correctly across nested Sub subtrahends, not just
; the first level: base - ((x + 100) - 30) folds to base - x - 70.
...
entry:
  ...
  %inner = add i64 %x, 100
  %mid = sub i64 %inner, 30
  %outer = sub i64 %base, %mid
  %ptr = inttoptr i64 %outer to i32 addrspace(1)*
...
; CHECK-NEXT: %[[OUTER:[0-9a-zA-Z_.]+]] = sub i64 %[[BASE]], %[[X]]
; CHECK-NEXT: %[[SUNK:[0-9a-zA-Z_.]+]] = add i64 %[[OUTER]], -70
