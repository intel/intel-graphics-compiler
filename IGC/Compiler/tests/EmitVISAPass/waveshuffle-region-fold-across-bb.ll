...
; ------------------------------------------------
; EmitVISAPass - a WaveBroadcast with a consumer in another basic block is
; emitted as a NoMask SIMD1 copy instead of being folded into a <0;1,0> read
; of the exec-mask defined source
; ------------------------------------------------
...
; CHECK: mov (M1_NM, 1) bcast(0,0)<1> base(0,0)<0;1,0>
; CHECK: _CSMain_003_consume:
; CHECK: add (M1, 32) result(0,0)<1> bcast(0,0)<0;1,0> lane32(0,0)<1;1,0>
...
produce:
  ; Non uniform source; a uniform one is not folded at all.
  %val = xor i32 %lane32, 7
  br label %join

join:
  %base = phi i32 [ %val, %produce ], [ 0, %entry ]
  %bcast = call i32 @llvm.genx.GenISA.WaveBroadcast.i32(i32 %base, i32 0, i32 0)

  ; The only consumer of %bcast sits on the other side of this branch.
