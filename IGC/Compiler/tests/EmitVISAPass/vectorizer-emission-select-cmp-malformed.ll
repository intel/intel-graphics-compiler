...
; CHECK: assertion failed: PredicateLengthIsCorrect, we can only emit non-uniform selects with matching predicate
...
._crit_edge575:                                   ; preds = %._crit_edge575, %.lr.ph89
  ...
  ; vectorized_cmp --> DOESN"T MAKE SENSE --> nonuniformness has 128 elements, predicate must be either Scalar or SIMD SIZE --> we have 8
  %vectorized_select_non = select <8 x i1> %vectorized_cmp, <8 x float> %nonuniformness, <8 x float> %vectorized_intrinsic
  %br = icmp eq i32 0, 0
