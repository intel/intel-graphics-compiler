...
; RUN: igc_opt -S %s --opaque-pointers -platformpvc -igc-emit-visa --regkey=DumpVISAASMToConsole=1 -simd-mode 16 &> %t_output.ll || true
; RUN: FileCheck --input-file %t_output.ll %s

; CHECK: input error, assertion failed: number of flags must be <= 32
...
._crit_edge575:                                   ; preds = %._crit_edge575, %.lr.ph89
  ...
  ;; currently there is no functionality to support cmp on non-uniform instructions
  ;; Predicate flags has at max 32 elements, nonuniformness has 128
  %vectorized_cmp_malformed = fcmp oeq <8 x float> %nonuniformness, zeroinitializer
  %vectorized_select_non = select <8 x i1> %vectorized_cmp_malformed, <8 x float> %nonuniformness, <8 x float> %vectorized_intrinsic
  %br = icmp eq i32 0, 0
