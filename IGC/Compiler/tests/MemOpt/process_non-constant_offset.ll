...
; CHECK: %gep1 = getelementptr float, float addrspace(4)* %srcptr, i64 %sext1
; CHECK: [[GEPADDR1:%.*]] = addrspacecast float addrspace(4)* %gep1 to float addrspace(1)*
; CHECK: [[BTC1:%.*]] = bitcast float addrspace(1)* [[GEPADDR1]] to <3 x float> addrspace(1)*
; CHECK: [[LD:%.*]] = load <3 x float>, <3 x float> addrspace(1)* [[BTC1]], align 4
; CHECK: %add4 = add nsw i32 %val3, 1
; CHECK: %sub4 = sub nsw i32 %add4, %val2
; CHECK: %sext4 = sext i32 %sub4 to i64
; CHECK: %gep4 = getelementptr float, float addrspace(4)* %dstptr, i64 %sext4
; CHECK: [[GEPADDR4:%.*]] = addrspacecast float addrspace(4)* %gep4 to float addrspace(1)*
; CHECK: [[BTC2:%.*]] = bitcast float addrspace(1)* [[GEPADDR4]] to <3 x float> addrspace(1)*
; CHECK: store <3 x float> [[DATA:%.*]], <3 x float> addrspace(1)* [[BTC2]], align 4
; CHECK: ret void
;
...
entry:
  ...
  %gep1 = getelementptr float, float addrspace(4)* %srcptr, i64 %sext1
  %gepaddr1 = addrspacecast float addrspace(4)* %gep1 to float addrspace(1)*
  %ld1 = load float, float addrspace(1)* %gepaddr1, align 4
  %add2 = add nsw i32 %val1, 2
  %sub2 = sub nsw i32 %add2, %val2
  %sext2 = sext i32 %sub2 to i64
  %gep2 = getelementptr float, float addrspace(4)* %srcptr, i64 %sext2
  %gepaddr2 = addrspacecast float addrspace(4)* %gep2 to float addrspace(1)*
  ...
  %gep3 = getelementptr float, float addrspace(4)* %srcptr, i64 %sext3
  %gepaddr3 = addrspacecast float addrspace(4)* %gep3 to float addrspace(1)*
  %ld3 = load float, float addrspace(1)* %gepaddr3, align 4
  %add4 = add nsw i32 %val3, 1
  %sub4 = sub nsw i32 %add4, %val2
  %sext4 = sext i32 %sub4 to i64
  %gep4 = getelementptr float, float addrspace(4)* %dstptr, i64 %sext4
  %gepaddr4 = addrspacecast float addrspace(4)* %gep4 to float addrspace(1)*
  store float %ld1, float addrspace(1)* %gepaddr4, align 4
  %add5 = add nsw i32 %val3, 2
  %sub5 = sub nsw i32 %add5, %val2
  %sext5 = sext i32 %sub5 to i64
  %gep5 = getelementptr float, float addrspace(4)* %dstptr, i64 %sext5
  %gepaddr5 = addrspacecast float addrspace(4)* %gep5 to float addrspace(1)*
  ...
  %gep6 = getelementptr float, float addrspace(4)* %dstptr, i64 %sext6
  %gepaddr6 = addrspacecast float addrspace(4)* %gep6 to float addrspace(1)*
  store float %ld3, float addrspace(1)* %gepaddr6, align 4
  ret void
}
...
; CHECK: [[GEP1:%.*]] = getelementptr float, float addrspace(4)* %srcptr, i64 [[SEXT1]]
; CHECK: [[GEPADDR1:%.*]] = addrspacecast float addrspace(4)* [[GEP1]] to float addrspace(1)*
; CHECK: [[GEP2:%.*]] = getelementptr float, float addrspace(1)* [[GEPADDR1]], i64 -2
; CHECK: [[BTC1:%.*]] = bitcast float addrspace(1)* [[GEP2]] to <3 x float> addrspace(1)*
; CHECK: [[LD:%.*]] = load <3 x float>, <3 x float> addrspace(1)* [[BTC1]], align 4
; CHECK: [[ADD2:%.*]] = add nsw i32 %val3, 3
; CHECK: [[SUB2:%.*]] = sub nsw i32 %val2, [[ADD2]]
; CHECK: [[SEXT2:%.*]] = sext i32 [[SUB2]] to i64
; CHECK: [[GEP3:%.*]] = getelementptr float, float addrspace(4)* %dstptr, i64 [[SEXT2]]
; CHECK: [[GEPADDR2:%.*]] = addrspacecast float addrspace(4)* [[GEP3]] to float addrspace(1)*
; CHECK: [[BTC2:%.*]] = bitcast float addrspace(1)* [[GEPADDR2]] to <3 x float> addrspace(1)*
; CHECK: store <3 x float> [[DATA:%.*]], <3 x float> addrspace(1)* [[BTC2]], align 4
; CHECK: ret void
...
; CHECK: [[GEP2:%.*]] = getelementptr float, float addrspace(1)* [[GEPADDR1]], i64 -2
; CHECK: [[BTC1:%.*]] = bitcast float addrspace(1)* [[GEP2]] to <3 x float> addrspace(1)*
; CHECK: [[LD1:%.*]] = load <3 x float>, <3 x float> addrspace(1)* [[BTC1]], align 4
