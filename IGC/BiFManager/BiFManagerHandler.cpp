...
void BiFManagerHandler::LinkBiF(llvm::Module& Module)
{
    ...
    for (auto bifsection_i = LoadedBiFSections.begin();
        ...
        ++bifsection_i)
    {
        auto BiFSection = &(*bifsection_i).second;
        ...
        {
            if (ld.linkInModule(std::move(*BiFSection),
                llvm::Linker::LinkOnlyNeeded, CallbackLinker))
            {
                IGC_ASSERT_MESSAGE(0, "Error linking generic builtin module");
            }
        }
...
void BiFManagerHandler::cleanModule(llvm::Module& Base)
{
            ...
            for (auto* pCallee : calledFuncs)
            {
                ...
                if (pCallee->isDeclaration())
                {
                        ...
                        ++bifsection_i)
                    {
                        ...
                        if (pSrcFunc && !pSrcFunc->isDeclaration()) break;
                    }
    ...
    for (auto bifsection_i = LoadedBiFSections.begin();
        ...
        ++bifsection_i)
    {
        CleanUnused(bifsection_i->second.get());
    }
}

void BiFManagerHandler::getPtrSizeInBits(llvm::Module* pMain)
{
    isPtrSizeInBits32 = (32 == pMain->getDataLayout().getPointerSizeInBits());
}
