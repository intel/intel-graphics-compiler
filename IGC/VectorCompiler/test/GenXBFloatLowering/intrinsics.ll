...
define <8 x bfloat> @test_sat(<8 x bfloat> %src) {
  ; CHECK: [[EXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %src to <8 x float>
  ; CHECK: [[RES:%[^ ]+]] = call <8 x float> @llvm.genx.sat.v8f32(<8 x float> [[EXT]])
  ; CHECK: [[TRUNC:%[^ ]+]] = fptrunc {{[a-z ]*}}<8 x float> [[RES]] to <8 x bfloat>
...
define <8 x bfloat> @test_maximum(<8 x bfloat> %a, <8 x bfloat> %b) {
  ; CHECK: [[AEXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %a to <8 x float>
  ; CHECK: [[BEXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %b to <8 x float>
...
define <8 x bfloat> @test_fma(<8 x bfloat> %a, <8 x bfloat> %b, <8 x bfloat> %c) {
  ; CHECK: [[AEXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %a to <8 x float>
  ; CHECK: [[BEXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %b to <8 x float>
  ; CHECK: [[CEXT:%[^ ]+]] = fpext {{[a-z ]*}}<8 x bfloat> %c to <8 x float>
