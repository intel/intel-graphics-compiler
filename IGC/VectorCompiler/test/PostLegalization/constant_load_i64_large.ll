...
define void @test(i64 %addr) {
  ; CHECK: %[[CONST:[^ ]+]] = call <1 x i64> @llvm.genx.constanti.v1i64(<1 x i64> {{(splat \(i64 4656722015785320448\)|<i64 4656722015785320448(, i64 4656722015785320448)*>)}})
  ; CHECK: %[[SPLAT:[^ ]+]] = call <2 x i64> @llvm.genx.rdregioni.v2i64.v1i64.i16(<1 x i64> %[[CONST]], i32 0, i32 2, i32 0, i16 0, i32 undef)
  ; CHECK: %[[VALUE:[^ ]+]] = call <2 x i64> @llvm.genx.wrconstregion.v2i64.v1i64.i16.i1(<2 x i64> %[[SPLAT]], <1 x i64> {{(splat \(i64 4656722015774834688\)|<i64 4656722015774834688(, i64 4656722015774834688)*>)}}, i32 1, i32 1, i32 1, i16 0, i32 undef, i1 true)
  ; CHECK: call void @llvm.genx.lsc.store.stateless.v1i1.i64.v2i64(<1 x i1> {{(splat \(i1 true\)|<i1 true(, i1 true)*>)}}, i8 4, i8 0, i8 0, i16 1, i32 0, i8 4, i8 2, i8 2, i8 0, i64 %addr, <2 x i64> %[[VALUE]], i32 0)
