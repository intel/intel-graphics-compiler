...
define dllexport spir_kernel void @kernelA(i32 addrspace(3)* %local_buffer) #0 {
  ...
  ; CHECK: %[[P3I32_TO_I64:.*]] = ptrtoint i32 addrspace(4)* %generic_ptr to i64
  ...
  ; CHECK: generic_ptr.tagged = inttoptr i64 %[[V2I32_TO_I64:.*]] to i32 addrspace(4)*

  %v = load i32, i32 addrspace(4)* %generic_ptr, align 4
  ; CHECK: %[[P4I32_TO_I64:.*]] = ptrtoint i32 addrspace(4)* %generic_ptr.tagged to i64
  ...
  ; CHECK: LocalBlock:
  ; CHECK:   %[[LOCAL_PTR:.*]] = addrspacecast i32 addrspace(4)* %generic_ptr.tagged to i32 addrspace(3)*
  ; CHECK:   %localLoad = load i32, i32 addrspace(3)* %[[LOCAL_PTR:.*]], align 4

  ; CHECK: GlobalBlock:
  ; CHECK:   %[[GLOBAL_PTR:.*]] = addrspacecast i32 addrspace(4)* %generic_ptr.tagged to i32 addrspace(1)*
  ...
  ; CHECK: store i32 %[[PHI_LOAD:.*]], i32 addrspace(3)* %local_buffer, align 4
  ret void
...
define dllexport spir_kernel void @kernelB(i32 addrspace(1)* %global_buffer) #0 {
  ...
  ; CHECK: %globalOrPrivateLoad = load i32, i32 addrspace(1)* %[[GLOBAL_PTR:.*]], align 4

  store i32 %v, i32 addrspace(1)* %global_buffer, align 4
  ; CHECK: store i32 %globalOrPrivateLoad, i32 addrspace(1)* %global_buffer, align 4
  ret void
}
