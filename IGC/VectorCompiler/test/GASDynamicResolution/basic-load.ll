...
;
; RUN: %opt_typed_ptrs %use_old_pass_manager% -GenXGASDynamicResolution -march=genx64 -mcpu=Xe2 -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-TYPED-PTRS
; RUN: %opt_opaque_ptrs %use_old_pass_manager% -GenXGASDynamicResolution -march=genx64 -mcpu=Xe2 -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-OPAQUE-PTRS
;
...
define dllexport spir_kernel void @kernelA(i32 addrspace(3)* %local_buffer) #0 {
  ...
  store i32 %v, i32 addrspace(3)* %local_buffer, align 4
  ; CHECK: %[[PHI_LOAD:.*]] = phi i32 [ %localLoad, %LocalBlock ], [ %globalLoad, %GlobalBlock ]
  ; CHECK-TYPED-PTRS: store i32 %[[PHI_LOAD:.*]], i32 addrspace(3)* %local_buffer, align 4
  ; CHECK-OPAQUE-PTRS: store i32 %[[PHI_LOAD:.*]], ptr addrspace(3) %local_buffer, align 4
  ret void

}

define dllexport spir_kernel void @kernelB(i32 addrspace(1)* %global_buffer) #0 {
  ...
  ; CHECK-TYPED-PTRS: %globalOrPrivateLoad = load i32, i32 addrspace(1)* %[[GLOBAL_PTR:.*]], align 4
  ; CHECK-OPAQUE-PTRS: %[[GLOBAL_PTR:.*]] = addrspacecast ptr addrspace(4) %generic_ptr to ptr addrspace(1)
  ; CHECK-OPAQUE-PTRS: %globalOrPrivateLoad = load i32, ptr addrspace(1) %[[GLOBAL_PTR:.*]], align 4

  store i32 %v, i32 addrspace(1)* %global_buffer, align 4
  ; CHECK-TYPED-PTRS: store i32 %globalOrPrivateLoad, i32 addrspace(1)* %global_buffer, align 4
  ; CHECK-OPAQUE-PTRS: store i32 %globalOrPrivateLoad, ptr addrspace(1) %global_buffer, align 4
  ret void
}
