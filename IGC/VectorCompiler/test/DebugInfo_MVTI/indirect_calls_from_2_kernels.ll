...
; CHECK-NEXT:     l.0 _Z24__cm_intrinsic_impl_test.foo___vyfvyf
; CHECK-NEXT: [2] f_f (K)
; CHECK-NEXT:     l.0 _Z24__cm_intrinsic_impl_test
; CHECK-NEXT: [3] bar___vyfvyf (I)
; CHECK-NEXT:     l.0 _Z24__cm_intrinsic_impl_test.bar___vyfvyf

source_filename = "indirect_calls_from_2_kernels.ll"
...
define internal spir_func <16 x float> @foo___vyfvyf(<16 x float> %a, <16 x float> %b, <16 x i1> %__mask) addrspace(9) #4 !FuncArgSize !18 !FuncRetSize !19 {
allocas:
...
define internal spir_func <16 x float> @bar___vyfvyf(<16 x float> %a, <16 x float> %b, <16 x i1> %__mask) addrspace(9) #5 !FuncArgSize !18 !FuncRetSize !19 {
allocas:
...
define dllexport void @f_f(float* noalias %RET, float* noalias %aFOO, i64 %privBase) #6 {
  ...
  %calltmp = call spir_func addrspace(9) <16 x float> %fptr(<16 x float> %src1, <16 x float> %src1, <16 x i1> zeroinitializer) #7, !FuncArgSize !18, !FuncRetSize !19
...
define dllexport void @result(float* noalias %RET, i64 %privBase) #6 {
  ...
  %rawaddr = extractelement <1 x i64> %rawaddr_v, i32 0
  %fptr = inttoptr i64 %rawaddr to <16 x float> (<16 x float>, <16 x float>, <16 x i1>) addrspace(9)*
  %calltmp = call spir_func addrspace(9) <16 x float> %fptr(<16 x float> zeroinitializer, <16 x float> zeroinitializer, <16 x i1> zeroinitializer) #7, !FuncArgSize !18, !FuncRetSize !19
  %svm_st_ptrtoint = ptrtoint float* %RET to i64
