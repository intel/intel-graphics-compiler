...
;
; RUN: %opt_typed_ptrs %use_old_pass_manager% -vc-use-bindless-buffers -GenXPromoteStatefulToBindless  -march=genx64 -mtriple=spir64-unknown-unknown -mcpu=Xe2 -S < %s | FileCheck %s
; RUN: %opt_opaque_ptrs %use_old_pass_manager% -vc-use-bindless-buffers -GenXPromoteStatefulToBindless  -march=genx64 -mtriple=spir64-unknown-unknown -mcpu=Xe2 -S < %s | FileCheck %s
; ------------------------------------------------
...
; CHECK: {{(#dbg_value\(|call void @llvm\.dbg\.value\(metadata )}}<8 x i32> [[VAL1_V]]{{(, |, metadata )}}[[VAL1_MD:![0-9]*]]{{(, |, metadata )}}!DIExpression(){{(, |\), !dbg )}}[[VAL1_LOC]]{{\)?}}
