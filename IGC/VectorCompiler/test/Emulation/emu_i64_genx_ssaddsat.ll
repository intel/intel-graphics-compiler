...
; CHECK-NEXT: [[SIGN_SRC0:%[^ ]+]] = and <[[ET]]> [[Hi_l]], {{(splat \(|<)}}[[MASK_BIT31:[^>)]+]]{{(\)|>)}}
; CHECK-NEXT: [[SIGN_SRC1:%[^ ]+]] = and <[[ET]]> [[Hi_r]], {{(splat \(|<)}}[[MASK_BIT31]]{{(\)|>)}}
; CHECK-NEXT: [[SIGN_RES:%[^ ]+]] = and <[[ET]]> [[HiADDC_2_ADD]], {{(splat \(|<)}}[[MASK_BIT31]]{{(\)|>)}}
...
; CHECK-NEXT: [[LO1:%[^ ]+]] = select <[[BL]]> [[F_OVERFLOW]], <[[ET]]> {{(splat \(|<)}}[[ONES:i32 -1(, i32 -1)*]]{{(\)|>)}}, <[[ET]]> [[LoADDC_ADD]]
; CHECK-NEXT: [[HI1:%[^ ]+]] = select <[[BL]]> [[F_OVERFLOW]], <[[ET]]> {{(splat \(|<)}}[[P_MASK:[^>)]+]]{{(\)|>)}}, <[[ET]]> [[HiADDC_2_ADD]]
...
; CHECK-NEXT: [[Hi:%[^ ]+]] = select <[[BL]]> [[F_NEG_OVERFLOW]], <[[ET]]> {{(splat \(|<)}}[[MASK_BIT31]]{{(\)|>)}}, <[[ET]]> [[HI1]]
...
; CHECK-NEXT: [[LO1:%[^ ]+]] = select <[[BL]]> [[F_OVERFLOW]], <[[ET]]> {{(splat \(|<)}}[[ONES:i32 -1]]{{(\)|>)}}, <[[ET]]> [[LoADDC_ADD]]
; CHECK-NEXT: [[HI1:%[^ ]+]] = select <[[BL]]> [[F_OVERFLOW]], <[[ET]]> {{(splat \(|<)}}[[P_MASK:[^>)]+]]{{(\)|>)}}, <[[ET]]> [[HiADDC_2_ADD]]
