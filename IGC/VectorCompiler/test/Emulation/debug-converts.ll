...
;
; Convert instructions lowering

; CHECK: void @test_emulate{{.*}} !dbg [[SCOPE:![0-9]*]]
; CHECK: entry:
; CHECK-DAG: {{(#dbg_value\(|call void @llvm\.dbg\.value\(metadata )}}i64 [[VAL1_V:%[A-z0-9.]*]]{{(, |, metadata )}}[[VAL1_MD:![0-9]*]]{{(, |, metadata )}}!DIExpression(){{(, |\), !dbg )}}[[VAL1_LOC:![0-9]*]]{{\)?}}
; CHECK-DAG: [[VAL1_V]] = {{.*}}, !dbg [[VAL1_LOC]]
; CHECK-DAG: {{(#dbg_value\(|call void @llvm\.dbg\.value\(metadata )}}float [[VAL2_V:%[A-z0-9.]*]]{{(, |, metadata )}}[[VAL2_MD:![0-9]*]]{{(, |, metadata )}}!DIExpression(){{(, |\), !dbg )}}[[VAL2_LOC:![0-9]*]]{{\)?}}
...
; CHECK-DAG: {{(#dbg_value\(|call void @llvm\.dbg\.value\(metadata )}}float [[VAL4_V:%[A-z0-9.]*]]{{(, |, metadata )}}[[VAL4_MD:![0-9]*]]{{(, |, metadata )}}!DIExpression(){{(, |\), !dbg )}}[[VAL4_LOC:![0-9]*]]{{\)?}}
; CHECK-DAG: [[VAL4_V]] = {{.*}}, !dbg [[VAL4_LOC]]
; CHECK: store float{{.*}} !dbg [[STR1_LOC:![0-9]*]]

define void @test_emulate(float* %dst) !dbg !6 {
entry:
  %0 = fptosi float 1.300000e+01 to i64, !dbg !15
  call void @llvm.dbg.value(metadata i64 %0, metadata !9, metadata !DIExpression()), !dbg !15
  %1 = sitofp i64 %0 to float, !dbg !16
  call void @llvm.dbg.value(metadata float %1, metadata !11, metadata !DIExpression()), !dbg !16
  %2 = fptoui float %1 to i64, !dbg !17
  call void @llvm.dbg.value(metadata i64 %2, metadata !13, metadata !DIExpression()), !dbg !17
  %3 = uitofp i64 %2 to float, !dbg !18
  call void @llvm.dbg.value(metadata float %3, metadata !14, metadata !DIExpression()), !dbg !18
  store float %3, float* %dst, !dbg !19
  ret void, !dbg !20
}
...
!1 = !DIFile(filename: "converts.ll", directory: "/")
!2 = !{}
!3 = !{i32 6}
!4 = !{i32 4}
