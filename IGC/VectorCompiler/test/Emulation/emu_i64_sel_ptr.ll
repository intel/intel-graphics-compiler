...
define float addrspace(1)* @test_constexpr(i1 %cond) {
  ; CHECK-TYPED-PTRS: [[SEL_LO:%[^ ]+]] = select i1 %cond, <1 x i32> zeroinitializer, <1 x i32> {{.*}}
  ; CHECK-TYPED-PTRS: [[SEL_HI:%[^ ]+]] = select i1 %cond, <1 x i32> zeroinitializer, <1 x i32> {{.*}}
  ; CHECK-OPAQUE-PTRS: [[SEL_LO:%[^ ]+]] = select i1 %cond, <1 x i32> zeroinitializer, <1 x i32> {{.*}}
  ; CHECK-OPAQUE-PTRS: [[SEL_HI:%[^ ]+]] = select i1 %cond, <1 x i32> zeroinitializer, <1 x i32> {{.*}}
