...
; RUN: -GenXPredRegionLowering -vc-lower-predregion=true -march=genx64 -mcpu=Xe2 \
...
; CHECK: %[[sel:.*]] = select <16 x i1> %[[rdpr]], <16 x i16> {{(splat \(i16 1\)|<i16 1(, i16 1)*>)}}, <16 x i16> zeroinitializer
; CHECK: %[[cmp:.*]] = icmp eq <16 x i16> %[[sel]], {{(splat \(i16 1\)|<i16 1(, i16 1)*>)}}
}
...
; CHECK: %[[sel:.*]] = select <16 x i1> %pred.new, <16 x i16> {{(splat \(i16 1\)|<i16 1(, i16 1)*>)}}, <16 x i16> zeroinitializer
; CHECK: %[[cmp:.*]] = icmp eq <16 x i16> %[[sel]], {{(splat \(i16 1\)|<i16 1(, i16 1)*>)}}
