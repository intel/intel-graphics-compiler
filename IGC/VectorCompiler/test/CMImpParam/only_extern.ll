...
; RUN: %opt_typed_ptrs %use_old_pass_manager% %pass_pref%CMImpParam -march=genx64 -mcpu=XeHP -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-TYPED-PTRS
; RUN: %opt_opaque_ptrs %use_old_pass_manager% %pass_pref%CMImpParam -march=genx64 -mcpu=XeHP -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-OPAQUE-PTRS
