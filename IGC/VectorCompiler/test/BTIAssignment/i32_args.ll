...
; RUN: %opt_typed_ptrs %use_old_pass_manager% -GenXBTIAssignment -march=genx64 -mcpu=Gen9 -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-TYPED-PTRS
; RUN: %opt_opaque_ptrs %use_old_pass_manager% -GenXBTIAssignment -march=genx64 -mcpu=Gen9 -S < %s | FileCheck %s --check-prefixes=CHECK,CHECK-OPAQUE-PTRS
...
; CHECK-TYPED-PTRS-DAG: [[IMAGE_ARRAY_NODE]] = !{void (i32, i32)* @image_array, null, null, null, [[IMAGE_ARRAY_BTIS:![0-9]+]]}
; CHECK-OPAQUE-PTRS-DAG: [[IMAGE_ARRAY_NODE]] = !{ptr @image_array, null, null, null, [[IMAGE_ARRAY_BTIS:![0-9]+]]}
; CHECK-DAG: [[IMAGE_ARRAY_BTIS]] = !{i32 0, i32 1}
; CHECK-TYPED-PTRS-DAG: [[IMAGE_MEDIA_BLOCK_NODE]] = !{void (i32)* @image_media_block, null, null, null, [[IMAGE_MEDIA_BLOCK_BTIS:![0-9]+]]}
; CHECK-OPAQUE-PTRS-DAG: [[IMAGE_MEDIA_BLOCK_NODE]] = !{ptr @image_media_block, null, null, null, [[IMAGE_MEDIA_BLOCK_BTIS:![0-9]+]]}
