...
; RUN: -mattr=+ocl_runtime -march=genx64 -mcpu=Xe2 -vc-ret-reg-size=100 -S < %s | FileCheck %s

; COM: Force -vc-ret-reg-size=0 to pass value on stack.

; RUN: %opt %use_old_pass_manager% -GenXPrologEpilogInsertion \
; RUN: -mattr=+ocl_runtime -march=genx64 -mcpu=Xe2 -vc-ret-reg-size=0 -S < %s | FileCheck %s
