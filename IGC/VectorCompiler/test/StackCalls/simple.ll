...
; RUN: -mcpu=XeLPG -S -vc-arg-reg-size=32 -vc-ret-reg-size=12 < %s | FileCheck %s --check-prefixes=CHECK,CHECK-TYPED-PTRS
; RUN: %opt_opaque_ptrs %use_old_pass_manager% -GenXPrologEpilogInsertion -dce -mattr=+ocl_runtime -march=genx64 \
; RUN: -mcpu=XeLPG -S -vc-arg-reg-size=32 -vc-ret-reg-size=12 < %s | FileCheck %s --check-prefixes=CHECK,CHECK-OPAQUE-PTRS
