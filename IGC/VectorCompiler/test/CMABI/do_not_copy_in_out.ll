...
; CHECK: define internal spir_func void @stackcall
; CHECK-SAME: float addrspace(4)*
; CHECK-SAME: float addrspace(4)*
...
; Function Attrs: noinline nounwind
define internal spir_func void @stackcall(float addrspace(4)* %A, float addrspace(4)* %C) #6 {
entry:
  %val = load float, float addrspace(4)* %A
  %res = fadd float %val, zeroinitializer
  store float %res, float addrspace(4)* %C
  ret void
}
...
entry:
  ...
  call spir_func void @stackcall(float addrspace(4)* %add.ptr4.i, float addrspace(4)* %add.ptr7.i) #6
  ret void
}
...
attributes #6 = { noinline nounwind "CMStackCall" }
