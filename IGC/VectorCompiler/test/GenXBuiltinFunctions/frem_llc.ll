...
; RUN: %llc_typed_ptrs %s -march=genx64 -mcpu=XeLP -vc-builtins-bif-path=%VC_BIF_XeLP_TYPED_PTRS% \
; RUN: -vc-skip-ocl-runtime-info -finalizer-opts='-asmToConsole' -o /dev/null | FileCheck %s
; RUN: %llc_opaque_ptrs %s -march=genx64 -mcpu=XeLP -vc-builtins-bif-path=%VC_BIF_XeLP_OPAQUE_PTRS% \
; RUN: -vc-skip-ocl-runtime-info -finalizer-opts='-asmToConsole' -o /dev/null | FileCheck %s
