...
bool GenXDeadVectorRemoval::runOnFunction(Function &F) {
  ...
  for (auto &I : instructions(F)) {
    ...
    if (LE.getLiveElements(&I).isAllDead()) {
      LLVM_DEBUG(dbgs() << "Adding to erase list\n");
      ToErase.push_back(&I);
    }
  }
...
Value *GenXDeadVectorRemoval::trySimplify(Value *V,
  ...
  // TODO: remove predef registers check in future.
  if (GenXIntrinsic::isReadWritePredefReg(V))
    return V;
...
Constant *GenXDeadVectorRemoval::trySimplify(ConstantAggregate *CA,
  ...
  for (auto &U : CA->operands()) {
    ...
    if (OpLiveElems.isAnyDead()) {
      if (OpLiveElems.isAllDead())
        U.set(UndefValue::get(U->getType()));
      else if (auto CD = cast<ConstantData>(U))
        U.set(trySimplify(CD, OpLiveElems));
    }
  }
...
Constant *GenXDeadVectorRemoval::trySimplify(ConstantData *CD,
  ...
  if (auto CDS = dyn_cast<ConstantDataSequential>(CD)) {
    ...
    for (unsigned Idx = 0; Idx < CDS->getNumElements(); Idx++) {
      auto Elem = CDS->getElementAsConstant(Idx);
