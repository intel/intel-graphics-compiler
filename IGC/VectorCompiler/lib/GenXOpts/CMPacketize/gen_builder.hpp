...
#ifndef GEN_BUILDER_HPP
#define GEN_BUILDER_HPP

GlobalVariable *GLOBAL_STRING(StringRef Str, const Twine &Name = "",
                              unsigned AddressSpace = 0) {
  return IRB->CreateGlobalString(Str, Name, AddressSpace);
}

CallInst *MEMSET(Value *Ptr, Value *Val, uint64_t Size, unsigned Align,
                 bool IsVolatile = false, MDNode *TBAATag = nullptr,
                 MDNode *ScopeTag = nullptr, MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemSet(Ptr, Val, Size, IGCLLVM::getCorrectAlign(Align),
                           IsVolatile, TBAATag, ScopeTag, NoAliasTag);
}

CallInst *MEMSET(Value *Ptr, Value *Val, Value *Size, unsigned Align,
                 bool IsVolatile = false, MDNode *TBAATag = nullptr,
                 MDNode *ScopeTag = nullptr, MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemSet(Ptr, Val, Size, IGCLLVM::getCorrectAlign(Align),
                           IsVolatile, TBAATag, ScopeTag, NoAliasTag);
...
CallInst *MEMCOPY(Value *Dst, Value *Src, uint64_t Size, unsigned Align,
                  bool IsVolatile = false, MDNode *TBAATag = nullptr,
                  MDNode *TBAAStructTag = nullptr, MDNode *ScopeTag = nullptr,
                  MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemCpy(Dst, IGCLLVM::getCorrectAlign(Align), Src,
                           IGCLLVM::getCorrectAlign(Align), Size, IsVolatile,
                           TBAATag, TBAAStructTag, ScopeTag, NoAliasTag);
}

CallInst *MEMCOPY(Value *Dst, Value *Src, Value *Size, unsigned Align,
                  bool IsVolatile = false, MDNode *TBAATag = nullptr,
                  MDNode *TBAAStructTag = nullptr, MDNode *ScopeTag = nullptr,
                  MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemCpy(Dst, IGCLLVM::getCorrectAlign(Align), Src,
...
CallInst *ELEMENT_UNORDERED_ATOMIC_MEM_CPY(Value *Dst, unsigned DstAlign,
                                           Value *Src, unsigned SrcAlign,
                                           uint64_t Size, uint32_t ElementSize,
                                           MDNode *TBAATag = nullptr,
                                           MDNode *TBAAStructTag = nullptr,
                                           MDNode *ScopeTag = nullptr,
                                           MDNode *NoAliasTag = nullptr) {
  return IRB->CreateElementUnorderedAtomicMemCpy(
      Dst, IGCLLVM::getAlignmentValueIfNeeded(IGCLLVM::getAlign(DstAlign)), Src,
      IGCLLVM::getAlignmentValueIfNeeded(IGCLLVM::getAlign(SrcAlign)),
      IRB->getInt64(Size), ElementSize, TBAATag, TBAAStructTag, ScopeTag,
      NoAliasTag);
}

CallInst *ELEMENT_UNORDERED_ATOMIC_MEM_CPY(Value *Dst, unsigned DstAlign,
                                           Value *Src, unsigned SrcAlign,
                                           Value *Size, uint32_t ElementSize,
                                           MDNode *TBAATag = nullptr,
                                           MDNode *TBAAStructTag = nullptr,
                                           MDNode *ScopeTag = nullptr,
                                           MDNode *NoAliasTag = nullptr) {
  return IRB->CreateElementUnorderedAtomicMemCpy(
      Dst, IGCLLVM::getAlignmentValueIfNeeded(IGCLLVM::getAlign(DstAlign)), Src,
      IGCLLVM::getAlignmentValueIfNeeded(IGCLLVM::getAlign(SrcAlign)), Size,
      ElementSize, TBAATag, TBAAStructTag, ScopeTag, NoAliasTag);
}

CallInst *MEMMOVE(Value *Dst, Value *Src, uint64_t Size, unsigned Align,
                  bool isVolatile = false, MDNode *TBAATag = nullptr,
                  MDNode *ScopeTag = nullptr, MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemMove(Dst, IGCLLVM::getCorrectAlign(Align), Src,
                            IGCLLVM::getCorrectAlign(Align), Size, isVolatile,
                            TBAATag, ScopeTag, NoAliasTag);
}

CallInst *MEMMOVE(Value *Dst, Value *Src, Value *Size, unsigned Align,
                  bool isVolatile = false, MDNode *TBAATag = nullptr,
                  MDNode *ScopeTag = nullptr, MDNode *NoAliasTag = nullptr) {
  return IRB->CreateMemMove(Dst, IGCLLVM::getCorrectAlign(Align), Src,
                            IGCLLVM::getCorrectAlign(Align), Size, isVolatile,
...
CallInst *ADD_REDUCE(Value *Src) { return IRB->CreateAddReduce(Src); }

CallInst *MUL_REDUCE(Value *Src) { return IRB->CreateMulReduce(Src); }

CallInst *AND_REDUCE(Value *Src) { return IRB->CreateAndReduce(Src); }

CallInst *OR_REDUCE(Value *Src) { return IRB->CreateOrReduce(Src); }

CallInst *XOR_REDUCE(Value *Src) { return IRB->CreateXorReduce(Src); }
...
CallInst *FP_MAX_REDUCE(Value *Src) { return IRB->CreateFPMaxReduce(Src); }

CallInst *FP_MIN_REDUCE(Value *Src) { return IRB->CreateFPMinReduce(Src); }
...
CallInst *ASSUMPTION(Value *Cond) { return IRB->CreateAssumption(Cond); }

CallInst *BINARY_INTRINSIC(Intrinsic::ID ID, Value *LHS, Value *RHS,
                           const Twine &Name = "") {
...
ReturnInst *RET_VOID() { return IRB->CreateRetVoid(); }
...
BranchInst *BR(BasicBlock *Dest) { return IRB->CreateBr(Dest); }
...
InvokeInst *INVOKE(Value *Callee, BasicBlock *NormalDest,
                   BasicBlock *UnwindDest, ArrayRef<Value *> Args = None,
                   const Twine &Name = "") {
  auto *PTy = Callee->getType();
  auto *FTy = cast<FunctionType>(IGCLLVM::getNonOpaquePtrEltTy(PTy));
  return IRB->CreateInvoke(FTy, Callee, NormalDest, UnwindDest, Args, Name);
}
...
InvokeInst *INVOKE(Value *Callee, BasicBlock *NormalDest,
  ...
  auto *PTy = Callee->getType();
  auto *FTy = cast<FunctionType>(IGCLLVM::getNonOpaquePtrEltTy(PTy));
  return IRB->CreateInvoke(FTy, Callee, NormalDest, UnwindDest, Args, OpBundles,
                           Name);
}

ResumeInst *RESUME(Value *Exn) { return IRB->CreateResume(Exn); }

CleanupReturnInst *CLEANUP_RET(CleanupPadInst *CleanupPad,
                               BasicBlock *UnwindBB = nullptr) {
  return IRB->CreateCleanupRet(CleanupPad, UnwindBB);
}

CatchSwitchInst *CATCH_SWITCH(Value *ParentPad, BasicBlock *UnwindBB,
                              unsigned NumHandlers, const Twine &Name = "") {
  return IRB->CreateCatchSwitch(ParentPad, UnwindBB, NumHandlers, Name);
}

CatchPadInst *CATCH_PAD(Value *ParentPad, ArrayRef<Value *> Args,
                        const Twine &Name = "") {
  return IRB->CreateCatchPad(ParentPad, Args, Name);
}

CleanupPadInst *CLEANUP_PAD(Value *ParentPad, ArrayRef<Value *> Args = None,
                            const Twine &Name = "") {
...
UnreachableInst *UNREACHABLE() { return IRB->CreateUnreachable(); }

Value *ADD(Value *LHS, Value *RHS, const Twine &Name = "", bool HasNUW = false,
           bool HasNSW = false) {
...
Value *SUB(Value *LHS, Value *RHS, const Twine &Name = "", bool HasNUW = false,
           bool HasNSW = false) {
...
Value *MUL(Value *LHS, Value *RHS, const Twine &Name = "", bool HasNUW = false,
           bool HasNSW = false) {
...
Value *SHL(Value *LHS, Value *RHS, const Twine &Name = "", bool HasNUW = false,
           bool HasNSW = false) {
  return IRB->CreateShl(LHS, RHS, Name, HasNUW, HasNSW);
}

Value *SHL(Value *LHS, const APInt &RHS, const Twine &Name = "",
...
Value *LSHR(Value *LHS, Value *RHS, const Twine &Name = "",
            bool IsExact = false) {
  return IRB->CreateLShr(LHS, RHS, Name, IsExact);
}

Value *LSHR(Value *LHS, const APInt &RHS, const Twine &Name = "",
            bool IsExact = false) {
  return IRB->CreateLShr(LHS, RHS, Name, IsExact);
}

Value *LSHR(Value *LHS, uint64_t RHS, const Twine &Name = "",
            bool IsExact = false) {
  return IRB->CreateLShr(LHS, RHS, Name, IsExact);
}

Value *ASHR(Value *LHS, Value *RHS, const Twine &Name = "",
            bool IsExact = false) {
  return IRB->CreateAShr(LHS, RHS, Name, IsExact);
}

Value *ASHR(Value *LHS, const APInt &RHS, const Twine &Name = "",
            bool IsExact = false) {
  return IRB->CreateAShr(LHS, RHS, Name, IsExact);
}
...
Value *BINOP(Instruction::BinaryOps Opc, Value *LHS, Value *RHS,
             const Twine &Name = "", MDNode *FPMathTag = nullptr) {
  return IRB->CreateBinOp(Opc, LHS, RHS, Name, FPMathTag);
}

Value *NEG(Value *V, const Twine &Name = "", bool HasNUW = false,
           bool HasNSW = false) {
...
Value *NOT(Value *V, const Twine &Name = "") { return IRB->CreateNot(V, Name); }

AllocaInst *ALLOCA(Type *Ty, unsigned AddrSpace, Value *ArraySize = nullptr,
                   const Twine &Name = "") {
  return IRB->CreateAlloca(Ty, ArraySize, Name,
                           AddrSpace /* IGCLLVM wrapper interface */);
}
...
inline LoadInst *ALIGNED_LOAD(Value *Ptr, IGCLLVM::Align Align,
                              const char *Name) {
  auto *PtrTy = IGCLLVM::getNonOpaquePtrEltTy(Ptr->getType());
  return IRB->CreateAlignedLoad(PtrTy, Ptr, Align, Name);
}
...
inline LoadInst *ALIGNED_LOAD(Value *Ptr, IGCLLVM::Align Align, bool IsVolatile,
                              const Twine &Name = "") {
  auto *PtrTy = IGCLLVM::getNonOpaquePtrEltTy(Ptr->getType());
  return IRB->CreateAlignedLoad(PtrTy, Ptr, Align, IsVolatile, Name);
}

inline StoreInst *ALIGNED_STORE(Value *Val, Value *Ptr, IGCLLVM::Align Align,
                                bool IsVolatile = false) {
  return IRB->CreateAlignedStore(Val, Ptr, Align, IsVolatile);
}

FenceInst *FENCE(AtomicOrdering Ordering,
...
Value *IN_BOUNDS_GEP(Value *Ptr, ArrayRef<Value *> IdxList,
                     const Twine &Name = "") {
  return IRB->CreateInBoundsGEP(Ptr, IdxList, Name);
}

Value *IN_BOUNDS_GEP(Type *Ty, Value *Ptr, ArrayRef<Value *> IdxList,
                     const Twine &Name = "") {
...
Value *CONST_GEP1_32(Type *Ty, Value *Ptr, unsigned Idx0,
                     const Twine &Name = "") {
  return IRB->CreateConstGEP1_32(Ty, Ptr, Idx0, Name);
}

Value *CONST_IN_BOUNDS_GEP1_32(Type *Ty, Value *Ptr, unsigned Idx0,
                               const Twine &Name = "") {
  return IRB->CreateConstInBoundsGEP1_32(Ty, Ptr, Idx0, Name);
}

Value *CONST_GEP2_32(Type *Ty, Value *Ptr, unsigned Idx0, unsigned Idx1,
                     const Twine &Name = "") {
  return IRB->CreateConstGEP2_32(Ty, Ptr, Idx0, Idx1, Name);
}

Value *CONST_IN_BOUNDS_GEP2_32(Type *Ty, Value *Ptr, unsigned Idx0,
                               unsigned Idx1, const Twine &Name = "") {
  return IRB->CreateConstInBoundsGEP2_32(Ty, Ptr, Idx0, Idx1, Name);
}

Value *CONST_GEP1_64(Type *Ty, Value *Ptr, uint64_t Idx0,
                     const Twine &Name = "") {
  return IRB->CreateConstGEP1_64(Ty, Ptr, Idx0, Name);
}

Value *CONST_IN_BOUNDS_GEP1_64(Type *Ty, Value *Ptr, uint64_t Idx0,
                               const Twine &Name = "") {
  return IRB->CreateConstInBoundsGEP1_64(Ty, Ptr, Idx0, Name);
}

Value *CONST_GEP2_64(Type *Ty, Value *Ptr, uint64_t Idx0, uint64_t Idx1,
                     const Twine &Name = "") {
  return IRB->CreateConstGEP2_64(Ty, Ptr, Idx0, Idx1, Name);
}

Value *CONST_IN_BOUNDS_GEP2_64(Type *Ty, Value *Ptr, uint64_t Idx0,
...
Value *POINTER_BIT_CAST_OR_ADDR_SPACE_CAST(Value *V, Type *DestTy,
                                           const Twine &Name = "") {
...
Value *ICMP(CmpInst::Predicate P, Value *LHS, Value *RHS,
            const Twine &Name = "") {
  return IRB->CreateICmp(P, LHS, RHS, Name);
}

Value *FCMP(CmpInst::Predicate P, Value *LHS, Value *RHS,
            const Twine &Name = "", MDNode *FPMathTag = nullptr) {
...
Value *VINSERT(Value *Vec, Value *NewElt, uint64_t Idx,
               const Twine &Name = "") {
  return IRB->CreateInsertElement(Vec, NewElt, Idx, Name);
}
...
Value *VSHUFFLE(Value *V1, Value *V2, ArrayRef<uint32_t> IntMask,
                const Twine &Name = "") {
  auto *Mask = ConstantDataVector::get(getContext(), IntMask);
  return IRB->CreateShuffleVector(V1, V2, Mask, Name);
}

Value *EXTRACT_VALUE(Value *Agg, ArrayRef<unsigned> Idxs,
                     const Twine &Name = "") {
  return IRB->CreateExtractValue(Agg, Idxs, Name);
}

Value *INSERT_VALUE(Value *Agg, Value *Val, ArrayRef<unsigned> Idxs,
                    const Twine &Name = "") {
  return IRB->CreateInsertValue(Agg, Val, Idxs, Name);
}

LandingPadInst *LANDING_PAD(Type *Ty, unsigned NumClauses,
                            const Twine &Name = "") {
...
Value *EXTRACT_INTEGER(const DataLayout &DL, Value *From,
                       IntegerType *ExtractedTy, uint64_t Offset,
                       const Twine &Name) {
  return IRB->CreateExtractInteger(DL, From, ExtractedTy, Offset, Name);
}

CallInst *ALIGNMENT_ASSUMPTION(const DataLayout &DL, Value *PtrValue,
                               unsigned Alignment,
